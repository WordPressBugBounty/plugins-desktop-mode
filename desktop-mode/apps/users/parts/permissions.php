 ...
 *
 * UI-side gating is purely UX polish — the actions and the REST
 * routes in `rest.php` re-validate every cap and every per-target
 ...
 * options the viewer can't apply; the mutation paths call this same
 * filtered helper and reject anything outside it.
 *
...
function openstation_users_window_assignable_roles( $viewer_id, $target_id = 0 ) {
	...
	// which is NOT auto-loaded by the time `init` fires.
	 ...
	 *
	 * Returning a SUPERSET widens the mutation paths too — the
	 * bulk-role and the create-user paths validate the requested
	 * role against this same filtered list, so only add roles you
	 * genuinely intend to make assignable.
	 *
