<?php
/**
 * Comments app — AI moderation.
 *
 * Lives on its own toggle ("Use AI to score new comments", surfaced
 * in OpenStation Preferences → Features for users with
 * `manage_options`). Off by default; when on, every new comment posted
 * to the site is queued for AI analysis through the AI Copilot's
 * existing `desktop_mode_ai_analyze_comment` job. The structured
 * verdict (`{ harmful, spam, topic, ai_summary }`) lands in comment
 * meta via `openstation_ai_save_meta()`; the app reads it back through
 * the `openstation_comments_window_spam_score` filter to bump the
 * per-row score, and surfaces the prose summary in a REST field.
 *
 * The app NEVER runs the AI itself — it routes everything through the
 * AI Copilot pipeline, which routes generation through the WordPress
 * AI Client — so a site's Settings → Connectors config is the single
 * source of truth for provider credentials and selection.
 *
 * SECURITY POSTURE
 * ================
 *
 ...
 *     the Preferences UI hides the toggle for non-admins.
 ...
 * written by live installs. The mismatch between this constant's name
 * and its value is deliberate — it is NOT a half-finished rename.
 */
