<?php
/**
 * Desktop Mode — Pinned notes Heartbeat sync.
 *
 ...
 * @return array
 */
function desktop_mode_notes_heartbeat_received( $response, $data ) {
	if ( ! is_array( $response ) ) {
		$response = array();
	}
	if ( empty( $data['desktop_mode_notes_subscribe'] ) || ! is_array( $data['desktop_mode_notes_subscribe'] ) ) {
		return $response;
	}
	...
	$sub       = $data['desktop_mode_notes_subscribe'];
	...
	$response['desktop_mode_notes'] = desktop_mode_notes_compute_heartbeat_delta( $known_ids, $since_ms, 100 );

	return $response;
}
add_filter( 'heartbeat_received', 'desktop_mode_notes_heartbeat_received', 5, 2 );
 ...
 * @return array
 */
function desktop_mode_notes_compute_heartbeat_delta( $known_ids, $since_ms, $cap ) {
	$cap       = max( 1, (int) $cap );
	$query_ids = desktop_mode_notes_query_changed_ids( $since_ms, $cap + 1 );
	...
	$alive_ids = desktop_mode_notes_alive_known_ids( $known_ids );
 ...
 */
function desktop_mode_notes_query_visible_ids( $extra ) {
 ...
 * @return int[]
 */
function desktop_mode_notes_query_changed_ids( $since_ms, $limit ) {
	$extra = array(
	...
	return desktop_mode_notes_query_visible_ids( $extra );
}
 ...
 * @return int[]
 */
function desktop_mode_notes_alive_known_ids( $known_ids ) {
	$known_ids = array_values( array_unique( array_filter( array_map( 'absint', (array) $known_ids ) ) ) );
	if ( empty( $known_ids ) ) {
		return array();
	}
	return desktop_mode_notes_query_visible_ids(
		array(
			'post__in'       => $known_ids,
