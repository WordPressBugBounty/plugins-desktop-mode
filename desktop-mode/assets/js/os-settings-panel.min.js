(function(){"use strict";const Ir="desktop-mode";function ri(){return window.wp?.i18n}function d(s,e=Ir){return ri()?.__(s,e)??s}function j(s,...e){const t=ri()?.sprintf;if(t)return t(s,...e);let o=0;return s.replace(/%(?:(\d+)\$)?[sd]/g,(i,n)=>{const r=n?Number.parseInt(n,10)-1:o++;return String(e[r]??"")})}function c(s,...e){return{__wpdHtml:!0,strings:s,values:e}}function Mr(s){return!!s&&s.__wpdHtml===!0}const Dr="$$wpd$$",D=/\$\$wpd\$\$(\d+)\$\$/g;function Nr(s){let e=s[0];for(let t=1;t<s.length;t++)e+=`${Dr}${t-1}$$`+s[t];return e}const ai=new WeakMap;function li(s){const e=ai.get(s);if(e)return e;const t=document.createElement("template");t.innerHTML=Nr(s);const o=[],i=(a,l)=>{if(a.nodeType===Node.ELEMENT_NODE){const p=a;for(const m of Array.from(p.attributes)){const _=m.name,b=m.value,w=_[0];if(D.test(b))if(D.lastIndex=0,w==="@"){const v=D.exec(b);D.lastIndex=0,o.push({path:l,kind:"event",name:_.slice(1),valueIndex:v?Number(v[1]):0}),p.removeAttribute(_)}else if(w==="."){const v=D.exec(b);D.lastIndex=0,o.push({path:l,kind:"prop",name:_.slice(1),valueIndex:v?Number(v[1]):0}),p.removeAttribute(_)}else if(w==="?"){const v=D.exec(b);D.lastIndex=0,o.push({path:l,kind:"bool",name:_.slice(1),valueIndex:v?Number(v[1]):0}),p.removeAttribute(_)}else{const v=[],g=[];let f=0,y;for(D.lastIndex=0;(y=D.exec(b))!==null;)v.push(b.slice(f,y.index)),g.push(Number(y[1])),f=y.index+y[0].length;v.push(b.slice(f)),o.push({path:l,kind:"attr",name:_,template:v,valueIndices:g}),p.setAttribute(_,"")}}}const u=Array.from(a.childNodes);let h=0;for(let p=0;p<u.length;p++){const m=u[p],_=p+h;if(m.nodeType===Node.TEXT_NODE){const b=m.textContent||"";if(!D.test(b)){D.lastIndex=0;continue}D.lastIndex=0;const w=m.parentNode;let v=0,g;const f=[],y=[];for(D.lastIndex=0;(g=D.exec(b))!==null;){g.index>v&&f.push(document.createTextNode(b.slice(v,g.index)));const A=document.createTextNode("");f.push(A),y.push({path:[...l,_+f.length-1],kind:"node",valueIndex:Number(g[1])}),v=g.index+g[0].length}v<b.length&&f.push(document.createTextNode(b.slice(v)));for(const A of f)w.insertBefore(A,m);w.removeChild(m),h+=f.length-1,o.push(...y)}else i(m,[...l,_])}};i(t.content,[]);const r={template:t,buildParts:a=>{const l=[];for(const u of o){let h=a;for(const p of u.path)h=h.childNodes[p];u.kind==="node"?l.push({kind:"node",valueIndex:u.valueIndex,child:{anchor:h,state:null}}):u.kind==="attr"?l.push({kind:"attr",element:h,name:u.name,template:u.template,valueIndices:u.valueIndices}):u.kind==="event"?l.push({kind:"event",valueIndex:u.valueIndex,element:h,name:u.name}):u.kind==="prop"?l.push({kind:"prop",valueIndex:u.valueIndex,element:h,name:u.name}):u.kind==="bool"&&l.push({kind:"bool",valueIndex:u.valueIndex,element:h,name:u.name})}return l}};return ai.set(s,r),r}const ci=new WeakMap;function Hr(s,e){for(const t of s.nodes)if(t.parentNode!==e)return!1;return!0}function L(s,e){const t=ci.get(e);if(t&&t.strings===s.strings&&Hr(t,e)){Nt(t.parts,s.values);return}const o=li(s.strings),i=o.template.content.cloneNode(!0),n=o.buildParts(i),r=Array.from(i.childNodes);for(;e.firstChild;)e.removeChild(e.firstChild);e.appendChild(i),Nt(n,s.values),ci.set(e,{strings:s.strings,parts:n,nodes:r})}function Nt(s,e){for(const t of s)if(t.kind==="node")ls(t.child,e[t.valueIndex]);else if(t.kind==="attr"){let o=t.template[0];for(let i=0;i<t.valueIndices.length;i++)o+=di(e[t.valueIndices[i]]),o+=t.template[i+1];o!==t.last&&(t.last=o,o===""?t.element.removeAttribute(t.name):t.element.setAttribute(t.name,o))}else if(t.kind==="event"){const o=e[t.valueIndex];o!==t.current&&(t.current&&t.element.removeEventListener(t.name,t.current),o&&t.element.addEventListener(t.name,o),t.current=o)}else if(t.kind==="prop"){const o=e[t.valueIndex];o!==t.last&&(t.last=o,t.element[t.name]=o)}else if(t.kind==="bool"){const o=!!e[t.valueIndex];o!==t.last&&(t.last=o,o?t.element.setAttribute(t.name,""):t.element.removeAttribute(t.name))}}function ls(s,e){if(e==null||e===!1){s.state&&(ne(s.state),s.state=null);return}if(Array.isArray(e)){Wr(s,e);return}if(Mr(e)){Fr(s,e);return}if(e instanceof Node){qr(s,e);return}Ur(s,di(e))}function qr(s,e){const t=s.state;t?.shape==="node"&&t.node===e||(t&&ne(t),Ht(s,[e]),s.state={shape:"node",node:e})}function Ur(s,e){const t=s.state;if(t?.shape==="text"){t.text!==e&&(t.node.textContent=e,t.text=e);return}t&&ne(t);const o=document.createTextNode(e);Ht(s,[o]),s.state={shape:"text",node:o,text:e}}function Fr(s,e){const t=s.state;if(t?.shape==="template"&&t.strings===e.strings){Nt(t.parts,e.values);return}t&&ne(t);const o=li(e.strings),i=o.template.content.cloneNode(!0),n=o.buildParts(i),r=Array.from(i.childNodes);Ht(s,[i]),Nt(n,e.values),s.state={shape:"template",strings:e.strings,parts:n,nodes:r}}function Wr(s,e){const t=s.state;if(t?.shape==="array"&&t.entries.length===e.length){for(let i=0;i<e.length;i++)ls(t.entries[i],e[i]);return}t&&ne(t);const o=[];for(const i of e){const n=document.createTextNode("");Ht(s,[n]);const r={anchor:n,state:null};ls(r,i),o.push(r)}s.state={shape:"array",entries:o}}function Ht(s,e){const t=s.anchor.parentNode;if(t)for(const o of e)t.insertBefore(o,s.anchor)}function ne(s){if(s.shape==="text"){s.node.remove();return}if(s.shape==="template"){for(const e of s.parts)e.kind==="node"&&e.child.state&&(ne(e.child.state),e.child.state=null);for(const e of s.nodes)e.parentNode&&e.parentNode.removeChild(e);return}if(s.shape==="node"){s.node.parentNode&&s.node.parentNode.removeChild(s.node);return}for(const e of s.entries)e.state&&ne(e.state),e.anchor.remove()}function di(s){return s==null||s===!1?"":String(s)}const se=class se extends HTMLElement{constructor(){super(),this._renderScheduled=!1,this._propValues={},this.constructor.shadow?(this.attachShadow({mode:"open"}),this._renderRoot=this.shadowRoot):this._renderRoot=this,this._installPropAccessors()}static get observedAttributes(){return this.props.map(ui)}connectedCallback(){this._adoptStyles(),this.requestUpdate()}attributeChangedCallback(e,t,o){if(t===o)return;const i=jr(e);this._propValues[i]=o,this.requestUpdate()}get classNames(){return Array.from(this.classList)}set classNames(e){if(e==null){this.removeAttribute("class");return}const o=(Array.isArray(e)?e:String(e).split(/\s+/)).map(i=>String(i).trim()).filter(i=>i!=="");this.className=o.join(" ")}requestUpdate(){this._scheduleRender()}emit(e,t){return this.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0}))}_installPropAccessors(){const e=this.constructor;for(const t of e.props){if(Object.getOwnPropertyDescriptor(this,t))continue;const o=ui(t);Object.defineProperty(this,t,{get:()=>t in this._propValues?this._propValues[t]:this.getAttribute(o),set:i=>{let n;i==null||i===!1?n=null:i===!0?n="":n=String(i),this._propValues[t]=n,n===null?this.removeAttribute(o):this.setAttribute(o,n),this.requestUpdate()},enumerable:!0,configurable:!0})}}_scheduleRender(){this._renderScheduled||!this.isConnected||(this._renderScheduled=!0,queueMicrotask(()=>{this._renderScheduled=!1,this.isConnected&&L(this.render(),this._renderRoot)}))}_adoptStyles(){const e=this.constructor;if(e.styles.length!==0)if(e.shadow&&this.shadowRoot){const t=e.styles.map(o=>o.sheet).filter(o=>o!==null);if(this.shadowRoot.adoptedStyleSheets=t,t.length!==e.styles.length){for(const o of e.styles)if(!o.sheet){const i=document.createElement("style");i.textContent=o.cssText,this.shadowRoot.appendChild(i)}}}else this._adoptLightStyles(e)}_adoptLightStyles(e){if(!se._lightStylesAdopted.has(e)){se._lightStylesAdopted.add(e);for(const t of e.styles){const o=document.createElement("style");o.dataset.osUi=this.tagName.toLowerCase(),o.textContent=t.cssText,document.head.appendChild(o)}}}};se.props=[],se.styles=[],se.shadow=!0,se._lightStylesAdopted=new WeakSet;let k=se;function $(s,e){customElements.get(s)||customElements.define(s,e)}function ui(s){return s.replace(/[A-Z]/g,e=>"-"+e.toLowerCase())}function jr(s){return s.replace(/-([a-z])/g,(e,t)=>t.toUpperCase())}const Vr=(()=>{try{return typeof new CSSStyleSheet().replaceSync=="function"}catch{return!1}})();function x(s,...e){let t=s[0];for(let o=1;o<s.length;o++){const i=e[o-1];if(typeof i=="string"||typeof i=="number")t+=String(i);else if(i&&i.__wpdCss)t+=i.cssText;else throw new TypeError("[os-ui] css`` interpolations must be strings, numbers, or other css`` results. Got: "+typeof i);t+=s[o]}if(Vr){const o=new CSSStyleSheet;return o.replaceSync(t),{__wpdCss:!0,sheet:o,cssText:t}}return{__wpdCss:!0,sheet:null,cssText:t}}function Or(s){const e=[],t=[];let o=null,i=s.parentElement;for(;i&&!(i===document.body||i===document.documentElement);){const r=i.id||"";if(r.startsWith("wp-window-")){o=r.slice(10);break}if(i.tagName.toLowerCase()==="os-tabpanel"){const a=i.getAttribute("for");a&&t.unshift(a)}i=i.parentElement}o&&e.push(cs(o));for(const r of t)e.push("tab-"+cs(r));const n=s.getAttribute("label");return n&&e.push(cs(n)),e.length===0?"os-unnamed":"os-"+e.filter(r=>r!=="").join("-")}function cs(s){return s.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function ge(s){if(s.id)return s.id;const e=Or(s);return s.id=e,e}const hi={close:{a:'fill="currentColor"',b:'<path d="m13.06 12 6.47-6.47-1.06-1.06L12 10.94 5.53 4.47 4.47 5.53 10.94 12l-6.47 6.47 1.06 1.06L12 13.06l6.47 6.47 1.06-1.06L13.06 12Z" />'},check:{a:'fill="currentColor"',b:'<path d="M16.5 7.5 10 13.9l-2.5-2.4-1 1 3.5 3.6 7.5-7.6z" />'},"chevron-right":{a:'fill="currentColor"',b:'<path d="M10.6 6L9.4 7l4.6 5-4.6 5 1.2 1 5.4-6z" />'},"arrow-up-right":{a:'fill="currentColor"',b:'<path d="M10 6H18V14H16.5V8.5L7 18L6 17L15.5 7.5H10V6Z" />'},plus:{a:'fill="currentColor"',b:'<path d="M11 12.5V17.5H12.5V12.5H17.5V11H12.5V6H11V11H6V12.5H11Z" />'},search:{a:'fill="currentColor"',b:'<path d="M13 5c-3.3 0-6 2.7-6 6 0 1.4.5 2.7 1.3 3.7l-3.8 3.8 1.1 1.1 3.8-3.8c1 .8 2.3 1.3 3.7 1.3 3.3 0 6-2.7 6-6S16.3 5 13 5zm0 10.5c-2.5 0-4.5-2-4.5-4.5s2-4.5 4.5-4.5 4.5 2 4.5 4.5-2 4.5-4.5 4.5z" />'},pin:{a:'fill="currentColor"',b:'<path d="m21.5 9.1-6.6-6.6-4.2 5.6c-1.2-.1-2.4.1-3.6.7-.1 0-.1.1-.2.1-.5.3-.9.6-1.2.9l3.7 3.7-5.7 5.7v1.1h1.1l5.7-5.7 3.7 3.7c.4-.4.7-.8.9-1.2.1-.1.1-.2.2-.3.6-1.1.8-2.4.6-3.6l5.6-4.1zm-7.3 3.5.1.9c.1.9 0 1.8-.4 2.6l-6-6c.8-.4 1.7-.5 2.6-.4l.9.1L15 4.9 19.1 9l-4.9 3.6z" />'},trash:{a:'fill="currentColor"',b:'<path fill-rule="evenodd" clip-rule="evenodd" d="M12 5.5A2.25 2.25 0 0 0 9.878 7h4.244A2.251 2.251 0 0 0 12 5.5ZM12 4a3.751 3.751 0 0 0-3.675 3H5v1.5h1.27l.818 8.997a2.75 2.75 0 0 0 2.739 2.501h4.347a2.75 2.75 0 0 0 2.738-2.5L17.73 8.5H19V7h-3.325A3.751 3.751 0 0 0 12 4Zm4.224 4.5H7.776l.806 8.861a1.25 1.25 0 0 0 1.245 1.137h4.347a1.25 1.25 0 0 0 1.245-1.137l.805-8.861Z" />'},download:{a:'fill="currentColor"',b:'<path d="M18 11.3l-1-1.1-4 4V3h-1.5v11.3L7 10.2l-1 1.1 6.2 5.8 5.8-5.8zm.5 3.7v3.5h-13V15H4v5h16v-5h-1.5z" />'},settings:{a:'fill="currentColor"',b:'<path d="m19 7.5h-7.628c-.3089-.87389-1.1423-1.5-2.122-1.5-.97966 0-1.81309.62611-2.12197 1.5h-2.12803v1.5h2.12803c.30888.87389 1.14231 1.5 2.12197 1.5.9797 0 1.8131-.62611 2.122-1.5h7.628z" /><path d="m19 15h-2.128c-.3089-.8739-1.1423-1.5-2.122-1.5s-1.8131.6261-2.122 1.5h-7.628v1.5h7.628c.3089.8739 1.1423 1.5 2.122 1.5s1.8131-.6261 2.122-1.5h2.128z" />'},info:{a:'fill="currentColor"',b:'<path fill-rule="evenodd" clip-rule="evenodd" d="M5.5 12a6.5 6.5 0 1 0 13 0 6.5 6.5 0 0 0-13 0ZM12 4a8 8 0 1 0 0 16 8 8 0 0 0 0-16Zm.75 4v1.5h-1.5V8h1.5Zm0 8v-5h-1.5v5h1.5Z" />'},bell:{a:'fill="currentColor"',b:'<path fill-rule="evenodd" clip-rule="evenodd" d="M17 11.5c0 1.353.17 2.368.976 3 .266.209.602.376 1.024.5v1H5v-1c.422-.124.757-.291 1.024-.5.806-.632.976-1.647.976-3V9c0-2.8 2.2-5 5-5s5 2.2 5 5v2.5ZM15.5 9v2.5c0 .93.066 1.98.515 2.897l.053.103H7.932a4.018 4.018 0 0 0 .053-.103c.449-.917.515-1.967.515-2.897V9c0-1.972 1.528-3.5 3.5-3.5s3.5 1.528 3.5 3.5Zm-5.492 9.008c0-.176.023-.346.065-.508h3.854A1.996 1.996 0 0 1 12 20c-1.1 0-1.992-.892-1.992-1.992Z" />'},more:{a:'fill="currentColor"',b:'<path d="M11 13h2v-2h-2v2zm-6 0h2v-2H5v2zm12-2v2h2v-2h-2z" />'},maximize:{a:'fill="currentColor"',b:'<path d="M6 4a2 2 0 0 0-2 2v3h1.5V6a.5.5 0 0 1 .5-.5h3V4H6Zm3 14.5H6a.5.5 0 0 1-.5-.5v-3H4v3a2 2 0 0 0 2 2h3v-1.5Zm6 1.5v-1.5h3a.5.5 0 0 0 .5-.5v-3H20v3a2 2 0 0 1-2 2h-3Zm3-16a2 2 0 0 1 2 2v3h-1.5V6a.5.5 0 0 0-.5-.5h-3V4h3Z" />'},edit:{a:'fill="currentColor"',b:'<path d="m19 7-3-3-8.5 8.5-1 4 4-1L19 7Zm-7 11.5H5V20h7v-1.5Z" />'},color:{a:'fill="currentColor"',b:'<path d="M17.2 10.9c-.5-1-1.2-2.1-2.1-3.2-.6-.9-1.3-1.7-2.1-2.6L12 4l-1 1.1c-.6.9-1.3 1.7-2 2.6-.8 1.2-1.5 2.3-2 3.2-.6 1.2-1 2.2-1 3 0 3.4 2.7 6.1 6.1 6.1s6.1-2.7 6.1-6.1c0-.8-.3-1.8-1-3zm-5.1 7.6c-2.5 0-4.6-2.1-4.6-4.6 0-.3.1-1 .8-2.3.5-.9 1.1-1.9 2-3.1.7-.9 1.3-1.7 1.8-2.3.7.8 1.3 1.6 1.8 2.3.8 1.1 1.5 2.2 2 3.1.7 1.3.8 2 .8 2.3 0 2.5-2.1 4.6-4.6 4.6z" />'},wallpaper:{a:'fill="currentColor"',b:'<path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM5 4.5h14c.3 0 .5.2.5.5v8.4l-3-2.9c-.3-.3-.8-.3-1 0L11.9 14 9 12c-.3-.2-.6-.2-.8 0l-3.6 2.6V5c-.1-.3.1-.5.4-.5zm14 15H5c-.3 0-.5-.2-.5-.5v-2.4l4.1-3 3 1.9c.3.2.7.2.9-.1L16 12l3.5 3.4V19c0 .3-.2.5-.5.5z" />'},warning:{a:'fill="currentColor"',b:'<path fill-rule="evenodd" clip-rule="evenodd" d="M5.5 12a6.5 6.5 0 1 0 13 0 6.5 6.5 0 0 0-13 0ZM12 4a8 8 0 1 0 0 16 8 8 0 0 0 0-16Zm-.75 12v-1.5h1.5V16h-1.5Zm0-8v5h1.5V8h-1.5Z" />'},minimize:{a:'fill="currentColor"',b:'<path d="M5 11.25h14v1.5H5z" />'},window:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="4.125" y="5.4375" width="15.75" height="13.125" rx="1.75"/><path d="M4.125 9.8125h15.75"/><circle cx="7.275" cy="7.625" r="0.7438" fill="currentColor" stroke="none"/>'},windows:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="3.6875" y="8.5" width="11.375" height="10.0625" rx="1.75"/><path d="M3.6875 12h11.375"/><path d="M8.5 8.5V7.1875a1.75 1.75 0 0 1 1.75-1.75h7.875A1.75 1.75 0 0 1 19.875 7.1875v7a1.75 1.75 0 0 1-1.75 1.75h-3.0625"/>'},dock:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="3.25" y="8.5" width="17.5" height="7" rx="3.5"/><circle cx="7.625" cy="12" r="1.0062" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.0062" fill="currentColor" stroke="none"/><circle cx="16.375" cy="12" r="1.0062" fill="currentColor" stroke="none"/>'},spaces:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="3.6875" y="5.4375" width="7.4375" height="7.875" rx="1.75"/><rect x="12.875" y="5.4375" width="7.4375" height="7.875" rx="1.75"/><circle cx="9.375" cy="17.6875" r="0.7875" fill="currentColor" stroke="none"/><circle cx="12" cy="17.6875" r="0.7875" fill="currentColor" stroke="none"/><circle cx="14.625" cy="17.6875" r="0.7875" fill="currentColor" stroke="none"/>'},copilot:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<path d="M10 2.5Q10.9 8.5 17.5 10Q10.9 11.5 10 17.5Q9.1 11.5 2.5 10Q9.1 8.5 10 2.5Z" fill="currentColor" stroke="none"/><path d="M17.9 14Q18.35 17.1 21.5 17.6Q18.35 18.1 17.9 21.2Q17.45 18.1 14.3 17.6Q17.45 17.1 17.9 14Z" fill="currentColor" stroke="none"/>'},snap:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="3.6875" y="5.4375" width="16.625" height="13.125" rx="1.75"/><path d="M12 5.4375v13.125"/><path d="M12 12h8.3125"/>'},command:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<path d="M14.625 6.75v10.5a2.625 2.625 0 1 0 2.625-2.625H6.75a2.625 2.625 0 1 0 2.625 2.625V6.75a2.625 2.625 0 1 0-2.625 2.625h10.5a2.625 2.625 0 1 0-2.625-2.625"/>'},apps:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="4.125" y="4.125" width="6.5625" height="6.5625" rx="1.75"/><rect x="13.3125" y="4.125" width="6.5625" height="6.5625" rx="1.75"/><rect x="4.125" y="13.3125" width="6.5625" height="6.5625" rx="1.75"/><rect x="13.3125" y="13.3125" width="6.5625" height="6.5625" rx="1.75"/>'},widgets:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="3.6875" y="3.6875" width="9.625" height="9.625" rx="1.75"/><rect x="15.0625" y="3.6875" width="5.25" height="5.25" rx="1.75"/><rect x="15.0625" y="10.6875" width="5.25" height="9.625" rx="1.75"/><rect x="3.6875" y="15.0625" width="9.625" height="5.25" rx="1.75"/>'},user:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<circle cx="12" cy="8.9375" r="3.5"/><path d="M5.4375 19.4375a6.5625 6.5625 0 0 1 13.125 0"/>'},lock:{a:'fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"',b:'<rect x="5.4375" y="10.6875" width="13.125" height="8.75" rx="1.75"/><path d="M8.5 10.6875V8.0625a3.5 3.5 0 0 1 7 0v2.625"/>'}};function pi(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function Kr(s){return Object.prototype.hasOwnProperty.call(hi,s)?hi[s]:null}function Br(s,e={}){const t=Kr(s);if(!t)return"";const{size:o=24,className:i,title:n,rotate:r}=e,a=['xmlns="http://www.w3.org/2000/svg"','viewBox="0 0 24 24"'];o!=null&&a.push(`width="${o}"`,`height="${o}"`),a.push(t.a),i&&a.push(`class="${pi(i)}"`),n?a.push('role="img"',`aria-label="${pi(n)}"`):a.push('aria-hidden="true"','focusable="false"');const l=r?`<g transform="rotate(${r} 12 12)">${t.b}</g>`:t.b;return`<svg ${a.join(" ")}>${l}</svg>`}function z(s,e={}){const t=Br(s,e),o=document.createElement("div");o.innerHTML=t;const i=o.firstElementChild;return i instanceof SVGSVGElement?i:document.createElementNS("http://www.w3.org/2000/svg","svg")}const R=x`:host{--_holo-fill:var( --os-ui-holo-fill,${"linear-gradient( 124deg, #afa2e8 0%, #f5a8ea 46%, #8ee9f7 100% )"} );--_holo-ink:var( --os-ui-holo-ink,#0c0b0f );--_holo-sheen:var( --os-ui-holo-sheen,linear-gradient( 124deg,rgba( 159,214,255,0.1 ) 0%,rgba( 236,155,255,0.12 ) 34%,rgba( 242,82,252,0.1 ) 58%,rgba( 147,240,198,0.09 ) 100% ) );--_holo-edge:var( --os-ui-holo-edge,linear-gradient( 124deg,rgba( 154,242,255,0.7 ) 0%,rgba( 236,155,255,0.85 ) 38%,rgba( 242,82,252,0.7 ) 62%,rgba( 159,152,255,0.6 ) 100% ) );--_holo-edge-quiet:var( --os-ui-holo-edge-quiet,linear-gradient( 124deg,rgba( 154,242,255,0.22 ) 0%,rgba( 236,155,255,0.28 ) 38%,rgba( 242,82,252,0.22 ) 62%,rgba( 159,152,255,0.2 ) 100% ) );--_holo-glow:var( --os-ui-holo-glow,0 0 0 1px rgba( 242,82,252,0.28 ),0 2px 10px rgba( 242,82,252,0.22 ) );--_holo-glow-strong:var( --os-ui-holo-glow-strong,0 0 0 1px rgba( 242,82,252,0.42 ),0 4px 18px rgba( 242,82,252,0.38 ),0 1px 3px rgba( 12,11,15,0.6 ) );--_holo-track:var( --os-ui-holo-track,rgba( 255,251,255,0.16 ) );--_holo-track-edge:var( --os-ui-holo-track-edge,#8c8f94 );--_holo-ring-color:var( --os-ui-accent-dim,#2271b1 );--_holo-focus:var( --os-ui-focus-ring,0 0 0 2px rgba( 12,11,15,0.9 ),0 0 0 4px #f252fc,0 0 12px 2px rgba( 242,82,252,0.45 ) );--_holo-focus-field:var( --os-ui-focus-ring-field,0 0 0 1px #f252fc,0 0 0 4px rgba( 242,82,252,0.18 ) );--_holo-t:var( --os-ui-holo-transition,220ms );--_holo-t-fast:var( --os-ui-motion-fast,140ms );--_holo-t-slow:var( --os-ui-motion-slow,340ms );--_holo-t-ambient:var( --os-ui-motion-ambient,12s );--_holo-spring:var( --os-ui-ease-spring,cubic-bezier( 0.32,1.5,0.55,1 ) );--_holo-ease:var( --os-ui-ease-out,cubic-bezier( 0.22,0.9,0.28,1 ) );--_holo-loop:var( --os-ui-ease-loop,cubic-bezier( 0.45,0,0.55,1 ) )}`,Gr=x` .os-holo-fill{background-color:transparent;background-image:var( --_holo-fill );background-size:220% 220%;background-position:22% 28%;background-repeat:no-repeat;color:var( --_holo-ink );transition:background-position var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease,filter var( --_holo-t ) ease}.os-holo-fill:hover{background-position:74% 66%}.os-holo-fill:active{background-position:88% 82%;filter:brightness( 0.94 )}@media ( prefers-reduced-motion:reduce ){.os-holo-fill{transition-duration:1ms}.os-holo-fill:hover,.os-holo-fill:active{background-position:22% 28%}}`,qt=x` .os-holo-sheen{position:relative;isolation:isolate}.os-holo-sheen::before{content:'';position:absolute;inset:0;z-index:-1;border-radius:inherit;background-image:var( --_holo-sheen );background-size:200% 200%;background-position:20% 30%;opacity:0;pointer-events:none;transition:opacity var( --_holo-t ) ease,background-position var( --_holo-t ) ease}.os-holo-sheen:hover::before,.os-holo-sheen:focus-visible::before{opacity:1;background-position:76% 68%}@media ( prefers-reduced-motion:reduce ){.os-holo-sheen::before{transition-duration:1ms}.os-holo-sheen:hover::before,.os-holo-sheen:focus-visible::before{background-position:20% 30%}}`,fi=x` .os-holo-edge{position:relative}.os-holo-edge::after{content:'';position:absolute;inset:0;border-radius:inherit;padding:1px;background-image:var( --_holo-edge-quiet );opacity:0;pointer-events:none;-webkit-mask:linear-gradient( #000 0 0 ) content-box,linear-gradient( #000 0 0 );-webkit-mask-composite:xor;mask:linear-gradient( #000 0 0 ) content-box,linear-gradient( #000 0 0 );mask-composite:exclude;transition:opacity var( --_holo-t ) ease}.os-holo-edge:hover::after,.os-holo-edge:focus-visible::after{background-image:var( --_holo-edge );opacity:1}.os-holo-edge.is-lit::after{background-image:var( --_holo-edge );opacity:1}@media ( prefers-reduced-motion:reduce ){.os-holo-edge::after{transition-duration:1ms}}`,Ut=x` .os-holo-glint{position:absolute;inset:0;border-radius:inherit;overflow:hidden;pointer-events:none}.os-holo-glint::before{content:'';position:absolute;top:-60%;bottom:-60%;width:45%;inset-inline-start:-60%;background:linear-gradient( 90deg,transparent 0%,rgba( 255,251,255,0.14 ) 45%,rgba( 255,251,255,0.22 ) 50%,rgba( 255,251,255,0.14 ) 55%,transparent 100% );transform:rotate( 18deg ) translateX( 0 );opacity:0}:hover>.os-holo-glint::before,:focus-visible>.os-holo-glint::before,:host(:hover )>.os-holo-glint::before,:host(:focus-visible )>.os-holo-glint::before{animation:os-holo-glint var( --_holo-t-slow ) var( --_holo-ease )}@keyframes os-holo-glint{0%{opacity:0;transform:rotate( 18deg ) translateX( 0 )}15%{opacity:1}85%{opacity:1}100%{opacity:0;transform:rotate( 18deg ) translateX( 240vw )}}@media ( prefers-reduced-motion:reduce ){:hover>.os-holo-glint::before,:focus-visible>.os-holo-glint::before,:host(:hover )>.os-holo-glint::before,:host(:focus-visible )>.os-holo-glint::before{animation:none}}`,ds=x` .os-holo-ring{position:absolute;inset:0;border-radius:inherit;box-shadow:0 0 0 0 var( --_holo-ring-color );opacity:0;pointer-events:none}:active:not(:disabled )>.os-holo-ring,:host(:active:not( [ disabled ] ) )>.os-holo-ring{animation:os-holo-ring var( --_holo-t-slow ) var( --_holo-ease )}@keyframes os-holo-ring{0%{opacity:0.45;box-shadow:0 0 0 0 var( --_holo-ring-color )}100%{opacity:0;box-shadow:0 0 0 10px transparent}}@media ( prefers-reduced-motion:reduce ){:active:not(:disabled )>.os-holo-ring,:host(:active:not( [ disabled ] ) )>.os-holo-ring{animation:none}}`,gi=x` .os-holo-shimmer{background-image:var( --_holo-fill );background-size:300% 300%;background-repeat:no-repeat;animation:os-holo-shimmer 2.4s var( --_holo-loop ) infinite}@keyframes os-holo-shimmer{0%{background-position:0% 50%}100%{background-position:100% 50%}}@media ( prefers-reduced-motion:reduce ){.os-holo-shimmer{animation:none;background-position:30% 50%}}`,us=x` .os-holo-enter{animation:os-holo-enter var( --_holo-t ) var( --_holo-spring )}@keyframes os-holo-enter{from{opacity:0;transform:scale( 0.96 )}}@media ( prefers-reduced-motion:reduce ){.os-holo-enter{animation:none}}`,hs=x` input:where(:not( [ type='checkbox' ] ):not( [ type='radio' ] ) ),select,textarea{transition:background-color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}input:where(:not( [ type='checkbox' ] ):not( [ type='radio' ] ) ):hover:not(:disabled ),select:hover:not(:disabled ),textarea:hover:not(:disabled ){border-color:var( --os-ui-border-strong,#8c8f94 )}input:where(:not( [ type='checkbox' ] ):not( [ type='radio' ] ) ):focus,input:where(:not( [ type='checkbox' ] ):not( [ type='radio' ] ) ):focus-visible,select:focus,select:focus-visible,textarea:focus,textarea:focus-visible{outline:none;border-color:var( --os-ui-accent,#2271b1 );box-shadow:var( --_holo-focus-field )}input::placeholder,textarea::placeholder{color:var( --os-ui-fg-faint,#8c8f94 );opacity:1}input::selection,textarea::selection{background:var( --os-ui-selection-bg,rgba( 159,152,255,0.6 ) );color:var( --os-ui-selection-fg,#fffbff )}@media ( prefers-reduced-motion:reduce ){input:where(:not( [ type='checkbox' ] ):not( [ type='radio' ] ) ),select,textarea{transition-duration:1ms}}`,Ft=x` input[ type='checkbox' ],input[ type='radio' ]{appearance:none;-webkit-appearance:none;position:relative;flex:0 0 auto;box-sizing:border-box;width:16px;height:16px;margin:0;padding:0;border:1px solid var( --_holo-track-edge );border-radius:4px;background-color:var( --_holo-track );background-image:none;background-repeat:no-repeat;cursor:inherit;transition:background-color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}input[ type='radio' ]{border-radius:50%}input[ type='checkbox' ]:hover:not(:disabled ),input[ type='radio' ]:hover:not(:disabled ){border-color:var( --os-ui-accent,#2271b1 )}input[ type='checkbox' ]:focus-visible,input[ type='radio' ]:focus-visible{outline:none;box-shadow:var( --_holo-focus )}input[ type='checkbox' ]:checked,input[ type='checkbox' ]:indeterminate,input[ type='radio' ]:checked{border-color:transparent;background-color:var( --os-ui-accent,#2271b1 )}input[ type='checkbox' ]:checked:focus-visible,input[ type='radio' ]:checked:focus-visible{box-shadow:var( --_holo-focus )}input[ type='checkbox' ]:checked::after{content:'';position:absolute;inset-inline-start:50%;top:46%;width:3.5px;height:7.5px;border:solid var( --os-ui-fg-on-accent,#fff );border-width:0 2px 2px 0;transform:translate( -50%,-50% ) rotate( 45deg );animation:os-holo-tick 180ms cubic-bezier( 0.3,1.4,0.6,1 )}input[ type='checkbox' ]:indeterminate::after{content:'';position:absolute;inset-inline-start:50%;top:50%;width:8px;height:2px;border:0;border-radius:1px;background:var( --os-ui-fg-on-accent,#fff );transform:translate( -50%,-50% )}input[ type='radio' ]:checked::after{content:'';position:absolute;inset-inline-start:50%;top:50%;width:6px;height:6px;border-radius:50%;background:var( --os-ui-fg-on-accent,#fff );transform:translate( -50%,-50% );animation:os-holo-tick 180ms cubic-bezier( 0.3,1.4,0.6,1 )}input[ type='checkbox' ]:disabled,input[ type='radio' ]:disabled{cursor:not-allowed}@keyframes os-holo-tick{from{opacity:0;transform:translate( -50%,-50% ) rotate( 45deg ) scale( 0.4 )}}@media ( prefers-reduced-motion:reduce ){input[ type='checkbox' ]:checked::after,input[ type='radio' ]:checked::after{animation:none}}`,bi=x` @keyframes os-holo-drift{0%{background-position:16% 26%}50%{background-position:84% 74%}100%{background-position:16% 26%}}.os-holo-alive{animation:os-holo-drift var( --_holo-t-ambient ) var( --_holo-loop ) infinite}@media ( prefers-reduced-motion:reduce ){.os-holo-alive{animation:none}}`;x` ${R} ${Gr} ${qt} ${fi} ${Ut} ${ds} ${gi} ${us} ${hs} ${Ft} ${bi} `;const Xr=x` ${R} ${qt} ${fi} ${Ut} ${ds} ${bi}:host{display:inline-flex}:host( [ fill-cell ] ){display:flex;width:100%}button{appearance:none;display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:var( --os-ui-button-padding,6px 12px );border-radius:var( --os-ui-button-border-radius,6px );font:inherit;font-weight:500;cursor:pointer;transition:background-color var( --_holo-t ) ease,color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease,transform 80ms ease;background:var( --os-ui-button-bg,transparent );background-image:var( --os-ui-button-bg-image,none );background-repeat:var( --os-ui-button-bg-image-repeat,repeat );background-size:var( --os-ui-button-bg-image-size,auto );background-position:var( --os-ui-button-bg-image-position,center );color:var( --os-ui-button-fg,var( --os-ui-fg,#1d2327 ) );border:var( --os-ui-button-border,1px solid var( --os-ui-border,#c3c4c7 ) )}:host( [ fill-cell ] ) button{width:100%;min-height:var( --os-ui-button-min-height,44px )}button:disabled{opacity:0.5;cursor:not-allowed}button:active:not(:disabled ){transform:translateY( 1px )}button:hover:not(:disabled ){background-color:var( --os-ui-button-bg-hover,var( --os-ui-hover,rgba( 0,0,0,0.04 ) ) )}button:focus-visible{outline:none;box-shadow:var( --_holo-focus )}button:disabled::before,button:disabled::after,button:disabled .os-holo-glint{opacity:0 !important}:host( [ variant='primary' ] ) button{background-color:var( --os-ui-button-bg,var( --wp-admin-theme-color,#2271b1 ) );color:var( --os-ui-button-fg,var( --os-ui-fg-on-accent,#fff ) );border:var( --os-ui-button-border,1px solid transparent )}:host( [ variant='primary' ] ) button:hover:not(:disabled ){filter:brightness( 1.06 );background-color:var( --os-ui-button-bg,var( --wp-admin-theme-color,#2271b1 ) );box-shadow:var( --_holo-glow )}:host( [ variant='holo' ] ) button{background-color:transparent;background-image:var( --_holo-fill );background-size:220% 220%;background-position:22% 28%;background-repeat:no-repeat;color:var( --os-ui-button-fg,var( --_holo-ink ) );border:var( --os-ui-button-border,1px solid transparent );box-shadow:var( --_holo-glow );font-weight:600;transition:background-position var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease,filter var( --_holo-t ) ease,transform 80ms ease}:host( [ variant='holo' ] ) button:hover:not(:disabled ){background-color:transparent;background-position:74% 66%;box-shadow:var( --_holo-glow-strong )}:host( [ variant='holo' ] ) button:active:not(:disabled ){background-position:88% 82%;filter:brightness( 0.94 )}:host( [ variant='holo' ] ) button::before{display:none}@media ( prefers-reduced-motion:reduce ){:host( [ variant='holo' ] ) button:hover:not(:disabled ),:host( [ variant='holo' ] ) button:active:not(:disabled ){background-position:22% 28%}}:host( [ variant='secondary' ] ) button{background-color:var( --os-ui-button-bg,var( --os-ui-hover,rgba( 0,0,0,0.06 ) ) );color:var( --os-ui-button-fg,var( --os-ui-fg,#1d2327 ) );border:var( --os-ui-button-border,1px solid transparent )}:host( [ variant='secondary' ] ) button:hover:not(:disabled ){background-color:var( --os-ui-button-bg-hover,var( --os-ui-hover,rgba( 0,0,0,0.1 ) ) )}:host( [ variant='danger' ] ) button{background-color:var( --os-ui-button-bg,transparent );color:var( --os-ui-button-fg,var( --os-ui-danger,#d63638 ) );border:var( --os-ui-button-border,1px solid currentColor )}:host( [ variant='danger' ] ) button:hover:not(:disabled ){background-color:var( --os-ui-danger,#d63638 );color:var( --os-ui-fg-on-accent,#fff )}:host( [ variant='danger' ] ) button::after{display:none}:host( [ variant='link' ] ) button{background-color:transparent;background-image:none;color:var( --os-ui-button-fg,var( --wp-admin-theme-color,#2271b1 ) );border:0;padding:0;text-decoration:underline}:host( [ variant='link' ] ) button::before,:host( [ variant='link' ] ) button::after,:host( [ variant='link' ] ) .os-holo-glint,:host( [ variant='link' ] ) .os-holo-ring{display:none}:host( [ variant='link' ] ) button:active:not(:disabled ){transform:none}:host( [ busy ] ) button{pointer-events:none;opacity:0.75}:host( [ variant='holo' ][ busy ] ) button{animation:os-holo-drift 12s ease-in-out infinite}@media ( prefers-reduced-motion:reduce ){:host( [ variant='holo' ][ busy ] ) button{animation:none}}.os-button__spinner{box-sizing:border-box;display:inline-block;width:12px;height:12px;border:2px solid currentColor;border-right-color:transparent;border-radius:50%;animation:os-button-spin 0.6s linear infinite;flex-shrink:0}@keyframes os-button-spin{to{transform:rotate( 360deg )}}`,Ce=class Ce extends k{render(){const e=this.disabled!==null,t=this.busy!==null,o=this.type||"button";return c`
		...
		`}};Ce.props=["variant","disabled","type","busy","fill-cell"],Ce.styles=[Xr],Ce.help=void 0;let ps=Ce;$("os-button",ps);const Zr=x` ${R} ${Ft}:host{display:inline-flex;align-items:center;gap:8px;font-size:12px;color:var( --os-ui-fg,#1d2327 );cursor:pointer;-webkit-user-select:none;user-select:none}label{display:inline-flex;align-items:center;gap:8px;cursor:pointer}:host( [ disabled ] ){opacity:0.5;cursor:not-allowed}:host( [ disabled ] ) label,:host( [ disabled ] ) input[ type='checkbox' ]{cursor:not-allowed}`,Se=class Se extends k{render(){const e=this.label||"",t=this.checked!==null,o=this.disabled!==null;return c`
			<label>
				<input
					type="checkbox"
					?checked=${t}
		...
		`}_onInput(e){const t=e.target;this.value=t.value,this.emit("os-color-change",{value:t.value})}};Ee.props=["label","value","variant"],Ee.styles=[Yr],Ee.help=void 0;let gs=Ee;$("os-color-field",gs);const Jr=x`:host{display:inline-flex;align-items:center;justify-content:center;width:var( --os-ui-icon-size,16px );height:var( --os-ui-icon-size,16px );color:inherit;line-height:1}:host( [ hidden ] ){display:none}.os-icon__glyph{font-size:var( --os-ui-icon-size,16px );width:var( --os-ui-icon-size,16px );height:var( --os-ui-icon-size,16px );line-height:1;color:inherit;display:inline-flex;align-items:center;justify-content:center}.os-icon__glyph--char{font-family:dashicons;font-style:normal;font-weight:normal;font-variant:normal;text-transform:none;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;speak:none}.os-icon__glyph.dashicons{font-family:dashicons}`;let Wt=null;function Qr(s){let e=s.trim();if(e==="")return null;(e.startsWith('"')&&e.endsWith('"')||e.startsWith("'")&&e.endsWith("'"))&&(e=e.slice(1,-1));const t=e.match(/^\\([0-9a-f]{1,6})\s?$/i);return t?String.fromCodePoint(parseInt(t[1],16)):e||null}function mi(){const s=new Map;if(typeof document>"u")return s;const e=Array.from(document.styleSheets??[]);for(const t of e){let o=null;try{o=t.cssRules}catch{continue}if(o)for(const i of Array.from(o)){const n=i;if(!n||!n.selectorText)continue;const r=n.selectorText.match(/\.dashicons-([a-z0-9-]+)::?before/i);if(!r)continue;const a=n.style?.content;if(!a)continue;const l=Qr(a);l&&s.set(r[1],l)}}return s}function ea(s){Wt||(Wt=mi());const e=s.startsWith("dashicons-")?s.slice(10):s;return Wt.get(e)??null}function ta(){Wt=mi()}let vi=!1;function sa(){if(vi||typeof window>"u")return;vi=!0;const s=()=>{ta()};document.readyState==="loading"&&document.addEventListener("DOMContentLoaded",s,{once:!0}),window.addEventListener("load",s,{once:!0})}sa();const Le=class Le extends k{render(){const e=this.name||"",t=e.startsWith("dashicons-")?e.slice(10):e,o=this.size;o&&/^\d+$/.test(o)&&this.style.setProperty("--os-ui-icon-size",`${o}px`);const i=ea(t);return i?c`<span
				class="os-icon__glyph os-icon__glyph--char dashicons dashicons-${t}"
				aria-hidden="true"
			>${i}</span>`:c`<span
			class="os-icon__glyph dashicons dashicons-${t}"
			aria-hidden="true"
		></span>`}};Le.props=["name","size"],Le.styles=[Jr],Le.help=void 0;let bs=Le;$("os-icon",bs);const oa=x`:host{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;padding:32px 24px;text-align:center;color:var( --os-ui-empty-state-fg,var( --os-ui-fg-muted,#646970 ) )}:host( [ hidden ] ){display:none}.os-empty-state__icon{margin-bottom:4px;color:var( --os-ui-empty-state-icon-color,currentColor );opacity:0.75}.os-empty-state__heading{margin:0;font-size:14px;font-weight:600;color:var( --os-ui-fg,#1d2327 )}.os-empty-state__description{margin:0;font-size:12px;line-height:1.4;max-width:48ch}.os-empty-state__description:empty{display:none}.os-empty-state__cta{margin-top:8px}.os-empty-state__cta:empty{display:none}`,Re=class Re extends k{render(){const e=this.icon||"",t=this.heading||"",o=this.description||"";return c`
			${e?c`<os-icon
		...
		`}_onDismiss(e){e.preventDefault(),e.stopPropagation(),this.dismiss()}};Te.props=["tone","notDismissible","icon","noticeId"],Te.styles=[ia],Te.help=void 0;let _s=Te;$("os-notice",_s);const da=x`:host{display:flex;flex-direction:column;gap:var( --os-ui-panel-gap,12px );padding:var( --os-ui-panel-padding,16px );background-image:var( --os-ui-panel-bg-image,none );background-repeat:var( --os-ui-panel-bg-image-repeat,repeat );background-size:var( --os-ui-panel-bg-image-size,auto );background-position:var( --os-ui-panel-bg-image-position,center );box-sizing:border-box}:host( [ hidden ] ){display:none}`,Pe=class Pe extends k{render(){const e=this.gap,t=this.padding;return e&&/^\d+$/.test(e)&&this.style.setProperty("--os-ui-panel-gap",`${e}px`),t&&/^\d+$/.test(t)&&this.style.setProperty("--os-ui-panel-padding",`${t}px`),c`<slot></slot>`}};Pe.props=["gap","padding"],Pe.styles=[da],Pe.help=void 0;let ys=Pe;$("os-panel",ys);const ua=x` ${R}:host{display:flex;align-items:center;gap:10px;font-size:12px;color:var( --os-ui-fg-muted,#646970 )}input[ type='range' ]{appearance:none;-webkit-appearance:none;flex:1;min-width:0;height:18px;margin:0;padding:0;background:transparent;cursor:pointer}input[ type='range' ]:disabled{cursor:not-allowed;opacity:0.5}input[ type='range' ]::-webkit-slider-runnable-track{height:6px;border-radius:999px;background-image:linear-gradient( var( --_range-angle,90deg ),transparent var( --_fill,0% ),var( --_holo-track ) var( --_fill,0% ) ),linear-gradient( var( --os-ui-accent,#2271b1 ),var( --os-ui-accent,#2271b1 ) );background-size:auto,auto;background-position:center,center;background-repeat:no-repeat;box-shadow:inset 0 0 0 1px var( --_holo-track-edge )}input[ type='range' ]::-moz-range-track{height:6px;border-radius:999px;background-image:linear-gradient( var( --_range-angle,90deg ),transparent var( --_fill,0% ),var( --_holo-track ) var( --_fill,0% ) ),linear-gradient( var( --os-ui-accent,#2271b1 ),var( --os-ui-accent,#2271b1 ) );background-size:auto,auto;background-position:center,center;background-repeat:no-repeat;box-shadow:inset 0 0 0 1px var( --_holo-track-edge )}input[ type='range' ]::-webkit-slider-thumb{appearance:none;-webkit-appearance:none;width:16px;height:16px;margin-top:-5px;border:0;border-radius:50%;background:var( --os-ui-switch-knob,#fffbff );box-shadow:0 1px 3px rgba( 12,11,15,0.5 ),0 0 0 1px rgba( 12,11,15,0.14 );cursor:inherit;transition:transform var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}input[ type='range' ]::-moz-range-thumb{width:16px;height:16px;border:0;border-radius:50%;background:var( --os-ui-switch-knob,#fffbff );box-shadow:0 1px 3px rgba( 12,11,15,0.5 ),0 0 0 1px rgba( 12,11,15,0.14 );cursor:inherit;transition:transform var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}input[ type='range' ]:active::-webkit-slider-thumb{transform:scale( 1.15 );box-shadow:var( --_holo-glow-strong )}input[ type='range' ]:active::-moz-range-thumb{transform:scale( 1.15 );box-shadow:var( --_holo-glow-strong )}input[ type='range' ]:focus-visible{outline:none}input[ type='range' ]:focus-visible::-webkit-slider-thumb{box-shadow:var( --_holo-focus )}input[ type='range' ]:focus-visible::-moz-range-thumb{box-shadow:var( --_holo-focus )}.os-range-field__value{width:var( --os-ui-range-readout-width,3ch );flex:none;text-align:end;font-variant-numeric:tabular-nums;color:var( --os-ui-fg,#1d2327 )}@media ( prefers-reduced-motion:reduce ){input[ type='range' ]::-webkit-slider-thumb{transition-duration:1ms}input[ type='range' ]::-moz-range-thumb{transition-duration:1ms}}`;function ha(s,e,t){const o=Number.parseFloat(s),i=Number.parseFloat(e),n=Number.parseFloat(t);if(!Number.isFinite(o)||!Number.isFinite(i)||!Number.isFinite(n)||n===i)return"0%";const r=(o-i)/(n-i);return`${Math.min(100,Math.max(0,r*100)).toFixed(2)}%`}function pa(s){const e=s.indexOf(".");return e<0?0:Math.min(3,s.length-e-1)}function fa(s,e,t,o){const i=r=>{const a=Number.parseFloat(r);return Number.isFinite(a)?String(Math.trunc(Math.abs(a))).length+(a<0?1:0):3};return Math.max(i(s),i(e))+(t>0?t+1:0)+o.length}const ze=class ze extends k{render(){const e=this.label||"",t=this.value||"0",o=this.min||"0",i=this.max||"100",n=this.step||"1",r=this.suffix||"",a=this.decimals,l=a?Math.min(6,Math.max(0,Number.parseInt(a,10)||0)):pa(n),u=Number.parseFloat(t),h=Number.isFinite(u)?u.toFixed(l):t,p=fa(o,i,l,r);return c`
			...
			<span
				...
				>${h}${r}</span
			>
		`}connectedCallback(){super.connectedCallback(),getComputedStyle(this).direction==="rtl"&&this.style.setProperty("--_range-angle","270deg")}_onInput(e){const t=e.target,o=parseFloat(t.value);Number.isFinite(o)&&(this.value=String(o),this.emit("os-range-change",{value:o}))}};ze.props=["label","value","min","max","step","suffix","decimals"],ze.styles=[ua],ze.help=void 0;let ws=ze;$("os-range-field",ws);const ga=x`:host{display:block;margin-block-end:36px}:host( [ hidden ] ){display:none}.os-section__heading{margin:0 0 5px;font-size:20px;font-weight:500;letter-spacing:-0.01em;color:var( --os-ui-fg,#1d2327 )}.os-section__description{margin:0 0 14px;max-width:78ch;font-size:14px;color:var( --os-ui-fg-muted,#646970 );line-height:1.55}.os-section__description:empty{display:none}:host( [ stack ] ) .os-section__body{display:flex;flex-direction:column;gap:var( --os-ui-section-gap,12px )}`,Ie=class Ie extends k{render(){const e=this.heading||"",t=this.description||"";return c`
		...
		`};let $s=De;$("os-segmented",$s);const va=x` ${R}:host{display:flex;flex-direction:column;gap:8px;font-size:13px;color:var( --os-ui-fg,#1d2327 );min-width:0}:host( [ hidden ] ){display:none}.os-select__label{font-size:13px;line-height:1.5;color:var( --os-ui-fg-muted,#646970 )}.os-select__trigger{appearance:none;display:flex;align-items:center;gap:8px;width:100%;min-width:0;padding:8px 12px;background:var( --os-ui-hover,rgba( 0,0,0,0.05 ) );border:1px solid transparent;border-radius:8px;font:inherit;font-size:13px;line-height:1.5;color:var( --os-ui-fg,#1d2327 );text-align:start;cursor:pointer;transition:background-color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}.os-select__trigger:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.08 ) );border-color:var( --os-ui-border-strong,#8c8f94 )}.os-select__trigger:focus,.os-select__trigger:focus-visible{outline:none;border-color:var( --os-ui-accent,#2271b1 );box-shadow:var( --_holo-focus-field )}.os-select__trigger:disabled{opacity:0.5;cursor:not-allowed}:host( [ plain ] ){display:inline-flex;flex-direction:row;align-items:center}:host( [ plain ] ) .os-select__label{display:none}:host( [ plain ] ) .os-select__trigger{width:auto;padding:4px 6px;background:transparent;border-color:transparent;border-radius:6px}:host( [ plain ] ) .os-select__trigger:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.05 ) );border-color:transparent}:host( [ plain ] ) .os-select__trigger:focus,:host( [ plain ] ) .os-select__trigger:focus-visible{border-color:var( --os-ui-accent,#2271b1 )}:host( [ plain ] ) .os-select__value{flex:0 1 auto}.os-select__value{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.os-select__value--placeholder{color:var( --os-ui-fg-muted,#646970 )}.os-select__chevron{flex:0 0 auto;pointer-events:none;color:var( --os-ui-fg-muted,#646970 )}.os-select__trigger:hover .os-select__chevron{color:var( --os-ui-fg,#1d2327 )}.os-select__trigger:focus .os-select__chevron,.os-select__trigger:focus-visible .os-select__chevron{color:var( --os-ui-accent,#2271b1 )}.os-select__popup{position:fixed;inset:auto;margin:0;max-height:min( 320px,60vh );overflow-y:auto;padding:4px;background:var( --os-ui-surface,#fff );border:1px solid var( --os-ui-border,#dcdcde );border-radius:8px;box-shadow:0 12px 32px rgba( 0,0,0,0.45 );color:var( --os-ui-fg,#1d2327 )}.os-select__popup:not( [ data-open ] ){display:none}.os-select__popup:popover-open{display:block}.os-select__popup[ data-open ]{position:absolute;top:calc( 100% + 4px );left:0;min-width:100%;z-index:1000}:host{position:relative}.os-select__option{display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:6px;font-size:13px;line-height:1.5;cursor:pointer;white-space:nowrap}.os-select__check{flex:0 0 auto;visibility:hidden}.os-select__option[ aria-selected='true' ] .os-select__check{visibility:visible}.os-select__option[ data-active ],.os-select__option:hover{background:var( --os-ui-accent,#2271b1 );color:var( --os-ui-fg-on-accent,#fff )}.os-select__option[ aria-disabled='true' ]{opacity:0.5;cursor:not-allowed}.os-select__option[ aria-disabled='true' ]:hover{background:transparent;color:inherit}@media ( prefers-reduced-motion:reduce ){.os-select__trigger{transition-duration:1ms}}`,_a=x`:host{display:none}`,Ne=class Ne extends k{render(){return c``}};Ne.props=["value","disabled"],Ne.styles=[_a],Ne.help=void 0;let As=Ne;$("os-option",As);const He=class He extends k{constructor(){super(...arguments),this._optionObserver=null,this._open=!1,this._activeIndex=-1,this._typed="",this._dismissedAt=0,this._typedTimer=null,this._onWindowScroll=e=>{const t=e.target,o=this._popup();o&&t instanceof Node&&(t===o||o.contains(t))||this._hide()},this._onWindowResize=()=>this._hide()}set items(e){const t=this.querySelectorAll(":scope > os-option");for(const n of Array.from(t))n.remove();for(const n of e){const r=document.createElement("os-option");r.setAttribute("value",n.value),r.textContent=n.label,this.appendChild(r)}const o=this.value;!(o!==null&&e.some(n=>n.value===o))&&e.length>0&&(this.value=e[0].value),this.requestUpdate()}connectedCallback(){super.connectedCallback(),ge(this),this._optionObserver=new MutationObserver(()=>this.requestUpdate()),this._optionObserver.observe(this,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["value","disabled"],characterData:!0})}disconnectedCallback(){this._optionObserver?.disconnect(),this._optionObserver=null,this._teardownDismiss(),this._open=!1,this._typedTimer&&(clearTimeout(this._typedTimer),this._typedTimer=null)}render(){const e=this.label||"",t=this.value,o=this.placeholder||"",i=this.disabled!==null;e&&this.setAttribute("aria-label",e);const n=this.getAttribute("aria-label")||"",r=e||n||o,a=this._readOptions(),l=a.find(b=>b.value===t),u=l?l.label:o,h=this.id||"os-unnamed",p=`${h}__listbox`,m=`${h}__trigger`,_=this._open&&this._activeIndex>=0?`${h}__opt-${this._activeIndex}`:"";return c`
			...
			>
				<span
					...
					>${u}</span
				>
				${z("chevron-right",{size:16,rotate:90,className:"os-select__chevron"})}
			</button>
			<div
			...
			>
				${a.map((b,w)=>c`
						<div
							...
							?data-active=${this._open&&w===this._activeIndex}
							@click=${()=>this._onOptionClick(b)}
							@pointermove=${()=>this._setActive(w)}
						>
							${z("check",{size:16,className:"os-select__check"})}
		...
		`};let Ts=je;$("os-tabpanel",Ts);function Aa(s,e,t){const o=s.querySelectorAll(`:scope > ${e}`);for(const i of Array.from(o))i.remove();for(const i of t){const n=document.createElement(e);n.setAttribute("value",i.value),n.textContent=i.label,s.appendChild(n)}}function Ca(s){const e=s.parentElement;if(!e)return null;const t=e.querySelector(":scope > os-tabs");return t||s.closest("os-tabs")}function wi(s,e){const t=new Set,o=s.parentElement;if(o)for(const i of Array.from(o.querySelectorAll(":scope > os-tabpanel")))t.add(i);for(const i of Array.from(s.querySelectorAll(":scope > os-tabpanel")))t.add(i);for(const i of t){const n=i.getAttribute("for"),r=n!==null&&n===e;r?i.removeAttribute("hidden"):i.setAttribute("hidden",""),i.setAttribute("aria-hidden",r?"false":"true")}}const xi=x` ${R} ${hs}:host{display:flex;flex-direction:column;gap:4px;font-size:13px;color:var( --os-ui-fg,#1d2327 );min-width:0}:host( [ hidden ] ){display:none}.os-text-field__label{font-size:12px;color:var( --os-ui-fg-muted,#646970 )}.os-text-field__row{position:relative;display:flex;align-items:center;width:100%}input{appearance:none;-webkit-appearance:none;display:block;width:100%;min-width:0;box-sizing:border-box;padding:7px 10px;background:var( --os-window-bg,#fff );border:1px solid var( --os-ui-border,#dcdcde );border-radius:6px;font:inherit;font-size:13px;color:var( --os-ui-fg,#1d2327 )}.os-text-field__suffix{position:absolute;inset-inline-end:10px;top:50%;transform:translateY( -50% );pointer-events:none;font-size:12px;color:var( --os-ui-fg-muted,#646970 )}.os-text-field__row--has-reveal input{padding-inline-end:36px}.os-text-field__reveal{position:absolute;inset-inline-end:0;top:0;bottom:0;width:34px;display:flex;align-items:center;justify-content:center;padding:0;border:none;background:transparent;color:var( --os-ui-fg-muted,#646970 );cursor:pointer;border-radius:0 6px 6px 0;transition:color 0.12s ease}.os-text-field__reveal:hover{color:var( --os-ui-accent,#2271b1 )}.os-text-field__reveal:focus-visible{outline:none;color:var( --os-ui-accent,#2271b1 );box-shadow:inset 0 0 0 2px var( --os-ui-accent,#2271b1 );border-radius:0 6px 6px 0}.os-text-field__reveal:disabled{opacity:0.45;cursor:not-allowed}.os-text-field__input--masked{-webkit-text-security:disc;text-security:disc}@supports not ( ( -webkit-text-security:disc ) or ( text-security:disc ) ){.os-text-field__input--masked{font-family:text-security-disc,"password",monospace;letter-spacing:0.2em}}input:disabled{opacity:0.55;cursor:not-allowed;background:var( --os-ui-hover,rgba( 0,0,0,0.03 ) )}input[ aria-invalid='true' ],input[ aria-invalid='true' ]:hover:not(:disabled ){border-color:var( --os-ui-danger,#d63638 )}input[ aria-invalid='true' ]:focus,input[ aria-invalid='true' ]:focus-visible{border-color:var( --os-ui-danger,#d63638 );box-shadow:0 0 0 1px var( --os-ui-danger,#d63638 ),0 0 0 4px rgba( 214,54,56,0.18 )}input[ type='number' ]::-webkit-inner-spin-button,input[ type='number' ]::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}input[ type='number' ]{-moz-appearance:textfield}`,Ve=class Ve extends k{constructor(){super(...arguments),this._revealed=!1}connectedCallback(){super.connectedCallback(),ge(this)}render(){const e=this.label||"",t=this.value??"",o=this.placeholder||"",i=this.disabled!==null,n=this.readonly!==null,r=this.autocomplete,a=this.type||"text",l=a==="password";let u=r||"off";l&&(!r||u==="off")&&(u="new-password");const h=this.maxlength,p=this.minlength,m=this.pattern||"",_=this.name||"",b=this.suffix||"",w=this.invalid!==null,v=this.reveal!==null,g=a==="password",f=g&&!(v&&this._revealed);let y;g||v&&this._revealed?y="text":y=a;const A=v?"os-text-field__row os-text-field__row--has-reveal":"os-text-field__row",M=f?"os-text-field__input os-text-field__input--masked":"os-text-field__input",N=`${this.id||"os-unnamed"}__input`;return c`
			...
			<span class=${A}>
				...
				${v?this._renderRevealButton(i):c``}
			</span>
	...
	`}const La="os-window-tab-change";function ki(s){return Array.from(s.querySelectorAll(".os-window__tab"))}function $i(s){return s.replace(/[^a-zA-Z0-9_-]/g,"-")}function Ai(s,e){return`${s.id||"os-window"}-tab-${$i(e)}`}function Ci(s,e){return`${s.id||"os-window"}-panel-${$i(e)}`}function Ra(s){const e=s.querySelector(".os-window__tab-plate");if(!e)return;const t=s.querySelector(".os-window__tab--active");if(!t){e.dataset.empty="",s.dataset.tabPlateEmpty="";return}delete e.dataset.empty,delete s.dataset.tabPlateEmpty,s.style.setProperty("--_tab-plate-x",`${t.offsetLeft}px`),s.style.setProperty("--_tab-plate-w",`${t.offsetWidth}px`),s.style.setProperty("--_tab-strip-w",`${s.clientWidth}px`),t.offsetWidth>0&&(e.dataset.placed="")}function Ta(s){if(s){if(s.querySelector(":scope > .os-window__tab")){s.setAttribute("role","tablist");const e=s.dataset.tablistLabel;e&&s.setAttribute("aria-label",e);return}s.setAttribute("role","presentation"),s.removeAttribute("aria-label")}}function Pa(s){const e=ki(s);if(e.length===0)return;const t=e.find(o=>o.classList.contains("os-window__tab--active"))??e[0];for(const o of e){const i=o===t?0:-1;o.tabIndex!==i&&(o.tabIndex=i)}}function Si(s){const e=new Map;for(const t of ki(s))t.dataset.kind==="panel"&&t.dataset.panel&&e.set(t.dataset.panel,t);return e}function Ei(s){const e=s.querySelector(".os-window__body");return e?Array.from(e.querySelectorAll("os-tabpanel[ for ]")).filter(t=>!t.parentElement?.closest("os-tabpanel, os-tabs")):[]}function za(s,e){const t=s.querySelector(".os-window__tabs");if(!t)return;let o=!1;for(const[i,n]of Si(t)){const r=i===e;o=o||r,n.classList.toggle("os-window__tab--active",r),n.setAttribute("aria-selected",r?"true":"false")}if(o){for(const i of Ei(s)){const n=i.getAttribute("for")===e;i.toggleAttribute("hidden",!n),i.setAttribute("aria-hidden",n?"false":"true")}Pa(t),Ra(t),s.dispatchEvent(new CustomEvent(La,{bubbles:!0,detail:{value:e}}))}}function Ia(s,e,t){const o=s.querySelector(".os-window__tabs");if(!o)return;const i=Si(o),n=Array.from(i.entries()).find(([,h])=>h.classList.contains("os-window__tab--active"))?.[0];let r=o.querySelector(".os-window__tab-plate");for(const h of e){let p=i.get(h.value);if(p)i.delete(h.value);else{const m=document.createElement("button");m.type="button",p=m,p.className="os-window__tab",p.dataset.kind="panel",p.dataset.panel=h.value,p.setAttribute("role","tab"),p.setAttribute("aria-selected","false"),p.tabIndex=-1}p.textContent!==h.label&&(p.textContent=h.label),p.id=Ai(s,h.value),p.setAttribute("aria-controls",Ci(s,h.value)),p.previousElementSibling!==r&&o.insertBefore(p,r?r.nextSibling:o.firstChild),r=p}for(const h of i.values())h.remove();Ta(o);const a=o.querySelector(":scope > .os-window__tab")!==null;for(const h of Ei(s)){const p=h.getAttribute("for");p&&(h.id=Ci(s,p),a?h.setAttribute("aria-labelledby",Ai(s,p)):h.removeAttribute("aria-labelledby"))}const l=h=>h!==void 0&&e.some(p=>p.value===h),u=[t,n,e[0]?.value].find(l);u!==void 0&&za(s,u)}const be={size:null},Li={appearance:()=>z("color",be),themes:()=>{const s=document.createElementNS("http://www.w3.org/2000/svg","svg");return s.setAttribute("viewBox","0 0 24 24"),s.setAttribute("fill","currentColor"),s.setAttribute("aria-hidden","true"),s.innerHTML='<path fill-rule="evenodd" clip-rule="evenodd" d="M20 12a8 8 0 1 1-16 0 8 8 0 0 1 16 0Zm-1.5 0a6.5 6.5 0 0 1-6.5 6.5v-13a6.5 6.5 0 0 1 6.5 6.5Z" />',s},windows:()=>z("windows",be),navigation:()=>{const s=document.createElementNS("http://www.w3.org/2000/svg","svg");return s.setAttribute("viewBox","0 0 24 24"),s.setAttribute("fill","currentColor"),s.setAttribute("aria-hidden","true"),s.innerHTML='<path d="M12 4c-4.4 0-8 3.6-8 8s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm0 14.5c-3.6 0-6.5-2.9-6.5-6.5S8.4 5.5 12 5.5s6.5 2.9 6.5 6.5-2.9 6.5-6.5 6.5zM9 16l4.5-3L15 8.4l-4.5 3L9 16z" />',s},features:()=>z("settings",be),help:()=>z("widgets",be),"os-file-associations":()=>{const s=document.createElementNS("http://www.w3.org/2000/svg","svg");return s.setAttribute("viewBox","0 0 24 24"),s.setAttribute("fill","currentColor"),s.setAttribute("aria-hidden","true"),s.innerHTML='<path d="M15.5 7.5h-7V9h7V7.5Zm-7 3.5h7v1.5h-7V11Zm7 3.5h-7V16h7v-1.5Z" /><path d="M17 4H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2ZM7 5.5h10a.5.5 0 0 1 .5.5v12a.5.5 0 0 1-.5.5H7a.5.5 0 0 1-.5-.5V6a.5.5 0 0 1 .5-.5Z" />',s},about:()=>z("info",be)},zs=1920,Is=1080,Ma=40,Da=300,jt="custom-gradient",V="custom-image",Ri="galaxy",me="custom",Ti=[{id:"pulse",label:"Pulse",value:"#f252fc"},{id:"nebula",label:"Nebula",value:"#ec9bff"},{id:"sirius",label:"Sirius",value:"#9af2ff"},{id:"lagoon",label:"Lagoon",value:"#9f98ff"},{id:"wp-blue",label:"WordPress Blue",value:"#2271b1"},{id:"indigo",label:"Indigo",value:"#3858e9"},{id:"teal",label:"Teal",value:"#04a4cc"},{id:"emerald",label:"Emerald",value:"#059669"},{id:"amber",label:"Amber",value:"#d97706"},{id:"rose",label:"Rose",value:"#e11d48"}];function Ms(){const e=window.wp?.os?.config?.accentColors;if(!Array.isArray(e)||e.length===0)return Ti;const t=[];for(const o of e)o&&typeof o=="object"&&typeof o.id=="string"&&typeof o.label=="string"&&typeof o.value=="string"&&o.id!==""&&o.label!==""&&/^#[0-9a-f]{3,8}$/i.test(o.value)&&t.push({id:o.id,label:o.label,value:o.value});return t.length>0?t:Ti}const Pi=[{id:"compact",label:"Compact",width:48,icon:18},{id:"default",label:"Default",width:56,icon:20},{id:"large",label:"Large",width:72,icon:26}],zi=[{id:"sharp",label:"Sharp",value:0},{id:"default",label:"Default",value:8},{id:"round",label:"Round",value:16}],Ii=[{id:"static",label:"Static"},{id:"dynamic",label:"Dynamic"},{id:"hidden",label:"Hidden"}],Mi=[{id:"bottom",label:"Bottom"},{id:"left",label:"Left"},{id:"right",label:"Right"}],Di=[{id:"unified",label:"Unified"},{id:"classic",label:"Split"}],Y={wallpaper:Ri,accent:"pulse",customAccent:"#f252fc",dockSize:"default",windowRadius:"round",adminBarMode:"hidden",desktopLayout:"unified",dockPlacement:"bottom",dockRailRenderer:"default",desktopTheme:"",appliedThemeRecommendations:[],unfocusEffect:"darken",windowReveal:"none",windowRevealDuration:0,windowLinkRenderer:"svg-splines",windowLinkVisibility:"always",windowLinksEnabled:!0,windowLinkRaiseOnFocus:!0,windowLinkHighlight:!0,customGradient:{from:"#2271b1",to:"#7c3aed",angle:135},customImage:null,wallpaperSettings:{},libraryHdOnly:!0,ai:{enabled:!1},heartbeatRate:60,nativePostsEnabled:!1,nativePostsHiddenColumns:[],nativePagesEnabled:!1,nativeUsersEnabled:!1,nativePluginsEnabled:!1,nativeCommentsEnabled:!1,stationHomeEnabled:!1,showDesktopOnWallpaperClick:!1,mioEnabled:!1,mioStyle:{appearance:{},physics:{}},showPostStatusRibbons:!0,developerModeEnabled:!1,foldersSharingEnabled:!0,navPlacement:{},navOrder:[],dockPromotedPositions:{}};function Ds(s){return!!s&&typeof s=="object"&&typeof s.then=="function"}function Na(s){return s.replace(/[^a-zA-Z0-9._-]+/g,"-").replace(/^-+|-+$/g,"")||"wallpaper"}function Ha(s){if(!s||typeof s.id!="number"||!s.source_url)return!1;const e=s.media_details;return!!e&&typeof e.width=="number"&&typeof e.height=="number"&&e.width>0&&e.height>0}function qa(s){if(!s)return"";const e=document.createElement("div");return e.innerHTML=s,e.textContent?.trim()||""}const Ni="X-WP-Nonce";function Ua(s,e){const t=Fa();if(!t)return e;const o=Wa(s);if(!o||!ja(o))return e;const i=e?.headers??(typeof Request<"u"&&s instanceof Request?s.headers:void 0),n=new Headers(i??{});return n.has(Ni)?e:(n.set(Ni,t),{...e??{},headers:n})}function Fa(){if(typeof window>"u")return;const e=window.openStationConfig?.restNonce;return typeof e=="string"&&e.length>0?e:void 0}function Wa(s){try{const e=typeof window<"u"&&window.location?window.location.href:void 0;return typeof s=="string"?new URL(s,e):s instanceof URL?s:typeof Request<"u"&&s instanceof Request?new URL(s.url,e):null}catch{return null}}function ja(s){return typeof window>"u"||!window.location||s.origin!==window.location.origin?!1:!!(s.pathname.includes("/wp-json/")||s.searchParams.has("rest_route"))}function O(s,e,t={}){const o=window.wp?.os?.fetch;if(typeof o=="function")return o(s,e,t);const i=Ua(s,e);return fetch(s,i)}function Va(){return{...Y,customGradient:{...Y.customGradient},customImage:null,wallpaperSettings:{...Y.wallpaperSettings},ai:{...Y.ai},appliedThemeRecommendations:[...Y.appliedThemeRecommendations],navPlacement:{...Y.navPlacement},navOrder:[...Y.navOrder],dockPromotedPositions:{...Y.dockPromotedPositions}}}const Hi="__openStationSharedStores";function Oa(){const s=window;let e=s[Hi];return e||(e=new Map,s[Hi]=e),e}function K(s,e){const t=Oa();let o=t.get(s);o||(o={state:e(),listeners:new Set,rebuild:e},t.set(s,o));const i={get state(){return o.state},set state(n){o.state=n},getState(){return o.state},notify(){for(const n of Array.from(o.listeners))try{n(o.state)}catch(r){console.error(`[desktop-mode/shared-store:${s}] subscriber threw:`,r)}},subscribe(n){return o.listeners.add(n),()=>{o.listeners.delete(n)}},setState(n){const r=o.state;if(typeof r!="object"||r===null){console.warn(`[desktop-mode/shared-store:${s}] setState called on a primitive store; use the state setter instead.`);return}Object.assign(r,n),i.notify()},reset(){const n=o.rebuild(),r=o.state;if(typeof r=="object"&&r!==null&&typeof n=="object"&&n!==null){const a=r;for(const l of Object.keys(a))delete a[l];Object.assign(a,n)}else o.state=n;o.listeners.clear()}};return i}const qi=K("desktop-mode/settings-tab-registry",()=>({registry:new Map,listeners:new Set})),Ka=qi.state.registry,Ui=qi.state.listeners;function Ba(){return Array.from(Ka.values()).sort((s,e)=>(s.order??100)-(e.order??100))}function Ga(s){return Ui.add(s),()=>{Ui.delete(s)}}const Fi="https://openstation.blog/",Wi="https://openstation.blog/feed/",Xa="https://github.com/WordPress/openstation/";function Za(s,e){return s.clientWidth>0&&s.clientHeight>0?Promise.resolve():new Promise(t=>{const o=()=>{i.disconnect(),e.removeEventListener("abort",o),t()},i=new ResizeObserver(()=>{s.clientWidth>0&&s.clientHeight>0&&o()});e.addEventListener("abort",o,{once:!0}),i.observe(s)})}function Ns(s){if(typeof s!="string"||s==="")return"";try{const e=new URL(s,window.location.href);return e.protocol==="http:"||e.protocol==="https:"?e.href:""}catch{return""}}function ae(s){return typeof s=="string"?s.trim():""}function Ya(s){if(!s||typeof s!="object")return null;const e=s,o=(Array.isArray(e.items)?e.items:[]).map(i=>{if(!i||typeof i!="object")return null;const n=i,r=ae(n.title),a=Ns(n.url);return r===""||a===""?null:{title:r,url:a,author:ae(n.author),publishedAt:ae(n.publishedAt),excerpt:ae(n.excerpt)}}).filter(i=>i!==null).slice(0,5);return{title:ae(e.title)||d("OpenStation"),description:ae(e.description)||d("A public dev diary — building a desktop OS for wp-admin."),homeUrl:Ns(e.homeUrl)||Fi,feedUrl:Ns(e.feedUrl)||Wi,items:o,stale:e.stale===!0}}function Ja(s){if(s==="")return"";const e=new Date(s);return Number.isNaN(e.getTime())?"":e.toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function ji(s){const e=Ja(s.publishedAt);return e&&s.author?j("%1$s · %2$s",e,s.author):e||s.author}function Qa(s){return c`
	...
	`}function tl(s){if(s.kind==="loading")return c`
			<div
		...
		`;if(s.feed.items.length===0)return c`
		...
		`;const[e,...t]=s.feed.items;return c`
		...
		>
			${Qa(e)}
			${t.length>0?c`<div class="os-settings__about-grid" role="list">
						${t.map(el)}
					</div>`:c``}
		</section>
	`}function Vt(s,e,t){const o=t.kind==="ready"?t.feed:null,i=o?.homeUrl||Fi,n=o?.feedUrl||Wi,r=o?.description||d("A public dev diary — building a desktop OS for wp-admin."),a=`${e.pluginUrl??""}/assets/images/openstation-mark.svg`,l=e.pluginVersion??"";L(c`
		...
		`,s)}function sl(){const s=document.createElement("div");s.classList.add("os-settings__about");const e=window.openStationConfig??{};Vt(s,e,{kind:"loading"});const t=new AbortController;let o=!1;const i=async()=>{if(await Za(s,t.signal),!(o||!s.isConnected)){if(!e.aboutFeedUrl){Vt(s,e,{kind:"error"});return}try{const r=await O(e.aboutFeedUrl,{credentials:"same-origin",signal:t.signal},{windowId:"desktop-mode-os-settings",source:"openstation/about-feed"});if(!r.ok)throw new Error(`about feed returned ${r.status}`);const a=await r.json(),l=a.success?Ya(a.data):null;if(!l)throw new Error("about feed returned an invalid payload");!o&&s.isConnected&&Vt(s,e,{kind:"ready",feed:l})}catch(r){if(t.signal.aborted||o)return;typeof console<"u"&&console.error("[openstation/about] journal feed failed:",r),Vt(s,e,{kind:"error"})}}};requestAnimationFrame(()=>{i()});const n=new MutationObserver(()=>{s.isConnected||(o=!0,t.abort(),n.disconnect())});return n.observe(document.body,{childList:!0,subtree:!0}),s}function ol(s,e){switch(s){case"wp-blue":return d("WordPress Blue");case"indigo":return d("Indigo");case"teal":return d("Teal");case"emerald":return d("Emerald");case"amber":return d("Amber");case"rose":return d("Rose");default:return e}}function il(s,e){switch(s){case"compact":return d("Compact");case"default":return d("Default");case"large":return d("Large");default:return e}}function nl(s,e){switch(s){case"sharp":return d("Sharp");case"default":return d("Default");case"round":return d("Round");default:return e}}function rl(s,e){switch(s){case"static":return d("Static");case"dynamic":return d("Dynamic");case"hidden":return d("Hidden");default:return e}}function al(s,e){switch(s){case"bottom":return d("Bottom");case"left":return d("Left");case"right":return d("Right");default:return e}}function ll(s,e){switch(s){case"classic":return d("Split");case"unified":return d("Unified");default:return e}}function cl(s){switch(s){case"classic":return d("Core admin menus are placed in a sidebar; plugins, apps, and OpenStation controls in a dock along the bottom edge.");case"unified":return d("Every menu in a single dock.");default:return""}}const dl="conic-gradient( #f252fc, #9af2ff, #93f0c6, #f8f2b6, #ff5a5a, #f252fc )";function ul(s){const e=()=>{const r=i.querySelector(".os-settings__accent-picker");if(r)try{r.showPicker()}catch{r.click()}},t=r=>{const a=r.detail?.value??"";a!==me&&!Ms().some(l=>l.id===a)||(s.state.accent=a,s.save(),s.apply(),n(),a===me&&e())},o=r=>{const a=r.target.value;/^#[0-9a-fA-F]{6}$/.test(a)&&(s.state.customAccent=a,s.state.accent=me,s.save(),s.apply(),n())},i=document.createElement("div"),n=()=>{const r=s.state.accent===me;L(c`
				<os-section
				...
				>
					<os-swatch-grid
						...
						@os-pick=${t}
					>
						${Ms().map(a=>c`<os-swatch
								value=${a.id}
								label=${ol(a.id,a.label)}
								preview=${a.value}
								size="small"
								variant="accent"
						...
						<span class="os-settings__accent-custom">
							<os-swatch
								value=${me}
								label=${d("Custom")}
								preview=${r?s.state.customAccent:dl}
								size="small"
								variant="accent"
								?selected=${r}
							></os-swatch>
			...
			`,i)};return n(),i}function hl(s){switch(s){case"dynamic":return d("The admin bar slides out of the way and comes back when you move the pointer to the top edge of the screen.");case"hidden":return d("The admin bar is never shown. Use the Exit OpenStation tile on the dock to get back to the classic admin.");default:return d("The admin bar is always visible above the desktop.")}}function pl(s){const e=i=>{const n=i.detail?.value??"";Ii.some(r=>r.id===n)&&(s.state.adminBarMode=n,s.save(),s.apply(),o())},t=document.createElement("div"),o=()=>L(c`
				<os-section
					heading=${d("Admin bar")}
					description=${hl(s.state.adminBarMode)}
				>
					<os-segmented
		...
		`}};Oe.props=["open","title","message","confirm-label","cancel-label","danger","hide-cancel","dismissable"],Oe.styles=[fl],Oe.help=void 0;let Hs=Oe;$("os-confirm-dialog",Hs);function Bi(s){return new Promise(e=>{const t=document.createElement("os-confirm-dialog");t.setAttribute("open",""),s.title&&t.setAttribute("title",s.title),t.setAttribute("message",s.message),s.confirmLabel&&t.setAttribute("confirm-label",s.confirmLabel),s.cancelLabel&&t.setAttribute("cancel-label",s.cancelLabel),s.danger&&t.setAttribute("danger",""),s.hideCancel&&t.setAttribute("hide-cancel",""),s.dismissable&&t.setAttribute("dismissable","");const o=i=>{t.remove(),e(i)};t.addEventListener("os-confirm",()=>o(!0)),t.addEventListener("os-cancel",()=>o(!1)),document.body.appendChild(t)})}const Gi=K("desktop-mode/dock-rail-registry",()=>({registry:new Map,listeners:new Set,activeId:"default"})),Xi=Gi.state.registry,Zi=Gi.state.listeners;function bl(s){return Xi.get(s)}function Yi(){return Array.from(Xi.values())}function ml(s){return Zi.add(s),()=>{Zi.delete(s)}}function Ot(){const s=window.wp?.hooks;if(!s)throw new Error("[openstation] `window.wp.hooks` is not available. The plugin declares `wp-hooks` as a script dependency; if you are seeing this error, verify the enqueue order.");return s}function vl(s,e,t,o){Ot().addAction(s,e,t,o)}function _l(s,e){return Ot().removeAction(s,e)}function J(s,e,...t){return Ot().applyFilters(s,e,...t)}function re(s,...e){Ot().doAction(s,...e)}const F={WALLPAPERS:"os.wallpapers",UNFOCUS_EFFECTS:"os.unfocus-effects",WINDOW_REVEALS:"os.window-reveals",WALLPAPER_PREVIEW_PARAMS:"os.wallpaper.preview-params",WALLPAPER_SETTINGS_CHANGED:"os.wallpaper.settings-changed",SHELL_ERROR:"os.shell.error",DESKTOP_THEME_ICON:"os.os-theme.icon",DESKTOP_THEME_ICON_COLOR:"os.os-theme.icon-color",WALLPAPERS_SERVER_CHANGED:"os.wallpapers.server-changed",WINDOW_LINK_RENDERERS:"os.window-links.renderers"};function Ji(s,e){if(!s||typeof s!="object")return["def (not an object)"];const t=s,o=[];for(const i of e)i.valid(t)||o.push(`${i.field} (${i.message})`);return o}class yl extends Error{constructor(e,t,o){super(`[openstation] ${e} registration rejected — fields: `+t.join(", ")+"."),this.name="RegistrationError",this.kind=e,this.errors=t,this.def=o}}function qs(s,e,t){if(e.length!==0)throw new yl(s,e,t)}const Q=[[0,0],[100,0],[100,100],[0,100]],wl=48,xl=80,Us=6,Fs=8,Qi=4,en=3,tn=64,Ws=105,ee=2;function sn(s){return`${Math.round(s*1e3)/1e3}%`}function te(s){return s.slice().reverse()}function ve(s,e,t,o){return[[s,e],[t,e],[t,o],[s,o]]}function kl(s,e,t,o){const i=[];for(let n=0;n<o;n++){const r=n/o*Math.PI*2;i.push([s+Math.cos(r)*t,e+Math.sin(r)*t])}return i}function B(s,e=[]){const t=[],o=i=>{t.push(`${sn(i[0])} ${sn(i[1])}`)};for(const i of s)o(i);if(e.length>0){o(s[0]);for(const i of e){for(const n of i)o(n);o(i[0]),o(s[0])}}return`polygon( ${t.join(", ")} )`}function on(s){return B(Q,[te(kl(50,50,s,wl))])}function nn(s){return B(Q,[te(ve(50-s,-ee,50+s,100+ee))])}function rn(s){const e=100/Us,t=[];for(let o=0;o<Us;o++){const i=o*e;t.push(te(ve(-ee,i,100+ee,i+s)))}return B(Q,t)}function an(s){return B(Q,[te(ve(-ee,50-s,100+ee,50+s))])}function ln(s){const e=100/Fs,t=[];for(let o=0;o<Fs;o++){const i=o*e;t.push(te(ve(i,-ee,i+s,100+ee)))}return B(Q,t)}function cn(s){return B(Q,[te([[50,50-s],[50+s,50],[50,50+s],[50-s,50]])])}function dn(s){const e=100/Qi,t=100/en,o=[];for(let i=0;i<en;i++)for(let n=0;n<Qi;n++){const r=n*e+e/2,a=i*t+t/2,l=e/2*s,u=t/2*s;o.push(te(ve(r-l,a-u,r+l,a+u)))}return B(Q,o)}function un(s){const e=-Math.PI/2,t=s/180*Math.PI,o=[[50,50]];for(let i=0;i<tn;i++){const n=e+i/(tn-1)*t;o.push([50+Math.cos(n)*Ws,50+Math.sin(n)*Ws])}return B(Q,[te(o)])}function $l(){return{from:on(0),to:on(xl)}}function Al(){return{from:nn(0),to:nn(52)}}function Cl(){return{from:rn(0),to:rn(100/Us+.5)}}function Sl(){return{from:an(0),to:an(52)}}function El(){return{from:ln(0),to:ln(100/Fs+.5)}}function Ll(){return{from:cn(0),to:cn(Ws)}}function Rl(){return{from:dn(0),to:dn(1.04)}}function Tl(){return{from:un(0),to:un(360)}}function Pl(){return{from:"inset( 0% 0% 0% 0% )",to:"inset( 0% 0% 100% 0% )"}}function zl(){return{from:"inset( 0% 0% 0% 0% )",to:"inset( 0% 0% 0% 100% )"}}function Il(){return{from:B([[-60,0],[100,0],[100,100],[0,100]]),to:B([[100,0],[260,0],[260,100],[160,100]])}}const js=6,Vs=100,le=Vs/2,Kt=200,hn=90,pn=["#5c5c6d","#525261","#484855","#3e3e49","#34343d","#2a2a31"],Ml="#101014",Dl=1.25,ce="http://www.w3.org/2000/svg";let Nl=0;function Bt(s){return Math.round(s*1e3)/1e3}function Os(s){return s/js*Math.PI*2-Math.PI/2}function Hl(s){const t=Os(s)+Math.PI/js+Math.PI/2;return{x:Math.cos(t),y:Math.sin(t)}}function ql(s){const e=Os(s),t=Os(s+1);return[[le,le],[le+Math.cos(e)*Kt,le+Math.sin(e)*Kt],[le+Math.cos(t)*Kt,le+Math.sin(t)*Kt]]}function fn(s,e){const t=ql(s),o=document.createElementNS(ce,"path");return o.setAttribute("d",`M ${t.map(([i,n])=>`${Bt(i)} ${Bt(n)}`).join(" L ")} Z`),e?o.setAttribute("fill","#fff"):(o.setAttribute("fill",pn[s%pn.length]),o.setAttribute("stroke",Ml),o.setAttribute("stroke-width",String(Dl)),o.setAttribute("vector-effect","non-scaling-stroke"),o.setAttribute("stroke-linejoin","round")),o}function Ul(){const s=`os-iris-${++Nl}`,e=document.createElementNS(ce,"svg");e.setAttribute("viewBox",`0 0 ${Vs} ${Vs}`),e.setAttribute("preserveAspectRatio","none"),e.setAttribute("aria-hidden","true"),e.style.width="100%",e.style.height="100%",e.style.display="block";const t=document.createElementNS(ce,"defs"),o=document.createElementNS(ce,"mask");o.setAttribute("id",s);const i=document.createElementNS(ce,"g"),n=document.createElementNS(ce,"g");n.setAttribute("mask",`url(#${s})`);const r=[];for(let l=0;l<js;l++){const u=fn(l,!0),h=fn(l,!1);i.appendChild(u),n.appendChild(h),r.push({el:u,index:l},{el:h,index:l})}o.appendChild(i),t.appendChild(o),e.appendChild(t),e.appendChild(n);const a=document.createElement("div");return a.appendChild(e),{element:a,play:l=>r.map(({el:u,index:h})=>{const p=Hl(h),m=Bt(p.x*hn),_=Bt(p.y*hn);return u.animate([{transform:"translate( 0px, 0px )"},{transform:`translate( ${m}px, ${_}px )`}],{duration:l.duration,easing:l.easing,delay:l.delay,fill:"both"})})}}const _e="none",gn=K("desktop-mode/window-reveal-registry",()=>({registry:new Map,listeners:new Set})),Ks=gn.state.registry,Bs=gn.state.listeners,bn=/^[a-z0-9_/-]+$/;function mn(s){const e=/^\s*([a-zA-Z-]+)\s*\(/.exec(s);return e?e[1].toLowerCase():""}const Fl=0;function vn(s,e){const t=[],o=typeof s.from=="string"&&s.from.trim()!=="",i=typeof s.to=="string"&&s.to.trim()!=="";if(o||t.push(`${e}.from (missing — the clip-path covering the window)`),i||t.push(`${e}.to (missing — the clip-path uncovering the window)`),o&&i){const n=mn(s.from),r=mn(s.to);n===""||r===""?t.push(`${e}.from|to (must be shape functions, e.g. \`inset( … )\` or \`polygon( … )\` — bare keywords like \`none\` cannot be animated)`):n!==r&&t.push(`${e}.from|to (must use the same shape function to interpolate — got \`${n}()\` and \`${r}()\`)`)}return t}function Wl(s){if(typeof KeyframeEffect!="function")return!0;try{return new KeyframeEffect(null,null,{easing:s}),!0}catch{return!1}}function W(s){const e=[];if(!s||typeof s!="object")e.push("def (not an object)");else{typeof s.id!="string"||s.id.trim()===""?e.push("id (missing)"):bn.test(s.id.trim().toLowerCase())?s.id.trim().toLowerCase()===_e&&e.push('id ("none" is reserved)'):e.push(`id (must match ${bn} — lowercase alphanum, hyphens, underscores, slashes for vendor/sub-id)`),(typeof s.label!="string"||s.label.trim()==="")&&e.push("label (missing)");const o=s.render!==void 0,i=Array.isArray(s.layers),n=s.from!==void 0||s.to!==void 0;o&&typeof s.render!="function"?e.push("render (not a function)"):o&&(i||n)?e.push("render|layers|from|to (supply exactly one of: a from/to pair, a layers array, or a render function)"):o||(i&&n?e.push("layers|from|to (supply EITHER a single from/to pair OR a layers array, not both)"):i?(s.layers.length===0&&e.push("layers (empty — supply at least one matched pair)"),s.layers.forEach((r,a)=>{if(!r||typeof r!="object"){e.push(`layers[ ${a} ] (not an object)`);return}e.push(...vn(r,`layers[ ${a} ]`))})):e.push(...vn(s,"").map(r=>r.replace(/^\./,"")))),s.easing!==void 0&&(typeof s.easing!="string"||s.easing.trim()===""?e.push("easing (not a non-empty string)"):Wl(s.easing)||e.push("easing (not an easing the browser can parse — `Element.animate()` would throw at play time, with the covering surface already over the window)"))}qs("WindowReveal",e,s);const t=s.id.trim().toLowerCase();Ks.set(t,{...s,id:t}),Ol()}function _n(){const s=Array.from(Ks.values()),e=J(F.WINDOW_REVEALS,s);return Array.isArray(e)?e:(typeof console<"u"&&console.warn("[openstation] `os.window-reveals` filter returned a non-array; falling back to registry list."),s)}function jl(s){return Ks.has(s)}function Vl(s){return Bs.add(s),()=>{Bs.delete(s)}}function Ol(){const s=Array.from(Bs);for(const e of s)try{e()}catch(t){typeof console<"u"&&console.error("[openstation] window-reveal registry listener threw:",t)}}W({id:"sweep",label:d("Sweep"),description:d("A straight edge travels across the window, uncovering the page behind it."),...zl(),duration:420}),W({id:"iris",label:d("Iris"),description:d("The page opens out from the centre of the window, like a camera shutter."),...$l(),duration:520}),W({id:"curtain",label:d("Curtain"),description:d("Two panels part from the middle of the window and slide off the sides."),...Al(),duration:480}),W({id:"blinds",label:d("Blinds"),description:d("Six horizontal slats retract at once, letting the page through between them."),...Cl(),duration:560}),W({id:"diagonal",label:d("Diagonal"),description:d("A slanted edge sweeps off the trailing corner of the window."),...Il(),duration:460}),W({id:"rise",label:d("Rise"),description:d("The page rises into the window from the bottom edge upward."),...Pl(),duration:440}),W({id:"shutter",label:d("Shutter"),description:d("Two panels part from the middle of the window and slide off the top and bottom."),...Sl(),duration:480}),W({id:"slats",label:d("Slats"),description:d("Eight vertical slats retract at once, letting the page through between them."),...El(),duration:560}),W({id:"diamond",label:d("Diamond"),description:d("The page opens out from the centre of the window through a growing rhombus."),...Ll(),duration:520}),W({id:"mosaic",label:d("Mosaic"),description:d("The page arrives as a grid of tiles, each opening out from its own centre."),...Rl(),duration:600}),W({id:"obturator",label:d("Camera shutter"),description:d("Six wedges slide aside in unison, opening a hexagonal aperture from the centre like the iris of a camera lens."),render:Ul,duration:720,edgeLag:0}),W({id:"radar",label:d("Radar"),description:d("A spoke sweeps a full turn around the centre of the window, uncovering the page behind it."),...Tl(),duration:760});const yn={dockSize:["compact","default","large"],desktopLayout:["classic","unified"],dockPlacement:["bottom","left","right"],windowRadius:["sharp","default","round"],adminBarMode:["static","dynamic","hidden"]},wn=["dockRailRenderer","windowReveal","accent"],xn={windowRevealDuration:{min:80,max:4e3}},Kl=/^[a-z0-9_-]+$/;[...Object.keys(yn),...wn,...Object.keys(xn)];function kn(s){if(!s||typeof s!="object"||Array.isArray(s))return{};const e=s,t={},o={};for(const[i,n]of Object.entries(yn)){const r=e[i];typeof r=="string"&&n.includes(r)&&(t[i]=r)}for(const i of wn){const n=e[i];typeof n=="string"&&Kl.test(n)&&(t[i]=n)}for(const[i,n]of Object.entries(xn)){const r=e[i];typeof r=="number"&&Number.isFinite(r)&&(o[i]=Math.min(n.max,Math.max(n.min,Math.round(r))))}return{...t,...o}}function Gs(s){const e=kn(s);return typeof e.dockRailRenderer=="string"&&bl(e.dockRailRenderer)===void 0&&delete e.dockRailRenderer,typeof e.windowReveal=="string"&&e.windowReveal!==_e&&!jl(e.windowReveal)&&delete e.windowReveal,typeof e.accent=="string"&&!Ms().some(t=>t.id===e.accent)&&delete e.accent,e}const Bl=/^[a-z0-9_-]+$/,$n=128;function Gl(s){return typeof s!="string"||s===""||s.length>2048?!1:s.startsWith("dashicons-")||s.startsWith("https://")||s.startsWith("http://")||s.startsWith("data:image/")}function An(s){if(!s||typeof s!="object")return null;const e=s,t=typeof e.slug=="string"?e.slug:"";if(!Bl.test(t))return null;const o={};if(e.icons&&typeof e.icons=="object"){let l=0;for(const[u,h]of Object.entries(e.icons)){if(l>=$n)break;u===""||!Gl(h)||(o[u]=h,l+=1)}}const i={};if(e.tokens&&typeof e.tokens=="object")for(const[l,u]of Object.entries(e.tokens))typeof u=="string"&&(i[l]=u);const n=[];if(Array.isArray(e.fonts))for(const l of e.fonts)typeof l=="string"&&l!==""&&n.push(l);const r={};if(e.iconColors&&typeof e.iconColors=="object"){let l=0;for(const[u,h]of Object.entries(e.iconColors)){if(l>=$n)break;u===""||typeof h!="string"||h===""||(r[u]=h,l+=1)}}const a=l=>typeof e[l]=="string"?e[l]:"";return{id:a("id")||t,slug:t,name:a("name")||t,version:a("version"),author:a("author"),description:a("description"),previewUrl:a("previewUrl"),cssUrl:a("cssUrl"),cssText:a("cssText"),tokens:i,fonts:n,icons:o,iconColors:r,recommendedOsSettings:kn(e.recommendedOsSettings),installedAt:typeof e.installedAt=="number"?e.installedAt:0,source:e.source==="code"?"code":"upload"}}function Xl(){const e=window.openStationConfig?.serverDesktopThemes,t=[];if(Array.isArray(e))for(const o of e){const i=An(o);i&&t.push(i)}return{themes:t,activeId:null,activeIcons:null,activeIconColors:null}}const X=K("desktop-mode/desktop-themes",Xl);function Cn(){return X}function Zl(){return X.getState().themes.slice()}function Sn(s){if(typeof s!="string"||s==="")return null;const e=s.replace(/\//g,"-");return X.getState().themes.find(t=>t.slug===e||t.id===s)??null}function Yl(s){const e=An(s);if(!e)return null;const t=X.state.themes.slice(),o=t.findIndex(i=>i.slug===e.slug);return o>=0?t[o]=e:t.push(e),t.sort((i,n)=>i.name.localeCompare(n.name)),X.setState({themes:t}),e}function Jl(s){const e=X.state.themes.filter(t=>t.slug!==s);e.length!==X.state.themes.length&&X.setState({themes:e})}function Ql(s){return X.subscribe(s)}async function En(s,e){try{const t=await s.json();if(t&&typeof t.message=="string"&&t.message!=="")return t.message}catch{}return`${e} (HTTP ${s.status}).`}function Ln(s){const e=s?.serverWallpapers;Array.isArray(e)&&re(F.WALLPAPERS_SERVER_CHANGED,{wallpapers:e})}async function ec(s,e){const t=s.desktopThemesUrl??"";if(t==="")throw new Error("[openstation] No desktopThemesUrl in the shell config — the desktop-themes REST route is unavailable.");const o=new FormData;o.append("file",e,e.name);const i=await O(t,{method:"POST",credentials:"same-origin",headers:{"X-WP-Nonce":s.restNonce},body:o},{source:"desktop-mode/desktop-themes/upload"});if(!i.ok)throw new Error(await En(i,"Theme upload failed"));const n=await i.json();return Ln(n),n}async function tc(s,e){const t=s.desktopThemesUrl??"";if(t==="")throw new Error("[openstation] No desktopThemesUrl in the shell config — the desktop-themes REST route is unavailable.");const o=await O(`${t}/${encodeURIComponent(e)}`,{method:"DELETE",credentials:"same-origin",headers:{"X-WP-Nonce":s.restNonce}},{source:"desktop-mode/desktop-themes/delete"});if(!o.ok)throw new Error(await En(o,"Theme delete failed"));try{Ln(await o.json())}catch{}}function sc(s){const e=Cn().state;if(e.activeIcons===null||!s)return null;const t=e.activeIcons[s];if(typeof t!="string"||t==="")return null;const o=J(F.DESKTOP_THEME_ICON,t,{slot:s,themeId:e.activeId??""});return typeof o=="string"&&o!==""?o:null}function oc(s){const e=Cn().state;if(e.activeIconColors===null||!s)return null;const t=e.activeIconColors[s];if(typeof t!="string"||t==="")return null;const o=J(F.DESKTOP_THEME_ICON_COLOR,t,{slot:s,themeId:e.activeId??""});return typeof o=="string"&&o!==""?ic(o):null}function ic(s){const e=s.trim();if(e===""||e.length>64||/[;{}<>"'\\]/.test(e))return null;let t=0;for(const o of e)if(o==="(")t+=1;else if(o===")"&&(t-=1,t<0))return null;return t===0?e:null}const ye={OS_SETTINGS:"OS_SETTINGS",RECYCLE_BIN:"RECYCLE_BIN",BUG_REPORT:"BUG_REPORT",EXIT_OPENSTATION:"EXIT_OPENSTATION",PWA_INSTALL:"PWA_INSTALL"},nc={"desktop-mode-os-settings":ye.OS_SETTINGS,"desktop-mode-recycle-bin":ye.RECYCLE_BIN,"desktop-mode-bug-report":ye.BUG_REPORT,"os-exit":ye.EXIT_OPENSTATION,"os-pwa-install":ye.PWA_INSTALL};function rc(s){if(typeof s!="string"||s==="")return"";const e=nc[s];if(e)return e;const t=s.toLowerCase().replace(/[^a-z0-9_-]/g,"");return t===""?"":`APP:${t}`}const Rn="",ac="system-default",Tn={accent:"pulse",desktopLayout:"unified",dockPlacement:"bottom"};function Pn(s,e,t={}){const o=e===Rn,i=o?null:Sn(e);if(!o&&!i)return{};const n=o?ac:i.slug,r=s.appliedThemeRecommendations.includes(n);if(r&&!t.force)return{};const a=Gs(o?Tn:i.recommendedOsSettings),l=Object.keys(a);if(l.length===0)return{};const u=s,h={};for(const p of l){const m=a[p];typeof m!="string"&&typeof m!="number"||!(p in s)||typeof u[p]!=typeof m||(u[p]=m,h[p]=m)}return Object.keys(h).length===0?{}:(r||(s.appliedThemeRecommendations=[...s.appliedThemeRecommendations,n].slice(-64)),h)}function lc(s){if(s===Rn)return Object.keys(Gs(Tn)).length>0;const e=Sn(s);return e?Object.keys(Gs(e.recommendedOsSettings)).length>0:!1}const Gt="",zn="OpenStation";function cc(s){const e=s.trim().split(/\s+/).filter(Boolean);return e.length===0?"?":e.length>=2?(e[0][0]+e[1][0]).toUpperCase():e[0].slice(0,2).toUpperCase()}function dc(s){const e=document.createElement("div");e.className="os-settings__themes";let t="",o=!1;const i=!!s.config.canManageDesktopThemes,n=g=>{s.state.desktopTheme!==g&&(s.state.desktopTheme=g,Pn(s.state,g),s.save(),s.apply(),w())},r=g=>{const f=Pn(s.state,g,{force:!0});Object.keys(f).length!==0&&(s.save(),s.apply(),w())},a=async g=>{if(!o){o=!0,t="",w();try{const f=await ec(s.config,g);Yl(f)}catch(f){t=f instanceof Error?f.message:d("That theme could not be installed.")}finally{o=!1,w()}}},l=async g=>{if(await Bi({title:d("Delete this theme?"),message:j(d("“%s” will be removed from this site for everyone. This cannot be undone."),g.name),confirmLabel:d("Delete"),danger:!0})){t="";try{await tc(s.config,g.slug),Jl(g.slug),s.state.desktopTheme===g.slug&&(s.state.desktopTheme=Gt,s.save(),s.apply())}catch(y){t=y instanceof Error?y.message:d("That theme could not be deleted.")}w()}},u=g=>{const f=g.currentTarget,y=f.files?.[0];y&&a(y),f.value=""},h=g=>{g.preventDefault(),g.currentTarget.classList.remove("os-settings__theme-upload--dragover");const f=g.dataTransfer?.files?.[0];f&&a(f)},p=g=>{const f=s.state.desktopTheme===g.slug;return c`<div class="os-settings__theme-card-wrap">
			<button
				type="button"
			...
			>
				...
				<span class="os-settings__theme-meta">
					${g.version!==""?j(d("Version %s"),g.version):""}
				</span>
			</button>
			${i&&g.source!=="code"?c`<button
						type="button"
						class="os-settings__theme-delete"
						aria-label=${j(d("Delete %s"),g.name)}
						@click=${()=>void l(g)}
					>
						×
					</button>`:""}
		</div>`},m=()=>{const g=s.state.desktopTheme===Gt;return c`<div class="os-settings__theme-card-wrap">
			<button
				type="button"
	...
	>
		<label class="os-settings__theme-upload-label">
	...
	</div>`,b=g=>{const f=s.state.desktopTheme;if(!lc(f))return"";const y=f===Gt?zn:g.find(A=>A.slug===f)?.name;return y===void 0?"":c`<div class="os-settings__theme-recommendation">
			<os-button
				variant="secondary"
				@click=${()=>r(f)}
				>${j(d("Apply %s’s recommended layout and effects"),y)}</os-button
			>
		</div>`};function w(){const g=Zl();L(c`
				${t!==""?c`<os-notice tone="error">${t}</os-notice>`:""}
			...
			`,e)}w();const v=Ql(()=>{if(!e.isConnected){v();return}w()});return e}function In(s){if(!s)return 214;let e=5381;for(let t=0;t<s.length;t++)e=Math.imul(e,33)+s.charCodeAt(t);return(e%360+360)%360}function uc(s){return typeof s!="string"||s===""||s.length>4096||!/^(https?:\/\/|data:image\/)/i.test(s)?!1:!/['"()\\<>\s]/.test(s)}function Mn(s,e,t){if(!uc(e))return!1;const o=`url("${e}") center / contain no-repeat`;return s.style.backgroundImage="none",s.style.backgroundColor=t,s.style.setProperty("-webkit-mask",o),s.style.setProperty("mask",o),!0}const Dn="data:image/svg+xml;base64,",Xt=new Map;function hc(s){const e=Xt.get(s);if(e!==void 0)return e;let t=!1;try{t=atob(s.slice(Dn.length)).includes("currentColor")}catch{}return Xt.size>=256&&Xt.clear(),Xt.set(s,t),t}function Nn(s,e){const t=e.className??"",o=e.title??"";let i=null;if(e.slot){const l=sc(e.slot);l!==null&&(s=l),i=oc(e.slot)}if(typeof s=="string"&&s.startsWith("dashicons-")){const l=document.createElement("span");return l.className=`dashicons ${s} ${t}`.trim(),l.setAttribute("aria-hidden","true"),i!==null&&(l.style.color=i),l}if(i!==null&&typeof s=="string"){const l=document.createElement("span");if(l.className=t,l.setAttribute("aria-hidden","true"),l.style.display="inline-block",Mn(l,s,i))return l}if(typeof s=="string"&&s.startsWith("data:image/svg+xml;base64,")){const l=s.slice(Dn.length);if(/^[A-Za-z0-9+/=]+$/.test(l)){const u=document.createElement("span");return u.className=t,u.setAttribute("aria-hidden","true"),u.style.display="inline-block",hc(s)&&Mn(u,s,"currentColor")||(u.style.backgroundImage=`url("${s}")`,u.style.backgroundRepeat="no-repeat",u.style.backgroundPosition="center",u.style.backgroundSize="contain"),u}}if(typeof s=="string"&&/^data:image\/(png|jpeg|jpg|gif|webp|x-icon|vnd\.microsoft\.icon);base64,/i.test(s)){const l=s.indexOf(","),u=l>=0?s.slice(l+1):"";if(/^[A-Za-z0-9+/=]+$/.test(u))return Hn(s,t)}if(typeof s=="string"&&(s.startsWith("http://")||s.startsWith("https://")))return Hn(s,t);const n=document.createElement("span");n.className=`${t} os-icon-letter`.trim(),n.setAttribute("aria-hidden","true");const r=pc(o);n.textContent=r;const a=In(o);return n.style.backgroundColor=`hsl( ${a}, 60%, 45% )`,n.style.color="#fff",n.style.display="inline-flex",n.style.alignItems="center",n.style.justifyContent="center",n.style.fontWeight="600",n.style.borderRadius="4px",n}function Hn(s,e){const t=document.createElement("img");return t.className=e,t.src=s,t.alt="",t.setAttribute("aria-hidden","true"),t.draggable=!1,t}function pc(s){const e=(s??"").trim();if(e==="")return"?";const t=e.split(/\s+/);if(t.length>=2)return(t[0][0]+t[1][0]).toUpperCase();const o=t[0];return o.length>=2?o.slice(0,2).toUpperCase():o.toUpperCase()}const fc={core:"rail",plugin:"rail",app:"desktop",control:"rail"};function gc(s,e){return s==="core"&&e==="classic"?"sidebar":"dock"}function bc(s,e){return s.locked?"rail":e[s.id]??s.defaultPlacement??fc[s.kind]}function mc(s){return s.map((e,t)=>({item:e,index:t})).sort((e,t)=>(e.item.order??0)-(t.item.order??0)||e.index-t.index).map(e=>e.item)}const vc=[{kinds:["core"],heading:d("WordPress Core"),description:d("The admin menus WordPress itself registers.")},{kinds:["plugin","app"],heading:d("Plugins & Apps"),description:d("Menus from installed plugins and apps.")},{kinds:["control"],heading:d("OpenStation"),description:d("The OpenStation’s own controls.")}];function _c(){const s=window.wp?.os;return typeof s?.getNavItems=="function"?s.getNavItems():[]}function yc(s){return s.locked||s.transient?!1:s.menu||s.entry?!0:s.tile?.placeable===!0}function wc(s){const e=document.createElement("div"),t=(u,h)=>{s.state.navPlacement={...s.state.navPlacement,[u]:h},s.save(),r()},o=u=>h=>{const m=h.detail?.value;(m==="both"||m==="rail"||m==="desktop"||m==="hidden")&&t(u,m)},i=u=>d(u==="classic"?"Choose where each menu shows up: on the dock/sidebar, on the desktop wallpaper, both, or hidden entirely. Changes apply instantly.":"Choose where each menu shows up: on the dock, on the desktop wallpaper, both, or hidden entirely. Changes apply instantly."),n=(u,h)=>[{id:"rail",label:gc(u,h)==="sidebar"?d("In the sidebar"):d("On the dock")},{id:"desktop",label:d("On the desktop")},{id:"both",label:d("On both")},{id:"hidden",label:d("Hidden")}],r=(u=s.state.navPlacement,h=s.state.desktopLayout)=>{const p=_c().filter(yc),m=vc.map(_=>({..._,rows:mc(p.filter(b=>_.kinds.includes(b.kind)))})).filter(_=>_.rows.length>0);L(c`
				<p class="os-settings__page-description os-nav-settings__lead">
					${i(h)}
				</p>
				${m.length===0?c`<os-empty-state
							heading=${d("Nothing registered yet")}
							description=${d("Plugins and the admin menu will appear here once they’re registered.")}
						></os-empty-state>`:m.map(_=>c`
								<os-section
									class="os-nav-settings"
									heading=${_.heading}
									description=${_.description}
								>
									${_.rows.map(b=>a(b,u,h))}
								</os-section>
							`)}
			`,e)},a=(u,h,p)=>c`<div class="os-nav-settings__row" data-item-id=${u.id}>
		<div class="os-nav-settings__identity">
			${Nn(u.icon,{title:u.title,className:"os-nav-settings__icon",slot:rc(u.id)})}
		...
		<os-select
			...
			@os-pick=${o(u.id)}
		>
			${n(u.kind,p).map(m=>c`<os-option value=${m.id}>${m.label}</os-option>`)}
		</os-select>
	</div>`;r();const l=window.wp?.os;if(l?.subscribeOsSettings){const u=l.subscribeOsSettings(h=>{if(!e.isConnected){u();return}r(h.navPlacement,h.desktopLayout)})}return e}const xc={unified:[{kind:"win",style:"left:12%;top:14%;width:44%;height:42%"},{kind:"win",style:"left:30%;top:30%;width:44%;height:42%"},{kind:"bar",accent:!0,style:"left:24%;bottom:10%;width:52%;height:9%"}],classic:[{kind:"bar",accent:!0,style:"left:0;top:0;bottom:0;width:5%"},{kind:"win",style:"left:14%;top:14%;width:44%;height:42%"},{kind:"win",style:"left:32%;top:30%;width:44%;height:42%"},{kind:"bar",accent:!0,style:"left:33%;bottom:10%;width:38%;height:9%"}]};function kc(s){const e=l=>{Di.some(u=>u.id===l)&&(s.state.desktopLayout=l,s.save(),s.apply(),a())},t=l=>{const u=l.detail?.value??"";Mi.some(h=>h.id===u)&&(s.state.dockPlacement=u,s.save(),s.apply(),a())},o=l=>{const u=l.detail?.value??"";Pi.some(h=>h.id===u)&&(s.state.dockSize=u,s.save(),s.apply(),a())},i=()=>c`<div class="os-settings__dock-options">
					<div class="os-settings__dock-option">
						...
						<os-segmented
							...
							@os-pick=${t}
						>
							${Mi.map(l=>c`<os-segment value=${l.id}
										>${al(l.id,l.label)}</os-segment
									>`)}
						</os-segmented>
					</div>
			  </div>`,n=()=>c`<div
				...
				>
					<div class="os-settings__dock-option">
						...
						<os-segmented
							...
							@os-pick=${o}
						>
							${Pi.map(l=>c`<os-segment value=${l.id}
										>${il(l.id,l.label)}</os-segment
									>`)}
						</os-segmented>
					</div>
			  </div>`,r=document.createElement("div"),a=()=>L(c`
				<os-section
				...
				>
					...
					>
						${Di.map(l=>{const u=s.state.desktopLayout===l.id;return c`<div class="os-settings__layout-card">
								<button
									type="button"
								...
								>
									<span
										...
										aria-hidden="true"
									>
										${(xc[l.id]??[]).map(h=>c`<span
												class="os-settings__layout-${h.kind}${h.accent?" is-accent":""}"
												style=${h.style}
											></span>`)}
									</span>
									<span class="os-settings__layout-name"
										>${ll(l.id,l.label)}</span
									>
			...
			`,r);return a(),r}function $c(s){const e=i=>{const n=i.detail?.value??"";zi.some(r=>r.id===n)&&(s.state.windowRadius=n,s.save(),s.apply(),o())},t=document.createElement("div"),o=()=>L(c`
				<os-section
				...
				>
					<os-segmented
						...
						@os-pick=${e}
					>
						${zi.map(i=>c`<os-segment value=${i.id}
								>${nl(i.id,i.label)}</os-segment
							>`)}
					</os-segmented>
				</os-section>
			`,t);return o(),t}const Ac="os.activity.";function Xs(s){return Ac+String(s).replace(/[^a-zA-Z0-9_.-]/g,".")}let Cc=0;const qn={publish(s,e){re(Xs(s),e)},subscribe(s,e){const t=`os/activity-sub/${++Cc}`,o=Xs(s);vl(o,t,n=>e(n));let i=!1;return()=>{i||(i=!0,_l(o,t))}},filter(s,e,...t){return J(Xs(s),e,...t)}};function Sc(s){s.style.setProperty("--os-file-tile-enter-delay",`${(Math.random()*.25).toFixed(3)}s`),s.style.setProperty("--os-file-tile-enter-duration",`${(.3+Math.random()*.25).toFixed(3)}s`)}const Zs=new Map;function Ec(s){let e,t;try{const i=new URL(s,document.baseURI);e=i.origin,t=i.pathname}catch{return null}if(!t)return null;const o=document.querySelectorAll("script[src]");for(const i of Array.from(o))try{const n=new URL(i.src,document.baseURI);if(n.origin===e&&n.pathname===t)return i}catch{}return null}function Lc(s,e){const t=Zs.get(s);if(t)return t;const o=new Promise((i,n)=>{const r=`script[data-os-vendor="${Rc(s)}"]`,a=document.querySelector(r);if(a){if(a.dataset.loaded==="1"){i();return}a.addEventListener("load",()=>i(),{once:!0}),a.addEventListener("error",()=>n(new Error(`Failed to load ${s}`)),{once:!0});return}const l=Ec(s);if(l){l.dataset.osVendor=s,l.dataset.loaded="1",i();return}e?.translations&&Zt(e.translations);for(const h of e?.l10n??[])Zt(h);for(const h of e?.before??[])Zt(h);const u=document.createElement("script");u.src=s,u.async=!0,u.dataset.osVendor=s,u.addEventListener("load",()=>{u.dataset.loaded="1";for(const h of e?.after??[])Zt(h);i()},{once:!0}),u.addEventListener("error",()=>{Zs.delete(s),u.remove(),n(new Error(`Failed to load ${s}`))},{once:!0}),document.head.appendChild(u)});return Zs.set(s,o),o}function Zt(s){if(!s)return;const e=document.createElement("script");e.textContent=s,e.dataset.osVendorInline="1",document.head.appendChild(e)}function Rc(s){return typeof CSS<"u"&&typeof CSS.escape=="function"?CSS.escape(s):s.replace(/["\\]/g,"\\$&")}K("desktop-mode/native-url-remap",()=>({remaps:[],deps:null}));function Tc(s){const e=document.createElement("div"),t=a=>{const l=a.detail?.value??"";l!==""&&(s.state.dockRailRenderer=l,s.save(),s.apply(),i())};let o=Yi();const i=()=>{if(o.length<=1){L(c``,e);return}L(c`
				<os-section
				...
				>
					<os-segmented
						...
						@os-pick=${t}
					>
						${o.map(a=>c`<os-segment value=${a.id}
									>${a.label}</os-segment
								>`)}
					</os-segmented>
				</os-section>
			`,e)},n=ml(()=>{o=Yi(),i()}),r=new MutationObserver(()=>{e.isConnected||(n(),r.disconnect())});return queueMicrotask(()=>{e.parentNode&&r.observe(e.parentNode,{childList:!0,subtree:!1})}),i(),e}const Yt="none",Un=K("desktop-mode/unfocus-effect-registry",()=>({registry:new Map,listeners:new Set})),Fn=Un.state.registry,Ys=Un.state.listeners,Wn=/^[a-z0-9_/-]+$/;function Js(s){const e=[];!s||typeof s!="object"?e.push("def (not an object)"):(typeof s.id!="string"||s.id.trim()===""?e.push("id (missing)"):Wn.test(s.id.trim().toLowerCase())?s.id.trim().toLowerCase()===Yt&&e.push('id ("none" is reserved)'):e.push(`id (must match ${Wn} — lowercase alphanum, hyphens, underscores, slashes for vendor/sub-id)`),(typeof s.label!="string"||s.label.trim()==="")&&e.push("label (missing)"),typeof s.className!="string"&&typeof s.apply!="function"&&e.push("className|apply (at least one must be provided — a CSS class to toggle or an apply callback)")),qs("UnfocusEffect",e,s);const t=s.id.trim().toLowerCase();Fn.set(t,{...s,id:t}),zc()}function jn(){const s=Array.from(Fn.values()),e=J(F.UNFOCUS_EFFECTS,s);return Array.isArray(e)?e:(typeof console<"u"&&console.warn("[openstation] `os.unfocus-effects` filter returned a non-array; falling back to registry list."),s)}function Pc(s){return Ys.add(s),()=>{Ys.delete(s)}}function zc(){const s=Array.from(Ys);for(const e of s)try{e()}catch(t){typeof console<"u"&&console.error("[openstation] unfocus-effect registry listener threw:",t)}}Js({id:"darken",label:d("Darken"),description:d("Dim unfocused windows so the focused one stands out."),className:"os-window--fx-darken"}),Js({id:"frost",label:d("Frost"),description:d("Throw unfocused windows out of focus — a soft, frosted-glass blur, as if you were looking at them through an iced-over pane."),className:"os-window--fx-frost"}),Js({id:"grayscale",label:d("Grayscale"),description:d("Drain the colour from unfocused windows so the focused one is the only thing still in colour — your eye snaps right to it."),className:"os-window--fx-grayscale"});const Qs="none",Vn=K("desktop-mode/window-link-renderer-registry",()=>({registry:new Map,listeners:new Set})),Ic=Vn.state.registry,On=Vn.state.listeners;function Kn(){const s=Array.from(Ic.values()),e=J(F.WINDOW_LINK_RENDERERS,s);return Array.isArray(e)?e:(typeof console<"u"&&console.warn("[openstation] `os.window-links.renderers` filter returned a non-array; falling back to registry list."),s)}function Mc(s){return On.add(s),()=>{On.delete(s)}}const Bn=[{value:Fl,label:()=>d("Default (per reveal)")},{value:200,label:()=>d("Very fast — 200 ms")},{value:320,label:()=>d("Fast — 320 ms")},{value:460,label:()=>d("Normal — 460 ms")},{value:700,label:()=>d("Slow — 700 ms")},{value:1100,label:()=>d("Very slow — 1100 ms")}],Dc=[{id:"focus",label:()=>d("When a related window is focused")},{id:"always",label:()=>d("Always")},{id:"off",label:()=>d("Off")}];function Nc(s){const e=document.createElement("div"),t=w=>{const v=w.detail?.value??"";v!==""&&(v!==Yt&&!a.some(g=>g.id===v)||(s.state.unfocusEffect=v,s.save(),s.apply(),h()))},o=w=>{const v=w.detail?.value??"";v!==""&&(v!==_e&&!l.some(g=>g.id===v)||(s.state.windowReveal=v,s.save(),h()))},i=w=>{const v=w.detail?.value??"";if(v==="")return;const g=Number(v);Bn.some(f=>f.value===g)&&(s.state.windowRevealDuration=g,s.save(),h())},n=w=>{const v=w.detail?.value??"";v!==""&&(v!==Qs&&!u.some(g=>g.id===v)||(s.state.windowLinkRenderer=v,s.save(),s.apply(),h()))},r=w=>{const v=w.detail?.value??"";v!=="focus"&&v!=="always"&&v!=="off"||(s.state.windowLinkVisibility=v,s.save(),s.apply(),h())};let a=jn(),l=_n(),u=Kn();const h=()=>{const w=a.find(E=>E.id===s.state.unfocusEffect),v=d("Apply a visual treatment to every window except the one you are working in."),g=s.state.unfocusEffect!==Yt&&w?.description?w.description:v,f=l.find(E=>E.id===s.state.windowReveal),y=d("Uncover a window’s content when it finishes loading, instead of fading it in."),A=s.state.windowReveal!==_e&&f?.description?f.description:y,M=u.find(E=>E.id===s.state.windowLinkRenderer),oe=d("Draw a visual tie between windows showing related content — a post and its comments or media."),N=s.state.windowLinkRenderer!==Qs&&M?.description?M.description:oe;L(c`
				<os-section
				...
				>
					...
					>
						...
						${a.map(E=>c`<os-option value=${E.id}
									>${E.label}</os-option
								>`)}
					</os-select>
				</os-section>
				...
				>
					...
					>
						...
						${l.map(E=>c`<os-option value=${E.id}
									>${E.label}</os-option
								>`)}
					</os-select>
					<os-select
						...
						@os-pick=${i}
					>
						${Bn.map(E=>c`<os-option value=${String(E.value)}
									>${E.label()}</os-option
								>`)}
					</os-select>
				</os-section>
				...
				>
					...
					>
						...
						${u.map(E=>c`<os-option value=${E.id}
									>${E.label}</os-option
								>`)}
					</os-select>
					<os-select
			...
			`,e)},p=Pc(()=>{a=jn(),h()}),m=Vl(()=>{l=_n(),h()}),_=Mc(()=>{u=Kn(),h()}),b=new MutationObserver(()=>{e.isConnected||(p(),m(),_(),b.disconnect())});return queueMicrotask(()=>{e.parentNode&&b.observe(e.parentNode,{childList:!0,subtree:!1})}),h(),e}function Hc(s){const{extendedOptions:e,extendedOptionsUrl:t,restNonce:o}=s.config,i={media_library_enhanced:e?.media_library_enhanced===!0,games:e?.games===!0,agents:e?.agents===!0,saving:!1,error:""},n=document.createElement("div");let r=!1;const a=async()=>{if(!(!t||!o)){if(i.saving){r=!0;return}i.saving=!0,i.error="",p();try{const m=await O(t,{method:"POST",headers:{"Content-Type":"application/json","X-WP-Nonce":o},body:JSON.stringify({options:{media_library_enhanced:i.media_library_enhanced,games:i.games,agents:i.agents}})},{source:"desktop-mode/settings/extended"});if(m.ok){const _=await m.json().catch(()=>null);_&&typeof _=="object"&&(s.config.extendedOptions=_)}else{const _=await m.json().catch(()=>({}));i.error=_.message??`Error ${m.status}`}}catch{i.error=d("Network error — check your connection.")}finally{i.saving=!1,r?(r=!1,await a()):p()}}},l=m=>{i.media_library_enhanced=m.detail?.checked===!0,a()},u=m=>{i.games=m.detail?.checked===!0,a()},h=m=>{i.agents=m.detail?.checked===!0,a()},p=()=>L(c`
				...
				>
					...
					${i.error?c`<p class="os-ext__error">${i.error}</p>`:c``}
					${i.saving?c`<p class="os-ext__saving">${d("Saving…")}</p>`:c``}
				</os-section>
			`,n);return p(),n}let eo=null;function Jt(){return!!window.openStationShellOverlays}function qc(s){return new Promise((e,t)=>{const o=document.querySelector('script[data-os-shell-overlays="1"]'),i=()=>{if(Jt()){e();return}t(new Error("[openstation] shell-overlays bundle loaded but did not set `window.openStationShellOverlays`."))};if(o){Jt()?i():(o.addEventListener("load",i),o.addEventListener("error",()=>t(new Error("failed to load shell-overlays bundle"))));return}const n=document.createElement("script");n.src=s,n.async=!0,n.dataset.osShellOverlays="1",n.addEventListener("load",i),n.addEventListener("error",()=>t(new Error("failed to load shell-overlays bundle"))),document.head.appendChild(n)})}function Uc(s){return Jt()||!s?Promise.resolve():(eo||(eo=qc(s)),eo)}function Fc(){return window.openStationConfig?.shellOverlaysBundleUrl??""}function Wc(s,e){const t=Fc();if(Jt()||!t){e();return}Uc(t).then(()=>{s()&&e()}).catch(o=>{typeof console<"u"&&console.warn("[openstation] shell-overlays failed to load; menu/dialog suppressed:",o)})}const jc=4e3,Vc=200,Oc=1200;function to(s){const e=qn.filter("os/toast-requested",{...s});if(!e||e.cancel===!0)return()=>{};let t=!1,o=null;return Wc(()=>!t,()=>{o=Kc(e)}),()=>{t=!0,o&&o()}}function Kc(s){const e=Zc(),t=document.createElement("os-toast");t.textContent=s.message,s.action&&(t.setAttribute("action",s.action.label),t.addEventListener("os-toast-action",()=>{s.action?.onClick(),l()})),s.dismissible&&(t.setAttribute("dismissible",""),t.addEventListener("os-toast-dismiss",()=>{s.onDismiss?.(),l()})),e.appendChild(t);let o=!1,i=null,n=s.duration??jc,r=0;const a=()=>{i!==null&&(window.clearTimeout(i),i=null)},l=()=>{o||(o=!0,a(),Xc(t),t.setAttribute("state","out"),window.setTimeout(()=>{t.remove(),Gc()},Vc))},u=()=>{o||s.persistent||i!==null||(r=Date.now(),i=window.setTimeout(l,n))},h=()=>{i!==null&&(n=Math.max(0,n-(Date.now()-r)),a())};return requestAnimationFrame(()=>{t.setAttribute("state","in")}),t.addEventListener("os-toast-hold",p=>{if(p.detail?.held){h();return}n=Math.max(n,Oc),u()}),Bc(t),u(),qn.publish("os/toast-shown",{...s}),l}let Qt=null,we=0;function Gn(s){const e=s.target;e instanceof HTMLElement&&(e.closest("os-toast-container")||(Qt=e))}function Bc(s){we===0&&document.addEventListener("focusin",Gn,!0),we+=1;const e=s.ownerDocument,t=e.activeElement;t instanceof HTMLElement&&t!==e.body&&!t.closest("os-toast-container")&&(Qt=t)}function Gc(){we=Math.max(0,we-1),we===0&&document.removeEventListener("focusin",Gn,!0)}function Xc(s){const e=s.ownerDocument.activeElement;e!==s&&!s.contains(e)||Qt?.isConnected===!0&&Qt.focus()}function Zc(){const s=document.querySelector("os-toast-container");if(s)return s;const e=document.createElement("os-toast-container");return document.body.appendChild(e),e}let so=!1;function Yc(){so=!0}document.addEventListener("os-settings-save-lifecycle",s=>{if(!so)return;const e=s.detail?.phase;if(e!=="saved"&&e!=="failed"||(so=!1,e!=="saved"))return;const t=window.wp?.os?.refreshMenu;typeof t=="function"&&t()});const Jc=typeof navigator<"u"&&/Mac|iPhone|iPad|iPod/i.test(navigator.platform||navigator.userAgent||"")?"⌘K":"Ctrl+K";function Qc(s){const e=document.createElement("div"),t=C=>{const S=C.detail?.checked===!0;s.state.nativePostsEnabled=S,s.save(),T()},o=C=>{const S=C.detail?.value,I=Number(S);if([15,30,45,60].includes(I)){s.state.heartbeatRate=I,s.save();try{const H=window.wp,ie=I>=60?"slow":"standard";H?.heartbeat?.interval?.(ie)}catch{}T()}},i=C=>{const S=C.detail?.checked===!0;s.state.nativePagesEnabled=S,s.save(),T()},n=C=>{const S=C.detail?.checked===!0;s.state.nativeUsersEnabled=S,s.save(),T()},r=C=>{const S=C.detail?.checked===!0;s.state.nativePluginsEnabled=S,s.save(),T()},a=C=>{const S=C.detail?.checked===!0;s.state.nativeCommentsEnabled=S,s.save(),T()},l=C=>{const S=C.detail?.checked===!0;s.state.stationHomeEnabled=S,s.save(),T()},u=C=>{const S=C.detail?.checked===!0;s.state.showDesktopOnWallpaperClick=S,s.save(),T()},h=C=>{const S=C.detail?.checked===!0;s.state.windowLinksEnabled=S,s.save(),s.apply(),T()},p=C=>{const S=C.detail?.checked===!0;s.state.windowLinkRaiseOnFocus=S,s.save(),s.apply(),T()},m=C=>{const S=C.detail?.checked===!0;s.state.windowLinkHighlight=S,s.save(),s.apply(),T()},_=C=>{const S=C.detail?.checked===!0;s.state.showPostStatusRibbons=S,s.save(),T()},b=C=>{const S=C.detail?.checked===!0;s.state.developerModeEnabled=S,s.save(),T(),Yc()},w=C=>{const S=C.detail?.checked===!0;s.state.foldersSharingEnabled=S,s.save(),T()};let v=!1;const g=async()=>{if(v||!await Bi({title:d("Delete folder sharing data?"),message:d("This drops every shares table on the site (current + legacy). All invites, accept/deny decisions, and share rows are permanently removed. Recipients lose their access until someone shares with them again. The empty tables are recreated on the next admin load so the feature keeps working — but every existing share is gone."),confirmLabel:d("Delete data"),danger:!0}))return;const S=f?.filesUrl,I=f?.restNonce;if(!S||!I){to({message:d("Files REST endpoint is not available.")});return}v=!0,T();try{const H=S.replace(/\/+$/,"")+"/folder-sharing-tables/purge",ie=await O(H,{method:"POST",headers:{"X-WP-Nonce":I},credentials:"same-origin"},{source:"os-settings/folder-sharing-purge"});if(!ie.ok){const Sh=await ie.text();throw new Error(`${ie.status}: ${Sh.slice(0,200)}`)}const Ch=await ie.json();to({message:d("Folder sharing data deleted.")+" ("+Ch.dropped.length+" tables)"})}catch(H){const ie=H instanceof Error?H.message:String(H);to({message:d("Could not delete sharing data.")+" "+ie})}finally{v=!1,T()}},f=window.openStationConfig,y={enabled:f?.commentsAi?.enabled??!1,providerConfigured:f?.commentsAi?.providerConfigured??!1,saving:!1},A=async C=>{const S=C.detail?.checked===!0;if(!(!f?.commentsAiUrl||y.saving)){y.saving=!0,y.enabled=S,T();try{const I=await O(f.commentsAiUrl,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json","X-WP-Nonce":f.restNonce??""},body:JSON.stringify({enabled:S})},{source:"os-settings/comments-ai"});if(I.ok){const H=await I.json();y.enabled=H.enabled,y.providerConfigured=H.providerConfigured,f.commentsAi&&(f.commentsAi.enabled=H.enabled,f.commentsAi.providerConfigured=H.providerConfigured)}else y.enabled=!S}catch{y.enabled=!S}y.saving=!1,T()}},M=C=>{const S=C.detail?.checked===!0;s.state.ai={...s.state.ai,enabled:S},s.save(),T()},oe=async()=>{const C=f?.aiAssistant;if(!(!C||!f?.aiStatusUrl))try{const S=await O(f.aiStatusUrl,{credentials:"same-origin",headers:{"X-WP-Nonce":f.restNonce??""}},{source:"os-settings/ai-status",silent:!0});if(!S.ok)return;const I=await S.json(),H=C.available!==(I.available===!0)||C.assistantProviderConfigured!==(I.assistantProviderConfigured===!0);C.available=I.available===!0,C.providerConfigured=I.providerConfigured===!0,C.assistantProviderConfigured=I.assistantProviderConfigured===!0,y.providerConfigured=C.providerConfigured,T(),H&&document.dispatchEvent(new CustomEvent("os-ai-status-changed"))}catch{}};let N=!1;const E=C=>{C.detail?.windowId==="desktop-mode-os-settings"&&(N||(N=!0,oe().finally(()=>{N=!1})))};document.addEventListener("os-window-focused",E);const Z=new MutationObserver(()=>{e.isConnected||(document.removeEventListener("os-window-focused",E),Z.disconnect())});Z.observe(document.body,{childList:!0,subtree:!0});const zr=()=>{const C=f?.aiAssistant?.connectorsUrl??"";if(!C)return;const S=window.wp?.os;if(S?.windowManager?.open){const I=S.deriveWindowId?S.deriveWindowId(C):C;S.windowManager.open({id:I,url:C,title:d("Connectors"),icon:"dashicons-admin-settings"})}else window.open(C,"_blank","noopener")};let Dt=!1;const Ah=async()=>{if(Dt)return;const C=window.openStationConfig;if(C?.seenIntrosUrl){Dt=!0,T();try{await O(C.seenIntrosUrl,{method:"DELETE",credentials:"same-origin",headers:{"X-WP-Nonce":C.restNonce??""}},{source:"os-settings/reset-intros"}),document.dispatchEvent(new CustomEvent("os-intros-reset"))}catch{}Dt=!1,T()}},T=()=>L(c`
				<!--
				...
				>
					${f?.aiAssistant?.available?c`
								<div class="os-features__item">
									<os-checkbox-label
										label=${d("AI assistant")}
										?checked=${s.state.ai.enabled}
										?disabled=${!f.aiAssistant.assistantProviderConfigured}
										@os-checkbox-change=${M}
									></os-checkbox-label>
									<p class="os-features__hint">
										${j(d("Adds an AI mode to the %s command palette: find content and ask about your site."),Jc)}
									</p>
									${f.aiAssistant.assistantProviderConfigured?"":c`
												<os-notice tone="warning" not-dismissible>
													${d("This feature requires an AI provider configured in")}
													<a
														href=${f.aiAssistant.connectorsUrl}
														@click=${C=>{C.preventDefault(),zr()}}
						...
						${f?.currentUserIsAdmin?c`
								<div class="os-features__danger-row">
									...
									<os-button
										...
										@click=${g}
									>
										${d(v?"Deleting…":"Delete folder sharing data")}
									</os-button>
								</div>
							`:""}
					</div>
					...
					<div class="os-features__row">
						...
						<os-button
							...
							@click=${Ah}
						>
							${d(Dt?"Resetting…":"Reset what’s-new dialogs")}
						</os-button>
					</div>
				</os-section>
				<os-section
...
`;function td(){const s="os-multiselect-popover-styles";if(document.getElementById(s))return;const e=document.createElement("style");e.id=s,e.textContent=`
...
`,document.head.appendChild(e)}typeof document<"u"&&td();const Ke=class Ke extends k{constructor(){super(...arguments),this._optionObserver=null,this._popover=null,this._teardownOpen=null,this._hasMore=!1,this._loadingMore=!1}set items(e){const t=this.querySelectorAll(":scope > os-option");for(const n of Array.from(t))n.remove();for(const n of e){const r=document.createElement("os-option");r.setAttribute("value",n.value),r.textContent=n.label,this.appendChild(r)}this._loadingMore=!1;const o=new Set(e.map(n=>n.value)),i=this._readValues().filter(n=>o.has(n));this._writeValueAttribute(i),this.requestUpdate(),this._refreshPopover()}get values(){return this._readValues()}set values(e){const t=Array.isArray(e)?e.map(o=>String(o)).filter(o=>o!==""):[];this._writeValueAttribute(t),this.requestUpdate(),this._refreshPopover()}get hasMore(){return this._hasMore}set hasMore(e){this._hasMore=!!e,this._refreshPopover()}get loadingMore(){return this._loadingMore}set loadingMore(e){this._loadingMore=!!e,this._refreshPopover()}appendItems(e){if(this._loadingMore=!1,!e||e.length===0){this._refreshPopover();return}const t=new Set(Array.from(this.querySelectorAll(":scope > os-option")).map(o=>o.getAttribute("value")));for(const o of e){if(t.has(o.value))continue;const i=document.createElement("os-option");i.setAttribute("value",o.value),i.textContent=o.label,this.appendChild(i)}this.requestUpdate(),this._refreshPopover()}connectedCallback(){super.connectedCallback(),ge(this),this._optionObserver=new MutationObserver(()=>{this.requestUpdate(),this._refreshPopover()}),this._optionObserver.observe(this,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["value","disabled"],characterData:!0})}disconnectedCallback(){this._optionObserver?.disconnect(),this._optionObserver=null,this._closePopover()}render(){const e=this.label||"",t=this.placeholder||"All",o=this.disabled!==null;e?this.setAttribute("aria-label",e):this.removeAttribute("aria-label");const i=e||t,n=this._summarize(t),r=this._readValues().length>0,l=`${this.id||"os-unnamed"}__trigger`;return c`
		...
		`}_renderGlyph(e){return e==="saved"?au():e==="failed"?lu():""}_labelForPhase(e){switch(e){case"pending":case"saving":return this["saving-label"]??"Saving…";case"saved":return this["saved-label"]??"Saved";case"failed":return(this.error??"")||"Couldn’t save";default:return this["idle-label"]??""}}_installAutoListener(){const e=this.event||cr;this._docListener=t=>{const o=t.detail;!o||typeof o.phase!="string"||(this.phase=o.phase,o.error?this.error=o.error:o.phase!=="failed"&&this.error&&this.removeAttribute("error"))},document.addEventListener(e,this._docListener)}_removeAutoListener(){if(!this._docListener)return;const e=this.event||cr;document.removeEventListener(e,this._docListener),this._docListener=null}_scheduleAutoClear(){this._autoTimer!==null&&(window.clearTimeout(this._autoTimer),this._autoTimer=null);const e=this.phase??"idle",t=this._autoClearMsFor(e);t<=0||(this._autoTimer=window.setTimeout(()=>{this._autoTimer=null,this.phase="idle"},t))}_autoClearMsFor(e){if(e==="saved"){const t=this["auto-clear-saved-ms"];return parseInt(t||"",10)||nu}if(e==="failed"){const t=this["auto-clear-failed-ms"];return parseInt(t||"",10)||ru}return 0}};St.props=["phase","mode","variant","animation","auto","event","error","saving-label","saved-label","idle-label","auto-clear-saved-ms","auto-clear-failed-ms"],St.styles=[iu],St.help=void 0;let Vo=St;$("os-save-status",Vo);function au(){return c`
		<svg
			viewBox="0 0 12 12"
			aria-hidden="true"
			focusable="false"
	...
	`}const cu=x`:host{display:flex;width:100%;min-width:0;max-width:100%;align-items:stretch}.os-cat{display:flex;flex-direction:column;align-items:stretch;gap:4px;padding:var( --os-ui-cat-padding,2px );min-height:24px;max-width:100%;width:100%}.os-cat__chips{display:inline-flex;flex-wrap:wrap;align-items:center;gap:var( --os-ui-cat-gap,4px );min-width:0}.os-cat__chains{display:flex;flex-wrap:wrap;gap:4px;min-width:0}.os-cat__viz-host{display:flex;align-items:center;gap:4px;min-width:0;flex:1 1 auto;max-width:100%;min-height:28px;cursor:pointer;position:relative}.os-cat__viz-svg{display:block;width:100%;max-width:100%;min-width:0;overflow:visible;flex:1 1 auto}.os-cat__viz-svg .os-cat-edge{fill:none;stroke:var( --os-ui-cat-edge-color,currentColor );stroke-width:1.25;stroke-linecap:round;opacity:0.55;transition:stroke-width 0.18s ease,opacity 0.18s ease}.os-cat__viz-svg .os-cat-edge[ data-active='true' ]{stroke-width:2;opacity:1}.os-cat__viz-svg .os-cat-node{cursor:pointer;transition:r 0.2s cubic-bezier( 0.34,1.56,0.64,1 ),fill 0.18s ease,stroke-width 0.18s ease,filter 0.18s ease}.os-cat__viz-svg .os-cat-node[ data-selected='true' ]{filter:drop-shadow( 0 0 6px var( --os-ui-cat-node-glow,rgba( 0,0,0,0.18 ) ) )}.os-cat__viz-svg .os-cat-node:hover,.os-cat__viz-svg .os-cat-node:focus-visible{filter:drop-shadow( 0 0 8px var( --os-ui-cat-node-glow,rgba( 0,0,0,0.3 ) ) );outline:none}.os-cat__viz-svg .os-cat-label{font-family:var( --os-ui-font,system-ui,sans-serif );font-size:10.5px;fill:var( --os-ui-cat-label-fg,var( --os-ui-fg,#1d2327 ) );font-weight:500;pointer-events:none;user-select:none}.os-cat__viz-svg .os-cat-label[ data-selected='false' ]{fill:var( --os-ui-cat-label-muted,var( --os-ui-fg-muted,#8c8f94 ) );font-weight:400;font-style:italic}.os-cat__trigger{appearance:none;display:inline-flex;align-items:center;gap:4px;padding:1px 8px;min-height:22px;font:inherit;font-size:11px;font-weight:500;line-height:1;color:var( --os-ui-cat-trigger-fg,var( --os-ui-fg-muted,#50575e ) );background:transparent;border:1px dashed var( --os-ui-cat-trigger-border,var( --os-ui-border,#c3c4c7 ) );border-radius:999px;cursor:pointer;transition:background-color 0.12s ease,color 0.12s ease,border-color 0.12s ease}.os-cat__trigger:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.04 ) );color:var( --os-ui-cat-trigger-fg-hover,var( --os-ui-fg,#1d2327 ) );border-color:var( --os-ui-cat-trigger-border-hover,var( --os-ui-border-strong,#8c8f94 ) )}.os-cat__trigger:focus-visible{outline:none;border-style:solid;box-shadow:0 0 0 2px var( --wp-admin-theme-color,#2271b1 )}.os-cat__trigger svg{display:block}.os-cat__uncategorized{display:inline-flex;align-items:center;gap:4px;padding:1px 10px;min-height:22px;font-size:11px;font-weight:500;line-height:1.6;color:var( --os-ui-cat-uncat-fg,var( --os-ui-fg-muted,#8c8f94 ) );background:transparent;border:1px dashed var( --os-ui-cat-uncat-border,var( --os-ui-border,#c3c4c7 ) );border-radius:999px;font-style:italic}.os-cat__popover{position:fixed;top:0;left:0;min-width:280px;max-width:360px;max-height:360px;display:flex;flex-direction:column;background:var( --os-ui-cat-pop-bg,var( --os-ui-surface,#fff ) );color:var( --os-ui-cat-pop-fg,var( --os-ui-fg,#1d2327 ) );border:1px solid var( --os-ui-cat-pop-border,var( --os-ui-border,#c3c4c7 ) );border-radius:8px;box-shadow:0 6px 16px rgba( 0,0,0,0.08 ),0 1px 2px rgba( 0,0,0,0.06 );z-index:1000}.os-cat__editor{position:relative;display:inline-flex;align-items:center}.os-cat__search{appearance:none;font:inherit;font-size:13px;padding:8px 12px;border:0;border-bottom:1px solid var( --os-ui-cat-pop-divider,var( --os-ui-border,#f0f0f1 ) );background:transparent;color:inherit;outline:none}.os-cat__search:focus{border-bottom-color:var( --wp-admin-theme-color,#2271b1 )}.os-cat__tree{flex:1 1 auto;min-height:0;overflow-y:auto;padding:4px 0}.os-cat__row-block{display:contents}.os-cat__row{display:flex;align-items:center;gap:6px;padding:4px 8px 4px var( --os-ui-cat-row-indent,12px );cursor:pointer;user-select:none;font-size:13px;line-height:1.4;position:relative}.os-cat__row:hover,.os-cat__row[ data-focused='true' ]{background:var( --os-ui-hover,rgba( 0,0,0,0.04 ) )}.os-cat__row[ data-selected='true' ]{color:var( --wp-admin-theme-color,#2271b1 );font-weight:600}.os-cat__row::before{content:'';position:absolute;left:0;top:0;bottom:0;width:var( --os-ui-cat-guide-width,0px );border-left:1px dotted var( --os-ui-cat-guide-color,var( --os-ui-border,rgba( 0,0,0,0.08 ) ) )}.os-cat__expander{display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;border:0;background:transparent;color:inherit;cursor:pointer;flex-shrink:0;opacity:0.65}.os-cat__expander:hover{opacity:1}.os-cat__expander svg{display:block;transition:transform 0.12s ease}.os-cat__row[ data-expanded='true' ] .os-cat__expander svg{transform:rotate( 90deg )}.os-cat__expander--placeholder{visibility:hidden}.os-cat__create-row{display:flex;align-items:center;padding:2px 8px 2px var( --os-ui-cat-row-indent,28px );position:relative}.os-cat__create-row::before{content:'';position:absolute;left:0;top:0;bottom:0;width:var( --os-ui-cat-guide-width,0px );border-left:1px dotted var( --os-ui-cat-guide-color,var( --os-ui-border,rgba( 0,0,0,0.08 ) ) )}.os-cat__create-wrap{display:inline-flex;align-items:center;flex:1 1 auto;min-width:0;gap:4px;padding:1px 1px 1px 0;border:1px solid transparent;border-radius:6px;background:transparent;transition:border-color 0.12s ease,background-color 0.12s ease,box-shadow 0.12s ease}.os-cat__create-wrap:hover{border-color:var( --os-ui-border,rgba( 0,0,0,0.12 ) );background:var( --os-ui-cat-pop-bg,var( --os-ui-surface,#fff ) )}.os-cat__create-wrap:focus-within{border-color:var( --wp-admin-theme-color,#2271b1 );background:var( --os-ui-cat-pop-bg,var( --os-ui-surface,#fff ) );box-shadow:0 0 0 2px color-mix( in srgb,var( --wp-admin-theme-color,#2271b1 ) 15%,transparent )}.os-cat__create-input{flex:1 1 auto;min-width:0;appearance:none;font:inherit;font-size:12px;padding:3px 6px;border:0;background:transparent;color:inherit;outline:none}.os-cat__create-input::placeholder{color:var( --os-ui-cat-pop-muted,var( --os-ui-fg-muted,#8c8f94 ) );font-style:italic;opacity:1}.os-cat__create-submit{appearance:none;display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;flex-shrink:0;padding:0;border:0;border-radius:4px;background:transparent;color:var( --os-ui-cat-pop-muted,var( --os-ui-fg-muted,#8c8f94 ) );cursor:pointer;transition:background-color 0.12s ease,color 0.12s ease}.os-cat__create-wrap:focus-within .os-cat__create-submit:not( [ disabled ] ){background:var( --wp-admin-theme-color,#2271b1 );color:var( --os-ui-fg-on-accent,#fff )}.os-cat__create-submit:hover:not( [ disabled ] ){filter:brightness( 1.05 )}.os-cat__create-submit[ disabled ]{cursor:default;opacity:0.5}.os-cat__create-submit svg{display:block}.os-cat__create-spinner{display:inline-block;width:12px;height:12px;margin:0 4px;border-radius:50%;border:2px solid var( --wp-admin-theme-color,#2271b1 );border-top-color:transparent;animation:os-cat-spin 0.8s linear infinite;flex-shrink:0}.os-cat__check{display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;flex-shrink:0;border:1.5px solid var( --os-ui-cat-check-border,var( --os-ui-border,#8c8f94 ) );border-radius:3px;color:transparent;transition:background-color 0.12s ease,border-color 0.12s ease,color 0.12s ease}.os-cat__row[ data-selected='true' ] .os-cat__check{background:var( --wp-admin-theme-color,#2271b1 );border-color:var( --wp-admin-theme-color,#2271b1 );color:var( --os-ui-fg-on-accent,#fff )}.os-cat__check svg{display:block;width:14px;height:14px}.os-cat__label{min-width:0;flex:1 1 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.os-cat__delete{display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;width:18px;height:18px;border:0;border-radius:50%;padding:0;background:transparent;color:var( --os-ui-cat-delete-color,var( --os-ui-danger,#d63638 ) );cursor:pointer;opacity:0;transition:opacity 0.12s ease,background-color 0.12s ease}.os-cat__row:hover .os-cat__delete,.os-cat__row[ data-focused='true' ] .os-cat__delete,.os-cat__delete:focus-visible{opacity:1}.os-cat__delete:hover,.os-cat__delete:focus-visible{background:var( --os-ui-badge-danger-bg,rgba( 214,54,56,0.12 ) )}.os-cat__delete svg{display:block;width:14px;height:14px}.os-cat__match{background:var( --os-ui-search-highlight-bg,rgba( 252,211,77,0.45 ) );border-radius:2px;padding:0 1px}.os-cat__empty,.os-cat__loading{padding:12px;font-size:12px;color:var( --os-ui-cat-pop-muted,var( --os-ui-fg-muted,#646970 ) );text-align:center}.os-cat__loading-spinner{display:inline-block;width:12px;height:12px;border-radius:50%;border:2px solid currentColor;border-top-color:transparent;animation:os-cat-spin 0.8s linear infinite;margin-inline-end:8px;vertical-align:middle}@keyframes os-cat-spin{to{transform:rotate( 360deg )}}.os-cat__footer{padding:8px 12px;font-size:11px;color:var( --os-ui-cat-pop-muted,var( --os-ui-fg-muted,#646970 ) );border-top:1px solid var( --os-ui-cat-pop-divider,var( --os-ui-border,#f0f0f1 ) );border-radius:10px;background:var( --os-ui-cat-pop-footer-bg,var( --os-ui-surface,#fafafb ) );display:flex;align-items:center;gap:6px;line-height:1.4}.os-cat__footer .dashicons{font-size:14px;width:14px;height:14px;flex-shrink:0}:host( [ disabled ] ) .os-cat{opacity:0.6;pointer-events:none}`,G="10px",du=x`:host{display:inline-flex;max-width:100%;align-items:center;min-width:0;font-family:var( --os-ui-font,system-ui,sans-serif );font-size:12px;line-height:1;font-weight:500}.os-crumb-chain{display:inline-flex;flex-wrap:nowrap;align-items:stretch;max-width:100%;min-width:0;min-height:22px;border-radius:999px;overflow:hidden;filter:drop-shadow( 0 1px 1px rgba( 0,0,0,0.06 ) )}.os-crumb{display:inline-flex;align-items:center;justify-content:center;gap:5px;min-height:22px;padding:2px 12px;background:var( --os-ui-crumb-bg,var( --os-ui-surface,#c3c4c7 ) );color:var( --os-ui-crumb-fg,var( --os-ui-fg,#1d2327 ) );text-align:center;min-width:0;max-width:100%;flex-shrink:1;font-size:12px;font-weight:500;letter-spacing:0.01em;white-space:nowrap;cursor:grab;transition:filter 0.15s ease,transform 0.15s ease,background-color 0.12s ease}.os-crumb:active{cursor:grabbing}.os-crumb:hover{filter:brightness( 1.06 )}.os-crumb__remove{cursor:pointer}.os-crumb--first{padding-inline-end:22px;clip-path:polygon( 0 0,calc( 100% - ${G} ) 0,100% 50%,calc( 100% - ${G} ) 100%,0 100% )}.os-crumb--middle{padding-inline:22px;margin-inline-start:calc( -1 * ${G} );clip-path:polygon( ${G} 0,calc( 100% - ${G} ) 0,100% 50%,calc( 100% - ${G} ) 100%,${G} 100%,0 50% )}.os-crumb--last{padding-inline-start:22px;padding-inline-end:14px;margin-inline-start:calc( -1 * ${G} );clip-path:polygon( ${G} 0,100% 0,100% 100%,${G} 100%,0 50% )}.os-crumb--solo{padding:2px 12px;border-radius:999px}.os-crumb__label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.4}.os-crumb__remove{appearance:none;display:inline-flex;align-items:center;justify-content:center;width:14px;height:14px;flex-shrink:0;padding:0;border:0;border-radius:50%;background:transparent;color:inherit;cursor:pointer;opacity:0.65;transition:opacity 0.12s ease,background-color 0.12s ease,transform 0.12s ease}.os-crumb__remove:hover,.os-crumb__remove:focus-visible{opacity:1;background:var( --os-ui-scrim,rgba( 0,0,0,0.22 ) );outline:none;transform:scale( 1.1 )}.os-crumb__remove svg{display:block;width:100%;height:100%}.os-crumb-chain:hover{filter:drop-shadow( 0 2px 3px rgba( 0,0,0,0.12 ) )}:host( [ disabled ] ) .os-crumb-chain{opacity:0.55;pointer-events:none}`,Et=class Et extends k{constructor(){super(...arguments),this._segments=[]}get segments(){return this._segments}set segments(e){this._segments=Array.isArray(e)?e.slice():[],this.requestUpdate()}render(){const e=this.removable!==null,t=this._segments;return t.length===0?c``:c`
			...
			<div class="os-cat__row-block">
				...
				>
					${n?c`<button
							...
							>${pr()}</button>`:c`<span class="os-cat__expander os-cat__expander--placeholder" aria-hidden="true">${pr()}</span>`}
					<span class="os-cat__check" aria-hidden="true">${bu()}</span>
					<span class="os-cat__label">${this._highlight(i.item.name,this._query)}</span>
					${os(i.item)?c``:c`<button
								type="button"
				...
				${a&&!os(i.item)?this._renderCreateRow(i.item.id,12+(i.depth+1)*16,(i.depth+1)*16,i.item.name):c``}
			</div>
		...
		`}};Tt.props=["rating","total"],Tt.styles=[_u],Tt.help=void 0;let Go=Tt;function gr(s,e){const t=s.getAttribute(e);if(t===null||t==="")return 0;const o=Number(t);return Number.isFinite(o)?o:0}function yu(s){return s<0?0:s>100?100:s}function wu(s,e){return e===0?"No ratings yet":`Average rating ${(s/100*5).toFixed(1)} out of 5, from ${e} ratings`}function xu(s){const e=Math.floor(s),t=s-e>=.5?1:0,o=5-e-t,i=[];for(let n=0;n<e;n++)i.push(br());for(let n=0;n<t;n++)i.push(ku());for(let n=0;n<o;n++)i.push($u());return i}function br(){return c`
		<svg viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
			<path
				d="M8 1.5l2.06 4.17 4.6.67-3.33 3.25.79 4.58L8 12L3.88 14.17l.79-4.58L1.34 6.34l4.6-.67L8 1.5z"
			/>
		</svg>
	`}function ku(){return c`
	...
	`}$("os-rating-summary",Go);const Au=x` ${R} ${gi}:host{display:block;--_track-bg:var( --os-ui-progress-track-bg,var( --os-ui-surface-sunken,rgba( 0,0,0,0.08 ) ) );--_fill:var( --os-ui-progress-fill,var( --wp-admin-theme-color,#2271b1 ) );--_height:var( --os-ui-progress-height,6px );--_radius:var( --os-ui-progress-radius,999px );--_label-color:var( --os-ui-progress-label-color,inherit );--_label-size:var( --os-ui-progress-label-size,12px );--_label-gap:var( --os-ui-progress-label-gap,4px );width:100%;font:inherit;color:var( --_label-color )}:host( [ hidden ] ){display:none}.header{display:flex;align-items:baseline;justify-content:space-between;gap:8px;margin-bottom:var( --_label-gap );font-size:var( --_label-size );line-height:1.3}.label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.percent{font-variant-numeric:tabular-nums;opacity:0.75;flex-shrink:0}.track{position:relative;width:100%;height:var( --_height );background:var( --_track-bg );border-radius:var( --_radius );overflow:hidden}.fill{position:absolute;inset-block:0;inset-inline-start:0;width:0;background:var( --_fill );border-radius:inherit;transition:width 0.18s ease-out}:host( [ tone='success' ] ){--os-ui-progress-fill:var( --os-ui-success-fg,#3a8a3a )}:host( [ tone='warning' ] ){--os-ui-progress-fill:var( --os-ui-warning-fg,#dba617 )}:host( [ tone='danger' ] ){--os-ui-progress-fill:var( --os-ui-danger,#d63638 )}:host( [ indeterminate ] ) .fill{width:100%;transition:none;background-image:var( --_fill );background-size:300% 300%;background-repeat:no-repeat;animation:os-holo-shimmer 2.4s var( --_holo-loop ) infinite}:host( [ indeterminate ][ tone ] ) .fill{width:33%;background-image:none;animation:os-progress-sweep 1.1s linear infinite}@keyframes os-progress-sweep{0%{transform:translateX( -120% )}100%{transform:translateX( 320% )}}@media ( prefers-reduced-motion:reduce ){.fill{transition:none}:host( [ indeterminate ] ) .fill,:host( [ indeterminate ][ tone ] ) .fill{animation:none;width:100%;opacity:0.6}}`,Pt=class Pt extends k{constructor(){super(...arguments),this._ownedAriaLabel=null}render(){return c`<div class="root" part="root">
									...
									${w.map(v=>c`
											...
											>
												...
												<code class="os-token-field__option-token"
													>${v.token}</code
												>
												${v.sample?c`<span class="os-token-field__option-sample"
															>${v.sample}</span
													  >`:null}
												${v.description?c`<span
		...
		`}};Mt.props=["label","placeholder","hint","multiline","rows","disabled","readonly","insert-label","preview-label","open"],Mt.styles=[Ru],Mt.help=void 0;let Jo=Mt;$("os-token-field",Jo);const Tu=["os-section","os-button","os-swatch","os-swatch-grid","os-segmented","os-segment","os-select","os-option","os-multiselect","os-color-field","os-range-field","os-text-field","os-number-field","os-checkbox","os-checkbox-label","os-switch","os-toast","os-toast-container","os-tabs","os-tab","os-tabpanel","os-window-button","os-menu","os-menu-item","os-context-menu","os-context-menu-option","os-confirm-dialog","os-modal","os-user-search","os-role-picker","os-flyout","os-tab-chip","os-stack","os-cluster","os-icon","os-body","os-panel","os-row","os-grid","os-display","os-empty-state","os-key","os-code","os-badge","os-ribbon","os-tile","os-log","os-steps","os-step","os-table","os-spinner","os-relative-time","os-avatar","os-textarea","os-chip","os-tag-input","os-form","os-save-status","os-category-picker","os-crumb-chain","os-card","os-rating-summary","os-notice","os-progress-bar","os-field-row","os-repeater","os-token-field"];function Pu(s,e,t,o){const i=[s,e,...o];if(t){i.push(t.summary??"",t.status??"");const n=[t.props,t.slots,t.events,t.parts,t.cssProps];for(const r of n)for(const a of r??[]){const l=a;i.push(l.name??"",l.description??"")}}return i.join(" ").toLowerCase()}function zu(s,e){const t=e.toLowerCase().split(/\s+/).filter(Boolean);return t.length?t.every(o=>s.haystack.includes(o)):!0}function Iu(s,e){const t=e?.help?.exampleInit,o=s.querySelector(".os-settings__help-example");if(!(!t||!o)){customElements.upgrade(o);try{t(o)}catch(i){console.error(`[openstation] <${e?.tag}> exampleInit threw:`,i)}}}const Mu=["<!-- 1 — invented name, nothing close in the registry. -->","<os-example-console-fail-due-to-unregistered-component>","</os-example-console-fail-due-to-unregistered-component>","","<!-- 2 — typo within edit distance of a real tag. -->","<os-buton></os-buton>"].join(`
`);let mr=!1;function Du(){if(mr)return;mr=!0;const s=["background: #ffb400","color: #1a1a1a","font-weight: 700","font-size: 12px","padding: 4px 8px","border-radius: 3px"].join(";"),e=["color: #b25c00","font-weight: 500"].join(";");console.log(`%c⚠ wp.os — INTENTIONAL DEMO%c
...
if you want a quieter Components tab.`,s,e)}function Nu(s){const e=Ou(),t=document.createElement("div");t.classList.add("os-settings__help");let o=e[0]?.tag??"",i="",n="";const r=()=>{s.state.developerModeEnabled&&Du();const l=e.filter(p=>zu(p,i)),u=l.find(p=>p.tag===o)??l[0];L(c`
				<!--
					...
					list.
				-->
				${s.state.developerModeEnabled?c`
						<!--
						...
						>
							<div class="os-settings__help-warner">
								<p class="os-settings__help-warner-text">
									${d('Both <os-*> tags in the next piece of code are intentionally bogus, one per answer the warner has. Open the browser console: within ~2 seconds you should see two console.error entries from the framework, each pointing the developer at the fix — a name nothing in the registry comes close to, and a typo the warner can suggest a fix for ("did you mean"). Live copies of the same two tags are rendered off-screen, so the demo fires without affecting layout. Remove this section in your fork if you want a quieter Components tab.')}
								</p>
								<pre
				...
				<div class="os-settings__help-layout">
					...
					>
						...
						${l.map(p=>c`
								<button
									type="button"
									class=${vr("os-settings__help-nav-item",p.tag===(u?.tag??"")?"is-active":"")}
			...
			`,t);const h=t.querySelector(".os-settings__help-detail");h&&(u?.tag??"")!==n&&(h.scrollTop=0),n=u?.tag??"",Iu(t,u)};r();const a=window.wp?.os;if(a?.subscribeOsSettings){const l=a.subscribeOsSettings(()=>{if(!t.isConnected){l();return}r()})}return t}function Hu(s){const e=s.help,t=e?.status??"stable";return c`
		<header class="os-settings__help-head">
			...
			>
			<span
				class=${vr("os-settings__help-badge",`is-${t}`)}
				>${Bu(t)}</span
			>
		</header>

		${e?.summary?c`<p class="os-settings__help-summary">
					${e.summary}
				</p>`:c``}
		...
		${Fu(e)} ${Wu(e)}
		${ju(e)}
		${e?c``:c`<p class="os-settings__help-note">
					${d("This component has no help descriptor yet. Add `static help` to its class for a fuller reference.")}
				</p>`}
	`}function qu(s,e){const t=e?.props??[],o=new Set(t.map(n=>n.name)),i=s.props.filter(n=>!o.has(n));return t.length===0&&i.length===0?c``:c`
		<section class="os-settings__help-group">
			<h4>${d("Props")}</h4>
	...
	`}function Uu(s){return s?.slots?.length?c`
		<section class="os-settings__help-group">
			<h4>${d("Slots")}</h4>
			<ul class="os-settings__help-list">
				${s.slots.map(e=>c`
						<li>
							<code>${e.name}</code>
							${e.description?c` — ${e.description}`:c``}
						</li>
					`)}
			</ul>
		</section>
	`:c``}function Fu(s){return s?.events?.length?c`
		<section class="os-settings__help-group">
			<h4>${d("Events")}</h4>
			<ul class="os-settings__help-list">
				${s.events.map(e=>c`
						<li>
							<code>${e.name}</code>
							${e.detail?c` — <code>${e.detail}</code>`:c``}
							${e.description?c` — ${e.description}`:c``}
						</li>
					`)}
			</ul>
		</section>
	`:c``}function Wu(s){return s?.parts?.length?c`
		<section class="os-settings__help-group">
	...
	`:c``}function ju(s){return s?.cssProps?.length?c`
		<section class="os-settings__help-group">
	...
	`:c``}function Vu(){return c`<p>${d("No components registered.")}</p>`}function Ou(){const s=[];for(const e of Tu){const t=customElements.get(e);if(!t)continue;const o=t.help??null,i=o?.title??Ku(e),n=t.props??[];s.push({tag:e,title:i,help:o,props:n,haystack:Pu(e,i,o,n)})}return s.sort((e,t)=>e.title.localeCompare(t.title)),s}function Ku(s){const e=s.replace(/^os-/,"").replace(/-/g," ");return e.charAt(0).toUpperCase()+e.slice(1)}function Bu(s){switch(s){case"experimental":return d("Experimental");case"planned":return d("Planned");case"stable":default:return d("Stable")}}function vr(...s){return s.filter(Boolean).join(" ")}const _r=K("desktop-mode/wallpaper-registry",()=>({seed:[],listeners:new Set})),de=_r.state.seed,Qo=_r.state.listeners;function ei(s){qs("Wallpaper",Ji(s,xr),s);const e=de.findIndex(t=>t.id===s.id);e>=0?de[e]=s:de.push(s),yr()}function Gu(s){const e=de.findIndex(t=>t.id===s);e>=0&&(de.splice(e,1),yr())}function Xu(s){return Qo.add(s),()=>{Qo.delete(s)}}function yr(){const s=Array.from(Qo);for(const e of s)try{e()}catch(t){typeof console<"u"&&console.error("[openstation] wallpaper registry listener threw:",t)}}function wr(){const s=de.slice(),e=J(F.WALLPAPERS,s);return Array.isArray(e)?e.filter(Zu):(typeof console<"u"&&console.warn("[openstation] `os.wallpapers` filter returned a non-array; falling back to seed list."),s)}function ue(s){return wr().find(e=>e.id===s)}const xr=[{field:"id",message:"missing or not a non-empty string",valid:s=>typeof s.id=="string"&&s.id!==""},{field:"label",message:"missing or not a non-empty string",valid:s=>typeof s.label=="string"&&s.label!==""},{field:"preview",message:"missing or not a non-empty string",valid:s=>typeof s.preview=="string"&&s.preview!==""},{field:"type",message:'must be "css" or "canvas"',valid:s=>s.type==="css"||s.type==="canvas"},{field:"value/resolveValue/mount",message:"css types need `value` or `resolveValue`; canvas types need `mount`",valid:s=>s.type==="css"?typeof s.value=="string"||typeof s.resolveValue=="function":s.type==="canvas"?typeof s.mount=="function":!0}];function Zu(s){return Ji(s,xr).length===0}const he=K("desktop-mode/wallpaper-lazy",()=>({pending:new Map,inflight:new Map}));function Yu(s){he.state.pending.delete(s)}function Ju(s){return(window.openStationWallpapers||{})[s]??null}function Qu(s){const e=he.state.inflight.get(s);if(e)return e;const t=he.state.pending.get(s);if(!t)return Promise.resolve(null);const o=(async()=>{try{await Lc(t.scriptUrl,{translations:t.scriptTranslations,l10n:t.scriptL10n,before:t.scriptBefore,after:t.scriptAfter})}catch(n){return re(F.SHELL_ERROR,{scope:"wallpaper-script-load",id:t.id,error:n}),null}let i=Ju(t.id);if(i&&!i.description&&t.description&&(i={...i,description:t.description}),!i)return re(F.SHELL_ERROR,{scope:"wallpaper-missing-def",id:t.id,error:new Error(`[openstation] No wallpaper def on window.openStationWallpapers["${t.id}"]. Script loaded but didn't publish a def — check the plugin's enqueue + global assignment.`)}),null;try{ei(i)}catch(n){return re(F.SHELL_ERROR,{scope:"wallpaper-register",id:t.id,error:n}),null}return Yu(t.id),i})().finally(()=>{he.state.inflight.delete(s)});return he.state.inflight.set(s,o),o}async function eh(){const s=Array.from(he.state.pending.keys());s.length!==0&&await Promise.all(s.map(e=>Qu(e)))}const kr=K("desktop-mode/wallpaper-settings",()=>({values:{}}));function ti(s){return{...kr.state.values[s]??{}}}function th(s,e){kr.state.values[s]={...e},re(F.WALLPAPER_SETTINGS_CHANGED,{id:s,settings:{...e}})}async function sh(s,e,t,o){const i=new URL(s.mediaUrl);i.searchParams.set("media_type","image"),i.searchParams.set("per_page",String(Ma)),i.searchParams.set("page",String(e)),i.searchParams.set("orderby","date"),i.searchParams.set("order","desc"),i.searchParams.set("_fields","id,source_url,alt_text,title,media_details"),t&&i.searchParams.set("search",t),o&&(i.searchParams.set("openstation_min_width",String(zs)),i.searchParams.set("openstation_min_height",String(Is)));const n=await O(i.toString(),{credentials:"same-origin",headers:{"X-WP-Nonce":s.restNonce}},{source:"desktop-mode/settings/media"});if(!n.ok){let u=`HTTP ${n.status}`;try{const h=await n.json();h&&typeof h.message=="string"&&(u=h.message)}catch{}throw new Error(u)}const r=n.headers.get("X-WP-TotalPages"),a=r?parseInt(r,10):1;return{items:(await n.json()).filter(Ha),totalPages:a||1}}async function oh(s,e){const t=await O(s.mediaUrl,{method:"POST",credentials:"same-origin",headers:{"X-WP-Nonce":s.restNonce,"Content-Type":e.type,"Content-Disposition":`attachment; filename="${Na(e.name)}"`},body:e},{source:"desktop-mode/settings/media-upload"});if(!t.ok){let i=`Upload failed (HTTP ${t.status}).`;try{const n=await t.json();n&&typeof n.message=="string"&&(i=n.message)}catch{}throw new Error(i)}const o=await t.json();return{id:o.id,url:o.source_url}}function ih(s,e){const t=[],o=document.createElement("div");o.className="os-settings__tab-pane",s.config.canUpload&&t.push({key:"upload",label:d("Upload new"),render:()=>nh(s,o,e)}),t.push({key:"library",label:d("Media Library"),render:()=>rh(s,o,e)});const i=t[0].key,n=a=>{const l=a.detail.value;t.find(u=>u.key===l)?.render()},r=document.createElement("div");return L(c`
		...
		`,r),t.find(a=>a.key===i)?.render(),r.firstElementChild}function nh(s,e,t){const o=document.createElement("div");o.className="os-settings__upload-tile",o.dataset.wallpaperId=V,o.setAttribute("aria-pressed",s.state.wallpaper===V?"true":"false");const i=document.createElement("input");i.type="file",i.accept="image/*",i.className="os-settings__file-input",i.addEventListener("change",()=>{const n=i.files?.[0];n&&$r(s,n,o,t),i.value=""}),L(c`${i}${o}`,e),is(s,o,i,t)}function is(s,e,t,o){e.classList.remove("os-settings__upload-tile--filled"),e.classList.remove("os-settings__upload-tile--dragover"),e.classList.remove("os-settings__upload-tile--busy"),e.removeAttribute("aria-label");const i=!!s.state.customImage;i?(e.classList.add("os-settings__upload-tile--filled"),e.setAttribute("aria-label",d("Custom image wallpaper")),e.style.backgroundImage=`url("${encodeURI(s.state.customImage.url)}")`):(e.style.backgroundImage="",e.setAttribute("aria-label",d("Upload a wallpaper image")));const n=r=>{r.stopPropagation(),s.state.customImage=null,s.state.wallpaper===V&&(s.state.wallpaper=Ri),oi(s.state),s.save(),s.apply(),is(s,e,t,o),ns(s,o)};L(i?c`
				...
				`,e),e.onclick=()=>{if(!e.classList.contains("os-settings__upload-tile--busy")){if(s.state.customImage){Lr(s,V,o);return}t.click()}},e.ondragover=r=>{r.preventDefault(),e.classList.add("os-settings__upload-tile--dragover")},e.ondragleave=()=>{e.classList.remove("os-settings__upload-tile--dragover")},e.ondrop=r=>{r.preventDefault(),e.classList.remove("os-settings__upload-tile--dragover");const a=r.dataTransfer?.files?.[0];a&&$r(s,a,e,o)}}async function $r(s,e,t,o){if(!e.type.startsWith("image/")){Ar(t,d("That file isn’t an image."));return}t.classList.add("os-settings__upload-tile--busy"),L(c`<span class="os-settings__upload-status"
			>${d("Uploading…")}</span
		>`,t);const i=t.parentElement?.querySelector(".os-settings__file-input");try{const n=await oh(s.config,e);s.state.customImage={id:n.id,url:n.url},s.state.wallpaper=V,oi(s.state),s.save(),s.apply(),i&&is(s,t,i,o),ns(s,o)}catch(n){t.classList.remove("os-settings__upload-tile--busy"),i&&is(s,t,i,o);const r=n instanceof Error?n.message:d("Upload failed.");Ar(t,r)}}function Ar(s,e){let t=s.querySelector(".os-settings__upload-error");t||(t=document.createElement("span"),t.className="os-settings__upload-error",t.setAttribute("role","status"),s.appendChild(t)),t.textContent=e,window.setTimeout(()=>{t?.remove()},4e3)}function rh(s,e,t){const o=document.createElement("input");o.type="search",o.placeholder=d("Search your media"),o.className="os-settings__library-search",o.setAttribute("aria-label",d("Search media"));const i=document.createElement("div");i.className="os-settings__library-grid";const n=document.createElement("span");n.className="os-settings__library-meta";const r=document.createElement("os-button");r.setAttribute("variant","ghost"),r.textContent=d("Load more");let a="",l=0,u=0,h=[],p=0,m=!1;const _=y=>{s.state.libraryHdOnly=y.detail.checked,s.save(),g()};L(c`
			<div class="os-settings__library">
				<div class="os-settings__library-toolbar">
					${o}
					<os-checkbox-label
						label=${j(d("Only HD (≥%1$d×%2$d)"),zs,Is)}
		...
		`,e);const b=()=>{const y=Cr(s.state,h).length,A=[j(d("Showing %d"),y)];s.state.libraryHdOnly&&p>0&&A.push(j(d("%d hidden by HD filter"),p)),n.textContent=A.join(" · "),r.hidden=l>=u,m?r.setAttribute("disabled",""):r.removeAttribute("disabled")},w=()=>{const y=Cr(s.state,h);if(p=h.length-y.length,y.length===0&&!m)L(c`<p class="os-settings__library-empty">
				...
				</p>`,i);else{i.innerHTML="";for(const A of y)i.appendChild(ah(s,A,t))}b()},v=async()=>{if(!(m||u>0&&l>=u)){m=!0,b(),l===0&&L(c`${Array.from({length:8},()=>c`<div
						class="os-settings__library-tile os-settings__library-tile--skeleton"
					></div>`)}`,i);try{const y=await sh(s.config,l+1,a,s.state.libraryHdOnly);l=l+1,u=y.totalPages,h=h.concat(y.items),w()}catch(y){L(c`<p class="os-settings__library-error">
					${y instanceof Error?j(d("Couldn’t load your media: %s"),y.message):d("Couldn’t load your media.")}
				</p>`,i)}finally{m=!1,b()}}},g=()=>{l=0,u=0,h=[],p=0,v()};let f=null;o.addEventListener("input",()=>{f!==null&&window.clearTimeout(f),f=window.setTimeout(()=>{f=null,a=o.value.trim(),g()},Da)}),r.addEventListener("click",()=>{v()}),v()}function Cr(s,e){return s.libraryHdOnly?e.filter(t=>t.media_details.width>=zs&&t.media_details.height>=Is):e}function ah(s,e,t){const o=s.state.wallpaper===V&&s.state.customImage?.id===e.id,i=e.media_details.sizes||{},n=i.medium?.source_url||i.thumbnail?.source_url||i.large?.source_url||e.source_url,r=e.alt_text||qa(e.title?.rendered||"")||`Image #${e.id}`,a=()=>{s.state.customImage={id:e.id,url:e.source_url},s.state.wallpaper=V,oi(s.state),s.save(),s.apply(),ns(s,t);const u=l.firstElementChild?.parentElement;u&&u.querySelectorAll("[data-media-id]").forEach(h=>{const p=h.dataset.mediaId===String(e.id);h.setAttribute("aria-pressed",p?"true":"false"),h.classList.toggle("os-settings__library-tile--selected",p)})},l=document.createElement("div");return L(c`
			<button
				type="button"
		...
		`,l),l.firstElementChild}const lh=4,Sr=24,Er=4,ch=250,dh="os-settings__wallpaper-live-preview";function uh(s){const e=s.type==="canvas"?s.needs:void 0;if(!e||e.length===0)return Promise.resolve();const t=window.wp?.os;return t?.loadModules?t.loadModules(e):Promise.reject(new Error(`[openstation] Wallpaper "${s.id}" declares needs but wp.os.loadModules is unavailable.`))}function hh(){return window.openStationConfig?.pluginUrl??""}function ph(){return typeof window.matchMedia=="function"&&window.matchMedia("( prefers-reduced-motion: reduce )").matches}function fh(s){const e={...s.previewParams??{}},t=J(F.WALLPAPER_PREVIEW_PARAMS,e,s.id);return!t||typeof t!="object"?e:t}function gh(s){const e=new Map;let t=!1;const o=()=>{let f=0;return e.forEach(y=>{(y.teardown||y.mounting)&&f++}),f},i=f=>{f.remountTimer!==null&&(clearTimeout(f.remountTimer),f.remountTimer=null)},n=f=>{if(f.generation++,f.mounting=!1,i(f),f.teardown){const y=f.teardown;f.teardown=null;try{y()}catch(A){typeof console<"u"&&console.error(`[openstation] Wallpaper "${f.defId}" preview teardown threw:`,A)}}f.overlay.innerHTML=""},r=f=>{t||!f.visible||f.teardown||f.mounting||_&&(f.tile.clientWidth<Sr||f.tile.clientHeight<Sr)||a(f)},a=f=>{const y=ue(f.defId);if(!y?.renderPreview||o()>=lh)return;const A=++f.generation;f.mounting=!0,f.mountWidth=f.tile.clientWidth,f.mountHeight=f.tile.clientHeight;const M={id:y.id,pluginUrl:hh(),prefersReducedMotion:ph(),visible:!document.hidden,settings:ti(y.id),params:fh(y),width:f.mountWidth,height:f.mountHeight},oe=E=>{if(A!==f.generation||t){try{E()}catch{}return}f.mounting=!1,f.teardown=E,l(f)&&u(f)},N=E=>{A===f.generation&&(f.mounting=!1,f.overlay.innerHTML="",typeof console<"u"&&console.error(`[openstation] Wallpaper "${y.id}" renderPreview failed:`,E))};uh(y).then(()=>{if(A!==f.generation||t)return;let E;try{E=y.renderPreview(f.overlay,M)}catch(Z){N(Z);return}if(Ds(E)){E.then(oe,N);return}oe(E)},N)},l=f=>Math.abs(f.tile.clientWidth-f.mountWidth)>Er||Math.abs(f.tile.clientHeight-f.mountHeight)>Er,u=f=>{i(f),f.remountTimer=setTimeout(()=>{f.remountTimer=null,!(t||!f.teardown||!l(f))&&(n(f),r(f))},ch)},h=f=>{for(const y of f){const A=e.get(y.target);A&&(A.visible=y.isIntersecting,y.isIntersecting?r(A):n(A))}},p=typeof IntersectionObserver=="function"?new IntersectionObserver(h,{threshold:.1}):null,m=f=>{for(const y of f){const A=e.get(y.target);!A||t||(!A.teardown&&!A.mounting?r(A):A.teardown&&l(A)&&u(A))}},_=typeof ResizeObserver=="function"?new ResizeObserver(m):null,b=f=>{n(f),p?.unobserve(f.tile),_?.unobserve(f.tile),f.overlay.remove(),e.delete(f.tile)},w=()=>{if(t)return;const f=s.querySelectorAll("os-swatch[data-wallpaper-id]"),y=new Set;f.forEach(A=>{y.add(A);const M=A.dataset.wallpaperId??"",N=!!ue(M)?.renderPreview&&!!p,E=e.get(A);if(E&&(E.defId!==M||!N)&&b(E),!N||e.has(A))return;const Z=document.createElement("div");Z.className=dh,Z.setAttribute("aria-hidden","true"),A.appendChild(Z),e.set(A,{tile:A,overlay:Z,defId:M,generation:0,teardown:null,mounting:!1,visible:!1,mountWidth:0,mountHeight:0,remountTimer:null}),p.observe(A),_?.observe(A)}),e.forEach((A,M)=>{y.has(M)||b(A)})},v=()=>{t||(t=!0,e.forEach(f=>n(f)),e.clear(),p?.disconnect(),_?.disconnect(),document.removeEventListener("os-window-closed",g))},g=()=>{s.isConnected||v()};return document.addEventListener("os-window-closed",g),{sync:w,dispose:v}}function si(s){const{from:e,to:t,angle:o}=s.customGradient;return`linear-gradient(${o}deg, ${e}, ${t})`}const bh=()=>d("Mix your own two-colour gradient and set the angle — your desk, your palette.");function mh(s){ei({id:jt,label:d("Custom gradient"),type:"css",preview:si(s.state),description:bh(),resolveValue:()=>si(s.state),renderEditor:e=>_h(s,e)})}function oi(s){if(!s.customImage){Gu(V);return}const t=`url("${encodeURI(s.customImage.url)}") center/cover no-repeat, #1d2327`;ei({id:V,label:d("Custom image"),type:"css",value:t,preview:t,description:d("Any image from your media library or an upload, sized to cover the whole desk.")})}function Lr(s,e,t){s.state.wallpaper=e,s.save(),s.apply(),ns(s,t);const o=t.querySelector(".os-settings__wallpaper-config-slot");o&&ii(s,o)}function ii(s,e){const t=e.firstElementChild;if(!t)return;const o=ue(s.state.wallpaper);if(!o||typeof o.renderConfig!="function"){e.dataset.expanded="false",e.style.marginTop="";return}e.style.marginTop="12px",L(c`
		...
		`,t),e.dataset.expanded="true"}function vh(s,e){if(typeof e.renderConfig!="function")return;const t=document.createElement("os-modal");t.setAttribute("size","sm"),t.setAttribute("title",j(d("%s settings"),e.label));const o=document.createElement("div");o.className="os-settings__wallpaper-config-form",o.style.display="flex",o.style.flexDirection="column",o.style.gap="14px",t.appendChild(o);const i=document.createElement("os-button");i.setAttribute("slot","footer"),i.setAttribute("variant","primary"),i.textContent=d("Done"),t.appendChild(i);let n=null,r=!1;const a=()=>{if(!r){if(r=!0,n){try{n()}catch(u){typeof console<"u"&&console.error(`[openstation] Wallpaper "${e.id}" config teardown threw:`,u)}n=null}t.remove()}};i.addEventListener("click",a),t.addEventListener("os-modal-cancel",a);const l={id:e.id,pluginUrl:"",prefersReducedMotion:typeof window.matchMedia=="function"&&window.matchMedia("( prefers-reduced-motion: reduce )").matches,visible:!document.hidden,settings:ti(e.id),setSettings:u=>{const h={...s.state.wallpaperSettings[e.id]??{},...u};s.state.wallpaperSettings[e.id]=h,s.save(),th(e.id,h)}};document.body.appendChild(t),t.setAttribute("open","");try{const u=e.renderConfig(o,l);Ds(u)?u.then(h=>{if(r){try{h()}catch{}return}n=h}):n=u}catch(u){typeof console<"u"&&console.error(`[openstation] Wallpaper "${e.id}" renderConfig threw:`,u),a()}}function ns(s,e){e.querySelectorAll("[data-wallpaper-id]").forEach(t=>{const o=t.dataset.wallpaperId===s.state.wallpaper;o?t.setAttribute("selected",""):t.removeAttribute("selected"),t.setAttribute("aria-pressed",o?"true":"false")})}function ni(s,e,t){Rr(s);const o=document.createElement("div");if(o.className="os-settings__editor-slot-inner",e.textContent="",e.appendChild(o),!t.renderEditor){e.dataset.expanded="false";return}const i={id:t.id,pluginUrl:"",prefersReducedMotion:typeof window.matchMedia=="function"&&window.matchMedia("( prefers-reduced-motion: reduce )").matches,visible:!document.hidden,settings:ti(t.id)};try{const n=t.renderEditor(o,i);Ds(n)?n.then(r=>{s.activeEditorTeardown=r}):s.activeEditorTeardown=n}catch(n){typeof console<"u"&&console.error(`[openstation] Wallpaper "${t.id}" renderEditor threw:`,n)}e.dataset.expanded="true"}function Rr(s){if(s.activeEditorTeardown){try{s.activeEditorTeardown()}catch(e){typeof console<"u"&&console.error("[openstation] Wallpaper editor teardown threw:",e)}s.activeEditorTeardown=null}}function _h(s,e){e.classList.add("os-settings__gradient-editor-inner");const t=a=>{s.state.customGradient.from=a.detail.value,n()},o=a=>{s.state.customGradient.to=a.detail.value,n()},i=a=>{s.state.customGradient.angle=a.detail.value,n()},n=()=>{s.save(),s.apply(),yh(s,e),r()},r=()=>L(c`
				...
				<os-range-field
					...
					@os-range-change=${i}
				></os-range-field>
			`,e);return r(),()=>{}}function yh(s,e){const o=e.closest("os-section")?.querySelector(`[data-wallpaper-id="${jt}"]`);o&&(o.style.background=si(s.state))}let Tr=null;function wh(s,e){const t=document.createElement("div");t.className="os-settings__editor-slot",t.dataset.expanded="false";const o=document.createElement("div");o.className="os-settings__editor-slot-inner",t.appendChild(o);const i=document.createElement("div");i.className="os-settings__wallpaper-config-slot",i.dataset.expanded="false";const n=document.createElement("div");n.className="os-settings__wallpaper-config-slot-inner",i.appendChild(n);const r=g=>{const f=g.detail?.value??"",y=ue(f);!y||y.id===V||(Lr(s,y.id,e),ni(s,t,y),b())},a=ih(s,e);let l=!!s.state.customImage;const u=document.createElement("div");u.className="os-settings__image-picker-slot",u.dataset.expanded=String(l);const h=document.createElement("div");h.className="os-settings__image-picker-slot-inner",h.appendChild(a),u.appendChild(h);const p=()=>{l=!l,u.dataset.expanded=String(l),b()},m=document.createElement("div");Tr?.dispose();const _=gh(m);Tr=_;const b=()=>L(c`
				<os-section
					heading=${d("Wallpaper")}
				...
				>
					<div
						...
						@os-pick=${r}
					>
						${wr().filter(g=>g.id!==V).sort((g,f)=>+(g.id===jt)-+(f.id===jt)).map(g=>c`<os-swatch
									value=${g.id}
									label=${g.label}
									preview=${g.preview}
									variant="wallpaper"
									data-wallpaper-id=${g.id}
									?selected=${s.state.wallpaper===g.id}
								>
			...
			`,m);b(),_.sync();const w=ue(s.state.wallpaper);w&&ni(s,t,w),ii(s,i);const v=Xu(()=>{if(!m.isConnected){v(),_.dispose();return}b(),_.sync();const g=ue(s.state.wallpaper);g&&ni(s,t,g),ii(s,i)});return eh(),m}function xh(s,e){return s.capability&&s.capability==="manage_options"?e:!0}function Pr(s){return s<20?1:s<40?2:3}const kh=["heading","description","label","placeholder"];function pe(s,e=""){return c`
		<header class="os-settings__page-header">
			<h2 class="os-settings__page-title">${s}</h2>
			${e?c`<p class="os-settings__page-description">
						${e}
					</p>`:c``}
		</header>
	`}function $h(s,e){mh(s),Rr(s),s.tabRegistryUnsubscribe&&(s.tabRegistryUnsubscribe(),s.tabRegistryUnsubscribe=null),e.classList.add("os-settings");const t=()=>{const b=s.state.customImage;s.state={...Va(),customImage:b},s.save(),s.apply(),s.renderPanel(e)},o=s.config.isAdmin,i=Ba().filter(b=>xh(b,o)),n=[{id:"appearance",order:10,label:d("Appearance"),panel:c`<os-tabpanel for="appearance">
				<os-panel>
					${pe(d("Appearance"),d("Personalize your desktop. Changes apply instantly and are saved to this browser."))}
					<!--
			...
			</os-tabpanel>`},{id:"windows",order:18,label:d("Windows"),panel:c`<os-tabpanel for="windows">
				<os-panel>
					${pe(d("Windows"),d("How windows look, how they arrive, and how they behave when they are not the one you are using."))}
					${$c(s)}
					${Nc(s)}
				</os-panel>
			</os-tabpanel>`},{id:"features",order:30,label:d("Features"),panel:c`<os-tabpanel for="features">
				<os-panel>
					${pe(d("Features"),d("The assistant, the developer tools, and the betas. Every switch here affects only your account and takes effect immediately."))}
					${Qc(s)}
					${o?Hc(s):""}
				</os-panel>
			</os-tabpanel>`},{id:"themes",order:12,label:d("Themes"),panel:c`<os-tabpanel for="themes">
				<os-panel>
					${pe(d("Themes"),d("A desktop theme repaints every token at once. A coarser version of what Appearance does one control at a time."))}
					${dc(s)}
				</os-panel>
			</os-tabpanel>`},{id:"navigation",order:22,label:d("Navigation"),panel:c`<os-tabpanel for="navigation">
				<os-panel>
					<!--
			...
			</os-tabpanel>`}];o&&n.push({id:"help",order:40,label:d("Components"),panel:c`<os-tabpanel for="help">
				<os-panel>
					${pe(d("Components"),d("Every <os-*> web component shipped by this plugin, with its props, slots, and a live example."))}
					${Nu(s)}
				</os-panel>
			</os-tabpanel>`}),n.push({id:"about",order:Number.MAX_SAFE_INTEGER,label:d("About"),panel:c`<os-tabpanel for="about">
			<os-panel padding="0">${sl()}</os-panel>
		</os-tabpanel>`});for(const b of i){const w=`ext-${b.id}`,v=`os-settings-tab-host-${b.id}`,g=b;n.push({id:w,order:b.order??100,label:b.label,icon:Li[b.id]?.(),panel:c`<os-tabpanel for=${w}>
				<os-panel><div data-host=${v}></div></os-panel>
			</os-tabpanel>`,mount:f=>{const y=f.querySelector(`[data-host="${v}"]`);if(y)try{g.render(y,{isAdmin:o,getOsSettings:()=>s.getOsSettingsSnapshot(),subscribeOsSettings:A=>s.subscribeOsSettings(A)})}catch(A){typeof console<"u"&&console.error("[openstation] settings tab render threw:",g.id,A)}}})}n.sort((b,w)=>b.order-w.order);const r=s.activeTabId??"appearance",l=n.some(b=>b.id===r)?r:"appearance",u=()=>{const b=new Map;for(const w of n){const v=e.querySelector(`os-tabpanel[for="${w.id}"]`),g=[w.label,v?.textContent??""];if(v){const f=v.querySelectorAll("[heading],[description],[label],[placeholder]");for(const y of Array.from(f))for(const A of kh)g.push(y.getAttribute(A)??"")}b.set(w.id,g.join(" ").replace(/\s+/g," ").toLowerCase())}return b};let h=null;const p=b=>{const w=b.target.value.trim().toLowerCase();h=h??u();let v=0;for(const f of Array.from(e.querySelectorAll(":scope > os-tabs > os-tab"))){const y=f.getAttribute("value")??"",A=w===""||(h.get(y)??"").includes(w);f.toggleAttribute("data-search-hidden",!A),A&&v++}const g=e.querySelector(".os-settings__search-empty");g&&(g.hidden=v>0)};L(c`
			...
			>
				${n.map((b,w)=>{const v=w>0&&Pr(b.order)!==Pr(n[w-1].order);return c`<os-tab
						value=${b.id}
						data-group-start=${v?"true":null}
						>${Li[b.id]?.()??b.icon??c`<span
			...
			${n.map(b=>b.panel)}
		`,e);for(const b of n)b.mount&&b.mount(e);const m=e.querySelector("os-tabs");if(m){s.tabChangeAbort?.abort();const b=new AbortController;s.tabChangeAbort=b,m.addEventListener("os-tab-change",w=>{const v=w.detail;v?.value&&(s.activeTabId=v.value)},{signal:b.signal})}const _=e.closest(".os-window");_&&Ia(_,[]),s.activeTabId=l,s.tabRegistryUnsubscribe=Ga(()=>{if(!e.isConnected){s.tabRegistryUnsubscribe&&(s.tabRegistryUnsubscribe(),s.tabRegistryUnsubscribe=null);return}s.renderPanel(e)})}window.openStationRenderOsSettingsPanel=$h})();
