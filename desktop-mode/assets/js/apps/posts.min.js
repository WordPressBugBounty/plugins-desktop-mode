var openStationApp_posts=function(lt){"use strict";const ts="desktop-mode";function ct(){return window.wp?.i18n}function f(o,e=ts){return ct()?.__(o,e)??o}function de(o,e,t,s=ts){return ct()?._n(o,e,t,s)??(t===1?o:e)}function V(o,...e){const t=ct()?.sprintf;if(t)return t(o,...e);let s=0;return o.replace(/%(?:(\d+)\$)?[sd]/g,(n,a)=>{const i=a?Number.parseInt(a,10)-1:s++;return String(e[i]??"")})}function k(o,...e){return{__wpdHtml:!0,strings:o,values:e}}function $o(o){return!!o&&o.__wpdHtml===!0}const So="$$wpd$$",Z=/\$\$wpd\$\$(\d+)\$\$/g,Eo="$$wpd-node$$",Ao=/^\$\$wpd-node\$\$(\d+)\$\$$/,To=new Set(["style","script","textarea","title"]);function Mo(o){let e="",t=!1,s=null,n=!1,a=null,i="",l=!1;for(let c=0;c<o.length;c++){const r=o[c];e+=r;for(let d=0;d<r.length;d++){const u=r[d];if(n){r.startsWith("-->",d)&&(n=!1,d+=2);continue}if(a!==null){u==="<"&&r[d+1]==="/"&&r.slice(d+2,d+2+a.length).toLowerCase()===a&&(d+=1+a.length,a=null,t=!0,l=!1,i="");continue}if(s!==null){u===s&&(s=null);continue}if(t){u==='"'||u==="'"?(s=u,l=!1):u===">"?(t=!1,l=!1,To.has(i)&&(a=i),i=""):l&&(/[a-zA-Z0-9-]/.test(u)?i+=u.toLowerCase():l=!1);continue}if(r.startsWith("<!--",d)){n=!0,d+=3;continue}const p=r[d+1]??"";u==="<"&&/[a-zA-Z/!]/.test(p)&&(t=!0,i="",l=/[a-zA-Z]/.test(p))}c<o.length-1&&(e+=t||n||a!==null?`${So}${c}$$`:`<!--${Eo}${c}$$-->`)}return e}const ss=new WeakMap;function os(o){const e=ss.get(o);if(e)return e;const t=document.createElement("template");t.innerHTML=Mo(o);const s=[],n=(l,c)=>{if(l.nodeType===Node.ELEMENT_NODE){const u=l;for(const p of Array.from(u.attributes)){const h=p.name,g=p.value,m=h[0];if(Z.test(g))if(Z.lastIndex=0,m==="@"){const b=Z.exec(g);Z.lastIndex=0,s.push({path:c,kind:"event",name:h.slice(1),valueIndex:b?Number(b[1]):0}),u.removeAttribute(h)}else if(m==="."){const b=Z.exec(g);Z.lastIndex=0,s.push({path:c,kind:"prop",name:h.slice(1),valueIndex:b?Number(b[1]):0}),u.removeAttribute(h)}else if(m==="?"){const b=Z.exec(g);Z.lastIndex=0,s.push({path:c,kind:"bool",name:h.slice(1),valueIndex:b?Number(b[1]):0}),u.removeAttribute(h)}else{const b=[],w=[];let $=0,T;for(Z.lastIndex=0;(T=Z.exec(g))!==null;)b.push(g.slice($,T.index)),w.push(Number(T[1])),$=T.index+T[0].length;b.push(g.slice($)),s.push({path:c,kind:"attr",name:h,template:b,valueIndices:w}),u.setAttribute(h,"")}}}const r=Array.from(l.childNodes);let d=0;for(let u=0;u<r.length;u++){const p=r[u],h=u+d;if(p.nodeType===Node.COMMENT_NODE){const g=Ao.exec(p.data);if(g){const m=document.createTextNode("");p.parentNode.replaceChild(m,p),s.push({path:[...c,h],kind:"node",valueIndex:Number(g[1])})}continue}if(p.nodeType===Node.TEXT_NODE){const g=p.textContent||"";if(!Z.test(g)){Z.lastIndex=0;continue}Z.lastIndex=0;const m=p.parentNode;let b=0,w;const $=[],T=[];for(Z.lastIndex=0;(w=Z.exec(g))!==null;){w.index>b&&$.push(document.createTextNode(g.slice(b,w.index)));const R=document.createTextNode("");$.push(R),T.push({path:[...c,h+$.length-1],kind:"node",valueIndex:Number(w[1])}),b=w.index+w[0].length}b<g.length&&$.push(document.createTextNode(g.slice(b)));for(const R of $)m.insertBefore(R,p);m.removeChild(p),d+=$.length-1,s.push(...T)}else n(p,[...c,h])}};n(t.content,[]);const i={template:t,buildParts:l=>{const c=[];for(const r of s){let d=l;for(const u of r.path)d=d.childNodes[u];r.kind==="node"?c.push({kind:"node",valueIndex:r.valueIndex,child:{anchor:d,state:null}}):r.kind==="attr"?c.push({kind:"attr",element:d,name:r.name,template:r.template,valueIndices:r.valueIndices}):r.kind==="event"?c.push({kind:"event",valueIndex:r.valueIndex,element:d,name:r.name}):r.kind==="prop"?c.push({kind:"prop",valueIndex:r.valueIndex,element:d,name:r.name}):r.kind==="bool"&&c.push({kind:"bool",valueIndex:r.valueIndex,element:d,name:r.name})}return c}};return ss.set(o,i),i}const ns=new WeakMap;function Io(o,e){for(const t of o.nodes)if(t.parentNode!==e)return!1;return!0}function ke(o,e){const t=ns.get(e);if(t&&t.strings===o.strings&&Io(t,e)){Xe(t.parts,o.values);return}const s=os(o.strings),n=s.template.content.cloneNode(!0),a=s.buildParts(n),i=Array.from(n.childNodes);for(;e.firstChild;)e.removeChild(e.firstChild);e.appendChild(n),Xe(a,o.values),ns.set(e,{strings:o.strings,parts:a,nodes:i})}function Xe(o,e){for(const t of o)if(t.kind==="node")Ye(t.child,e[t.valueIndex]);else if(t.kind==="attr"){let s=t.template[0];for(let n=0;n<t.valueIndices.length;n++)s+=is(e[t.valueIndices[n]]),s+=t.template[n+1];s!==t.last&&(t.last=s,s===""?t.element.removeAttribute(t.name):t.element.setAttribute(t.name,s))}else if(t.kind==="event"){const s=e[t.valueIndex];s!==t.current&&(t.current&&t.element.removeEventListener(t.name,t.current),s&&t.element.addEventListener(t.name,s),t.current=s)}else if(t.kind==="prop"){const s=e[t.valueIndex];s!==t.last&&(t.last=s,t.element[t.name]=s)}else if(t.kind==="bool"){const s=!!e[t.valueIndex];s!==t.last&&(t.last=s,s?t.element.setAttribute(t.name,""):t.element.removeAttribute(t.name))}}function Ye(o,e){if(e==null||e===!1){o.state&&(ue(o.state),o.state=null);return}if(Array.isArray(e)){Ro(o,e);return}if($o(e)){Po(o,e);return}if(e instanceof Node){Lo(o,e);return}zo(o,is(e))}function Lo(o,e){const t=o.state;t?.shape==="node"&&t.node===e||(t&&ue(t),Me(o,[e]),o.state={shape:"node",node:e})}function zo(o,e){const t=o.state;if(t?.shape==="text"){t.text!==e&&(t.node.textContent=e,t.text=e);return}t&&ue(t);const s=document.createTextNode(e);Me(o,[s]),o.state={shape:"text",node:s,text:e}}function Po(o,e){const t=o.state;if(t?.shape==="template"&&t.strings===e.strings){Xe(t.parts,e.values);return}t&&ue(t);const s=os(e.strings),n=s.template.content.cloneNode(!0),a=s.buildParts(n),i=Array.from(n.childNodes);Me(o,[n]),Xe(a,e.values),o.state={shape:"template",strings:e.strings,parts:a,nodes:i}}function Ro(o,e){const t=o.state;if(t?.shape==="array"){const n=Math.min(t.entries.length,e.length);for(let a=0;a<n;a++)Ye(t.entries[a],e[a]);if(e.length<t.entries.length){for(let a=e.length;a<t.entries.length;a++){const i=t.entries[a];i.state&&ue(i.state),i.anchor.remove()}t.entries.length=e.length}else for(let a=t.entries.length;a<e.length;a++){const i=document.createTextNode("");Me(o,[i]);const l={anchor:i,state:null};Ye(l,e[a]),t.entries.push(l)}return}t&&ue(t);const s=[];for(const n of e){const a=document.createTextNode("");Me(o,[a]);const i={anchor:a,state:null};Ye(i,n),s.push(i)}o.state={shape:"array",entries:s}}function Me(o,e){const t=o.anchor.parentNode;if(t)for(const s of e)t.insertBefore(s,o.anchor)}function ue(o){if(o.shape==="text"){o.node.remove();return}if(o.shape==="template"){for(const e of o.parts)e.kind==="node"&&e.child.state&&(ue(e.child.state),e.child.state=null);for(const e of o.nodes)e.parentNode&&e.parentNode.removeChild(e);return}if(o.shape==="node"){o.node.parentNode&&o.node.parentNode.removeChild(o.node);return}for(const e of o.entries)e.state&&ue(e.state),e.anchor.remove()}function is(o){return o==null||o===!1?"":String(o)}async function as(o){if(o==="")return!1;const e=globalThis.navigator?.clipboard;if(e?.writeText)try{return await e.writeText(o),!0}catch{}return No(o)}function No(o){const e=globalThis.document;if(!e?.body||typeof e.execCommand!="function")return!1;const t=e.createElement("textarea");t.value=o,t.setAttribute("readonly",""),t.setAttribute("aria-hidden","true"),t.style.position="fixed",t.style.insetInlineStart="-9999px",t.style.opacity="0",e.body.appendChild(t);const s=e.activeElement;t.select();let n=!1;try{n=e.execCommand("copy")}catch{n=!1}return t.remove(),s?.focus?.(),n}const Do="data-os-mode";function dt(o=typeof document<"u"?document.documentElement:null){return!!o&&o.getAttribute(Do)==="mobile"}function Oo(o){return o.phone??dt()?k`<os-select
			class="os-app-list__status"
			os-bind=${o.bind}
			os-action=${o.action}
			.value=${o.value}
			aria-label=${o.label}
		>${o.segments.map(t=>k`<os-option .value=${t.value}>${t.label}</os-option>`)}</os-select>`:k`<os-segmented
		class="os-app-list__status"
		os-bind=${o.bind}
		os-action=${o.action}
		value=${o.value}
		label=${o.label}
	>${o.segments.map(t=>k`<os-segment value=${t.value}>${t.label}</os-segment>`)}</os-segmented>`}function qo(o,e){const s=o.closest(".os-window")?.querySelector(".os-window__menu-panel")??null;if(!s||e.items.length===0)return null;const n=`${e.prefix}:`,a="data-os-app-menu-section";s.querySelectorAll(`[${a}="${e.prefix}"]`).forEach(d=>d.remove());const i=document.createElement("div");i.className="os-app__menu-section",i.setAttribute("role","presentation"),i.setAttribute(a,e.prefix),i.textContent=e.section,s.appendChild(i);const l=new Map;for(const d of e.items){const u=document.createElement("os-menu-item");u.setAttribute("role","menuitemcheckbox"),u.setAttribute("value",n+d.key),u.setAttribute(a,e.prefix),u.classList.add("os-window__menu-item","os-app__menu-item"),u.textContent=d.label||d.key,s.appendChild(u),l.set(d.key,u)}const c=()=>{for(const[d,u]of l)e.isChecked(d)?u.setAttribute("checked",""):u.removeAttribute("checked")};c();const r=d=>{const u=d.detail?.value;if(typeof u!="string"||!u.startsWith(n))return;const p=u.slice(n.length);l.has(p)&&(e.onToggle(p),c())};return s.addEventListener("os-menu-item-click",r),{refresh:c,dispose:()=>{s.removeEventListener("os-menu-item-click",r),i.remove();for(const d of l.values())d.remove();l.clear()}}}function Ho(o,e=null){const t=e??(typeof document<"u"?document.documentElement:null);if(!!t&&dt(t)){o.hasAttribute("stacked")||o.setAttribute("stacked","");const a=o.getAttribute("sticky-columns");return a!==null&&(o.setAttribute("data-os-sticky-columns",a),o.removeAttribute("sticky-columns")),!0}o.hasAttribute("stacked")&&o.removeAttribute("stacked");const n=o.getAttribute("data-os-sticky-columns");return n!==null&&(o.setAttribute("sticky-columns",n),o.removeAttribute("data-os-sticky-columns")),!1}function rs(o){return Array.from(o.selection??[],String)}function Fo(){let o=null,e=!1,t=null,s=null;return{invalidateColumns(){o=null},invalidateData(){s=null},sync(n){const a={phone:!1,columnsChanged:!1,dataChanged:!1,selectionChanged:!1},i=n.table;if(!i)return a;const l=n.rowId??(r=>r.id),c=Ho(i);if(a.phone=c,c!==o&&(o=c,i.columns=n.columns(c),a.columnsChanged=!0),e||(e=!0,n.wire?.(i)),n.listKey!==t&&(t=n.listKey,rs(i).length>0&&(i.clearSelection(),a.selectionChanged=!0,n.onSelection?.([]))),n.fingerprint!==s){s=n.fingerprint,i.data=n.rows,a.dataChanged=!0;const r=new Set(n.rows.map(p=>String(l(p)))),d=rs(i),u=d.filter(p=>r.has(p));u.length!==d.length&&(i.selection=u,a.selectionChanged=!0,n.onSelection?.(u))}return a}}}function Uo(o,e){const t=e.local??{},s={id:o,hasLocal:i=>Object.prototype.hasOwnProperty.call(t,i),runLocal:(i,l,c,r)=>{const d=t[i];if(!d)return l;const u={...l},p=d(u,c,r);return p===void 0?u:p},render:i=>{ke(e.view(i),i.root),e.updated?.(i)},mounted:i=>e.mounted?.(i)},{placeholder:n}=e;n&&(s.placeholder=i=>n(i));const a=window;return(a.openStationApps??(a.openStationApps={}))[o]=s,s}const ut=new Map;async function Wo(o){if(!o)return null;let e;try{e=new URL(o,window.location.href)}catch{return o}if(!/gravatar\.com$/i.test(e.hostname))return o;e.searchParams.delete("d"),e.searchParams.delete("s");const t=e.toString(),s=ut.get(t);if(s!==void 0)return s instanceof Promise,s;const n=new URL(o,window.location.href);n.searchParams.set("d","blank");const a=new Promise(i=>{const l=new Image;l.crossOrigin="anonymous",l.onload=()=>{try{const c=document.createElement("canvas");c.width=1,c.height=1;const r=c.getContext("2d",{willReadFrequently:!0});if(!r){i(o);return}r.drawImage(l,0,0,1,1);const d=r.getImageData(0,0,1,1).data;i(d[3]===0?null:o)}catch{i(o)}},l.onerror=()=>i(null),l.src=n.toString()}).then(i=>(ut.set(t,i),i));return ut.set(t,a),a}function Vo(o){return o?.["48"]??o?.["96"]??o?.["24"]??""}function Bo(o,e){e&&Wo(e).then(t=>{o.isConnected&&(t?o.setAttribute("src",t):o.removeAttribute("src"))})}function pe(o){const e=document.createElement("textarea");return e.innerHTML=o,e.value}const ge=class ge extends HTMLElement{constructor(){super(),this._renderScheduled=!1,this._propValues={},this.constructor.shadow?(this.attachShadow({mode:"open"}),this._renderRoot=this.shadowRoot):this._renderRoot=this,this._installPropAccessors()}static get observedAttributes(){return this.props.map(ls)}connectedCallback(){this._adoptStyles(),this.requestUpdate()}attributeChangedCallback(e,t,s){if(t===s)return;const n=jo(e);this._propValues[n]=s,this.requestUpdate()}get classNames(){return Array.from(this.classList)}set classNames(e){if(e==null){this.removeAttribute("class");return}const s=(Array.isArray(e)?e:String(e).split(/\s+/)).map(n=>String(n).trim()).filter(n=>n!=="");this.className=s.join(" ")}requestUpdate(){this._scheduleRender()}emit(e,t){return this.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0}))}_installPropAccessors(){const e=this.constructor;for(const t of e.props){if(Object.getOwnPropertyDescriptor(this,t))continue;const s=ls(t);Object.defineProperty(this,t,{get:()=>t in this._propValues?this._propValues[t]:this.getAttribute(s),set:n=>{let a;n==null||n===!1?a=null:n===!0?a="":a=String(n),this._propValues[t]=a,a===null?this.removeAttribute(s):this.setAttribute(s,a),this.requestUpdate()},enumerable:!0,configurable:!0})}}_scheduleRender(){this._renderScheduled||!this.isConnected||(this._renderScheduled=!0,queueMicrotask(()=>{this._renderScheduled=!1,this.isConnected&&ke(this.render(),this._renderRoot)}))}_adoptStyles(){const e=this.constructor;if(e.styles.length!==0)if(e.shadow&&this.shadowRoot){const t=e.styles.map(s=>s.sheet).filter(s=>s!==null);if(this.shadowRoot.adoptedStyleSheets=t,t.length!==e.styles.length){for(const s of e.styles)if(!s.sheet){const n=document.createElement("style");n.textContent=s.cssText,this.shadowRoot.appendChild(n)}}}else this._adoptLightStyles(e)}_adoptLightStyles(e){if(!ge._lightStylesAdopted.has(e)){ge._lightStylesAdopted.add(e);for(const t of e.styles){const s=document.createElement("style");s.dataset.osUi=this.tagName.toLowerCase(),s.textContent=t.cssText,document.head.appendChild(s)}}}};ge.props=[],ge.styles=[],ge.shadow=!0,ge._lightStylesAdopted=new WeakSet;let X=ge;function Q(o,e){customElements.get(o)||customElements.define(o,e)}function ls(o){return o.replace(/[A-Z]/g,e=>"-"+e.toLowerCase())}function jo(o){return o.replace(/-([a-z])/g,(e,t)=>t.toUpperCase())}const Ko=(()=>{try{return typeof new CSSStyleSheet().replaceSync=="function"}catch{return!1}})();function H(o,...e){let t=o[0];for(let s=1;s<o.length;s++){const n=e[s-1];if(typeof n=="string"||typeof n=="number")t+=String(n);else if(n&&n.__wpdCss)t+=n.cssText;else throw new TypeError("[os-ui] css`` interpolations must be strings, numbers, or other css`` results. Got: "+typeof n);t+=o[s]}if(Ko){const s=new CSSStyleSheet;return s.replaceSync(t),{__wpdCss:!0,sheet:s,cssText:t}}return{__wpdCss:!0,sheet:null,cssText:t}}function Go(o){const e=[],t=[];let s=null,n=o.parentElement;for(;n&&!(n===document.body||n===document.documentElement);){const i=n.id||"";if(i.startsWith("wp-window-")){s=i.slice(10);break}if(n.tagName.toLowerCase()==="os-tabpanel"){const l=n.getAttribute("for");l&&t.unshift(l)}n=n.parentElement}s&&e.push(pt(s));for(const i of t)e.push("tab-"+pt(i));const a=o.getAttribute("label");return a&&e.push(pt(a)),e.length===0?"os-unnamed":"os-"+e.filter(i=>i!=="").join("-")}function pt(o){return o.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function ht(o){if(o.id)return o.id;const e=Go(o);return o.id=e,e}function Xo(o){if(!o)return 214;let e=5381;for(let t=0;t<o.length;t++)e=Math.imul(e,33)+o.charCodeAt(t);return(e%360+360)%360}const Yo=H`:host{display:inline-flex;position:relative;width:var( --os-ui-avatar-size,32px );height:var( --os-ui-avatar-size,32px );flex:0 0 auto;vertical-align:middle;line-height:0;perspective:calc( var( --os-ui-avatar-size,32px ) * 8 );--os-ui-avatar-tilt-x:0deg;--os-ui-avatar-tilt-y:0deg;--os-ui-avatar-hover:0;--os-ui-avatar-glare-x:50%;--os-ui-avatar-glare-y:50%}:host( [ hidden ] ){display:none}.os-avatar__tile{position:relative;width:100%;height:100%;border-radius:50%;overflow:hidden;background:var( --os-window-bg,#f0f0f1 );color:var( --os-ui-fg-on-accent,#fff );display:flex;align-items:center;justify-content:center;font-weight:700;font-size:calc( var( --os-ui-avatar-size,32px ) * 0.48 );line-height:1;letter-spacing:0;font-feature-settings:'tnum' 1;user-select:none;transform-style:preserve-3d;transform:rotateX( var( --os-ui-avatar-tilt-x ) ) rotateY( var( --os-ui-avatar-tilt-y ) ) scale( calc( 1 + var( --os-ui-avatar-hover ) * 0.07 ) );transition:transform 220ms cubic-bezier( 0.2,0.8,0.2,1 ),box-shadow 220ms cubic-bezier( 0.2,0.8,0.2,1 );box-shadow:inset 0 0 0 1px rgba( 255,255,255,calc( 0.18 + 0.22 * var( --os-ui-avatar-hover ) ) ),inset 0 0 0 calc( 1px + var( --os-ui-avatar-hover ) * 1px ) rgba( 0,0,0,calc( 0.08 + 0.04 * var( --os-ui-avatar-hover ) ) ),0 calc( 1px + var( --os-ui-avatar-hover ) * 8px ) calc( 6px + var( --os-ui-avatar-hover ) * 18px ) rgba( 0,0,0,calc( 0.08 + 0.18 * var( --os-ui-avatar-hover ) ) )}.os-avatar__tile::after{content:'';position:absolute;inset:0;border-radius:50%;background:radial-gradient( circle at var( --os-ui-avatar-glare-x ) var( --os-ui-avatar-glare-y ),var( --os-ui-scrim,rgba( 255,255,255,0.55 ) ) 0%,var( --os-ui-hover,rgba( 255,255,255,0 ) ) 55% );opacity:var( --os-ui-avatar-hover );mix-blend-mode:overlay;pointer-events:none;transition:opacity 220ms cubic-bezier( 0.2,0.8,0.2,1 )}.os-avatar__tile::before{content:'';position:absolute;inset:calc( var( --os-ui-avatar-hover ) * -3px );border-radius:50%;background:radial-gradient( circle at var( --os-ui-avatar-glare-x ) var( --os-ui-avatar-glare-y ),color-mix( in srgb,var( --os-ui-avatar-halo,rgb( 99,102,241 ) ) calc( 35% * var( --os-ui-avatar-hover ) ),transparent ) 0%,transparent 70% );filter:blur( 4px );pointer-events:none;z-index:-1;transition:inset 220ms cubic-bezier( 0.2,0.8,0.2,1 ),background 220ms}.os-avatar__tile img{width:calc( 100% + 2px );height:calc( 100% + 2px );margin:-1px;object-fit:cover;display:block;transform:translateZ( 1px )}.os-avatar__dot{position:absolute;bottom:0;inset-inline-end:0;width:calc( var( --os-ui-avatar-size,32px ) * 0.32 );height:calc( var( --os-ui-avatar-size,32px ) * 0.32 );min-width:8px;min-height:8px;border-radius:50%;box-sizing:border-box;border:2px solid var( --os-ui-avatar-dot-ring,var( --os-window-bg,#fff ) );background:var( --os-ui-avatar-dot-color,transparent );z-index:2}.os-avatar__dot--online{background:var( --os-ui-success-fg,#00a32a )}.os-avatar__dot--online::after{content:'';position:absolute;inset:-2px;border-radius:50%;border:1px solid var( --os-ui-success-fg,#00a32a );animation:os-avatar-presence 6s var( --os-ui-ease-out,cubic-bezier( 0.22,0.9,0.28,1 ) ) infinite;pointer-events:none}@keyframes os-avatar-presence{0%{opacity:0.6;transform:scale( 1 )}20%{opacity:0;transform:scale( 2.2 )}100%{opacity:0;transform:scale( 2.2 )}}.os-avatar__dot--inactive{background:var( --os-ui-warning-fg,#dba617 )}.os-avatar__dot--offline{background:var( --os-ui-fg-muted,#8c8f94 )}@media ( prefers-reduced-motion:reduce ){.os-avatar__dot--online::after{animation:none;opacity:0.6}.os-avatar__tile{transform:none;transition:box-shadow 200ms}.os-avatar__tile::after,.os-avatar__tile::before{display:none}}`,cs={xs:20,sm:24,md:40,lg:64,xl:96},ds=new Set(["online","inactive","offline"]),Re=class Re extends X{constructor(){super(...arguments),this._presenceHandler=null,this._imgFailed=!1,this._onPointerMove=null,this._onPointerEnter=null,this._onPointerLeave=null,this._tiltRaf=0,this._pendingTiltX="0deg",this._pendingTiltY="0deg",this._pendingGlareX="50%",this._pendingGlareY="50%"}connectedCallback(){super.connectedCallback(),this._maybeAttachPresenceListener(),this._attachHoverEffect()}disconnectedCallback(){this._presenceHandler&&(document.removeEventListener("os-presence-changed",this._presenceHandler),this._presenceHandler=null),this._detachHoverEffect()}attributeChangedCallback(e,t,s){super.attributeChangedCallback(e,t,s),e==="src"&&(this._imgFailed=!1),(e==="user-id"||e==="presence")&&this._maybeAttachPresenceListener()}render(){const e=this._attr("src"),t=this._attr("name")||"",s=this._attr("alt"),n=s!==null?s:t,a=this._attr("size"),i=this._resolveSize(a),l=this._presenceForRender(),c=this._attr("clickable")!==null;this.style.setProperty("--os-ui-avatar-size",`${i}px`);const r=e&&!this._imgFailed?"":this._initialsBg(t),d=e&&!this._imgFailed?k`<img
					src=${e}
					alt=${n}
					@error=${()=>this._onImgError()}
					loading="lazy"
				/>`:this._initials(t),u=l?k`<span
					class=${`os-avatar__dot os-avatar__dot--${l}`}
					aria-label=${this._presenceLabel(l)}
				></span>`:k``;return c?k`
				<button
					type="button"
		...
		`}_renderChipRow(e,t,s,n){const a=this._selectedItemsInOrder();if(a.length===0)return k`
			...
			`;const i=this._buildChains(a);return k`
			<div
				class="os-cat__chains"
				role="list"
				@click=${this._onCellClick}
			>
				${i.map(l=>this._renderChain(l,t,s))}
			</div>
		`}_buildChains(e){const t=new Map;for(const l of this._items)t.set(l.id,l);const s=new Set(e.map(l=>l.id)),n=l=>{for(const c of s){if(c===l)continue;let r=t.get(c),d=16;for(;r&&d-- >0;){if(r.parent===l)return!0;if(!r.parent)break;r=t.get(r.parent)}}return!1},a=e.filter(l=>!n(l.id)),i=[];for(const l of a){const c=[];let r=l,d=16;for(;r&&d-- >0&&((r===l||s.has(r.id))&&c.unshift(r),!!r.parent);)r=t.get(r.parent);const u=c.map(p=>({id:p.id,name:p.name}));i.push({id:l.id,segments:u})}return i}_renderChain(e,t,s){const n=!t&&!s,a=l=>{l.stopPropagation();const c=l.detail,r=typeof c?.index=="number"?c.index:e.segments.length-1,d=new Set;for(const p of e.segments.slice(r))typeof p.id=="number"&&d.add(p.id);const u=this._value.filter(p=>!d.has(p));u.length!==this._value.length&&this.emit("os-categories-change",{value:u})},i=document.createElement("os-crumb-chain");return i.segments=e.segments,n&&i.setAttribute("removable",""),i.addEventListener("os-chain-remove",a),k`<div role="listitem">${i}</div>`}_renderPopover(e,t){const s=this._buildTree(),n=this._filterTree(s,this._query),a=this._flattenForDisplay(n);return this._focusedRow>=a.length&&(this._focusedRow=a.length>0?a.length-1:-1),k`
		...
		`}_renderTreeBody(e,t){return e?k`
			...
			`:t.map((s,n)=>this._renderRow(s,n,t.length))}_renderRow(e,t,s){const{node:n,hasChildren:a}=e,i=this._value.includes(n.item.id),l=!this._collapsed.has(n.item.id),c=12+n.depth*16,r=n.depth>0?n.depth*16:0,d=t===this._focusedRow;return k`
			<div class="os-cat__row-block">
				<div
					...
					data-focused=${d?"true":"false"}
					data-row-id=${String(n.item.id)}
					style=${`--os-ui-cat-row-indent: ${c}px; --os-ui-cat-guide-width: ${r}px;`}
					@mouseenter=${()=>{this._focusedRow=t,this.requestUpdate()}}
					@click=${u=>{u.preventDefault(),this._toggleSelection(n.item.id)}}
				>
					${a?k`<button
								type="button"
								class="os-cat__expander"
								aria-label=${l?"Collapse":"Expand"}
								@click=${u=>{u.stopPropagation(),this._toggleExpand(n.item.id)}}
							>${Ms()}</button>`:k`<span class="os-cat__expander os-cat__expander--placeholder" aria-hidden="true">${Ms()}</span>`}
					<span class="os-cat__check" aria-hidden="true">${En()}</span>
					<span class="os-cat__label">${this._highlight(n.item.name,this._query)}</span>
					${Qe(n.item)?k``:k`<button
								type="button"
								class="os-cat__delete"
								aria-label=${`Delete ${n.item.name}`}
								title=${`Delete ${n.item.name}`}
								@click=${u=>this._onDeleteClick(u,n.item)}
							>${An()}</button>`}
				</div>
				${l&&!Qe(n.item)?this._renderCreateRow(n.item.id,12+(n.depth+1)*16,(n.depth+1)*16,n.item.name):k``}
			</div>
		`}_renderCreateRow(e,t,s,n){const a=this._creatingValues.get(e)??"",i=this._creatingPending.has(e),l=e===0?"Add new category…":`Add child of "${n}"…`;return k`
			<div
			...
			>
				<div class="os-cat__create-wrap">
					...
					/>
					${i?k`<span class="os-cat__create-spinner" aria-hidden="true"></span>`:k`<button
								type="button"
		...
		`}requestUpdate(){super.requestUpdate(),this._schedulePaint()}_schedulePaint(){this._paintScheduled||!this.isConnected||(this._paintScheduled=!0,queueMicrotask(()=>{this._paintScheduled=!1,this.isConnected&&this._paint()}))}_paint(){const e=this.shadowRoot;if(!e)return;e.querySelector("tbody")||ke(this.render(),e);const t=e.querySelector("colgroup"),s=e.querySelector("thead"),n=e.querySelector("tbody");if(!t||!s||!n)return;const a=this._effectiveColumns();this._stacked=this.hasAttribute("stacked");const i=this._readStickyColumns();this._lastStickyIndex=this._computeLastStickyIndex(a,i),this._paintColgroup(t,a),this._paintHead(s,a,i),this._paintBody(n,a,i),this._applyStickyOffsets(),this._measureHeaderHeight(),this._scheduleStickyOffsets(),this._maybeWarnStickyHeader(),this._maybeWarnLoadingDesync(n),this._ensureResizeObserver()}_maybeWarnLoadingDesync(e){this._loadingDesyncWarned||this.hasAttribute("loading")&&(e.querySelector("tr.skeleton")||(this._loadingDesyncWarned=!0,console.warn("[os-table] `loading` attribute is set but no skeleton rows rendered. Either attributeChangedCallback didn't route through requestUpdate (framework regression), or `loading` was set after the most recent paint and no follow-up trigger ran. Toggling `data` will force a paint as a workaround.")))}_scheduleStickyOffsets(){this._stickyMicroScheduled||(this._stickyMicroScheduled=!0,queueMicrotask(()=>{this._stickyMicroScheduled=!1,this.isConnected&&this._applyStickyOffsets()})),this._stickyRafHandle===null&&typeof requestAnimationFrame<"u"&&(this._stickyRafHandle=requestAnimationFrame(()=>{this._stickyRafHandle=null,this.isConnected&&(this._applyStickyOffsets(),this._measureHeaderHeight())}))}_ensureResizeObserver(){if(this._resizeObserver||typeof ResizeObserver>"u")return;const e=this.shadowRoot?.querySelector(".scroll");e&&(this._resizeObserver=new ResizeObserver(()=>{this.isConnected&&(this._applyStickyOffsets(),this._measureHeaderHeight(),this._stickyHeaderWarned=!1,this._maybeWarnStickyHeader())}),this._resizeObserver.observe(e),this._resizeObserver.observe(this))}_paintColgroup(e,t){const s=[];for(const n of t){const a=document.createElement("col");n.width&&(a.style.width=n.width),s.push(a)}e.replaceChildren(...s)}_paintHead(e,t,s){const n=document.createElement("tr");n.setAttribute("part","header-row");for(let c=0;c<t.length;c++)n.appendChild(this._buildHeaderCell(t[c],c,s));const a=e.querySelector(':scope > tr[part="header-row"]');a?e.replaceChild(n,a):e.insertBefore(n,e.firstChild);const i=t.some(c=>c.filter||Array.isArray(c.filterOptions)||typeof c.filterRender=="function");let l=e.querySelector(":scope > tr.filter-row");if(i){const c=[];for(let u=0;u<t.length;u++)c.push(this._buildFilterCell(t[u],u,s));l||(l=document.createElement("tr"),l.classList.add("filter-row"),l.setAttribute("part","filter-row"),e.appendChild(l));const r=Array.from(l.children);let d=r.length===c.length;if(d){for(let u=0;u<c.length;u++)if(r[u]!==c[u]){d=!1;break}}if(!d){const u=new Set(c);for(const p of c)l.appendChild(p);for(const p of Array.from(l.children))u.has(p)||l.removeChild(p)}}else l&&l.remove()}_buildHeaderCell(e,t,s){const n=document.createElement("th");if(n.setAttribute("scope","col"),n.dataset.key=e.key,this._applyCellClasses(n,e,t,s),e.minWidth&&(n.style.minWidth=e.minWidth),e.key===$e){if(this._readSelectable()==="multi"){const i=document.createElement("input");i.type="checkbox",i.className="select-all-checkbox",i.setAttribute("data-noclick",""),i.setAttribute("aria-label","Select all rows");const{total:l,selected:c}=this._visibleSelectionStats();i.checked=l>0&&c===l,i.indeterminate=c>0&&c<l,i.addEventListener("change",()=>{i.checked?this.selectAll():this.clearSelection()}),n.appendChild(i)}return n}if(n.textContent=e.label??(e.key===Ce?"":e.key),e.sortable){n.classList.add("is-sortable");const a=this._sort?.key===e.key,i=document.createElement("span");i.className="sort-indicator";let l="";a&&(l=this._sort.direction==="asc"?" ▲":" ▼"),i.textContent=l,n.appendChild(i),a&&n.classList.add(this._sort.direction==="asc"?"sort-asc":"sort-desc"),n.addEventListener("click",()=>this._cycleSort(e.key))}return n}_buildFilterCell(e,t,s){const n=this._filterCache.get(e.key),a=Array.isArray(e.filterOptions),i=typeof e.filterRender=="function";let l;if(!e.filter&&!a&&!i||e.key===Ce||e.key===$e?l="none":i?l="custom":e.filter==="select"||a?l="select":l="text",n&&n.kind===l){if(n.th.className="",this._applyCellClasses(n.th,e,t,s),l==="select"){const u=n.control,p=this._resolveFilterOptions(e),h=p.map(g=>g.value).join("|");h!==n.optionsKey?(this._populateSelect(u,p,this._filters[e.key]??""),n.optionsKey=h):u.value=this._filters[e.key]??""}else if(l==="text"){const u=n.control,p=this._filters[e.key]??"";u.value!==p&&u.ownerDocument.activeElement!==u&&(u.value=p)}else l==="custom"&&e.filterRender&&e.filterRender(n.th,{value:this._filters[e.key]??"",setValue:u=>this._onFilterChange(e.key,u),col:e});return n.th}const c=document.createElement("th");if(this._applyCellClasses(c,e,t,s),l==="none")return this._filterCache.set(e.key,{th:c,control:null,optionsKey:"",kind:"none"}),c;if(l==="custom"&&e.filterRender)return e.filterRender(c,{value:this._filters[e.key]??"",setValue:u=>this._onFilterChange(e.key,u),col:e}),this._filterCache.set(e.key,{th:c,control:null,optionsKey:"",kind:"custom"}),c;let r,d="";if(l==="select"){const u=document.createElement("select");u.classList.add("filter-select"),u.setAttribute("data-noclick",""),u.setAttribute("aria-label",`Filter ${e.label??e.key}`);const p=this._resolveFilterOptions(e);this._populateSelect(u,p,this._filters[e.key]??""),d=p.map(h=>h.value).join("|"),u.addEventListener("change",()=>{this._onFilterChange(e.key,u.value)}),r=u}else{const u=document.createElement("input");u.type="search",u.classList.add("filter-input"),u.setAttribute("data-noclick",""),u.setAttribute("placeholder","Filter…"),u.setAttribute("aria-label",`Filter ${e.label??e.key}`),u.value=this._filters[e.key]??"",u.addEventListener("input",()=>{this._onFilterChange(e.key,u.value)}),r=u}return c.appendChild(r),this._filterCache.set(e.key,{th:c,control:r,optionsKey:d,kind:l}),c}_populateSelect(e,t,s){e.replaceChildren();const n=document.createElement("option");n.value="",n.textContent="All",e.appendChild(n);for(const a of t){const i=document.createElement("option");i.value=a.value,i.textContent=a.label,a.value===s&&(i.selected=!0),e.appendChild(i)}e.value=s}_resolveFilterOptions(e){return Array.isArray(e.filterOptions)?e.filterOptions:this._uniqueValues(e.key).map(t=>({value:t,label:t}))}_paintBody(e,t,s){if(e.replaceChildren(),this.hasAttribute("loading")){const a=this._readLoadingRows();for(let i=0;i<a;i++)e.appendChild(this._buildSkeletonRow(t,i));return}const n=this._sortedRows(this._filteredRows());if(n.length===0){e.appendChild(this._buildEmptyRow(t.length));return}for(const{row:a,index:i}of n)if(e.appendChild(this._stacked?this._buildStackedRow(a,i,t):this._buildBodyRow(a,i,t,s)),this._expanded.has(i)&&this._subTable){const l=this._subTable(a,i);l&&e.appendChild(this._buildSubTableRow(l,t.length))}}_buildEmptyRow(e){const t=document.createElement("tr");t.classList.add("empty");const s=document.createElement("td");s.colSpan=e;const n=document.createElement("slot");return n.name="empty",n.textContent=this.getAttribute("empty")||"No data",s.appendChild(n),t.appendChild(s),t}_buildSkeletonRow(e,t){const s=document.createElement("tr");if(s.classList.add("skeleton"),s.setAttribute("aria-hidden","true"),this._stacked){s.classList.add("stack-row");const n=document.createElement("td");n.className="stack-body",n.colSpan=Math.max(1,e.length);for(const a of[55+t*7%25,30+t*11%15]){const i=document.createElement("span");i.className="skeleton-bar",i.style.width=`${a}%`,n.appendChild(i)}return s.appendChild(n),s}for(const n of e){const a=document.createElement("td"),i=document.createElement("span");i.className="skeleton-bar";const l=50+(t*7+s.children.length*13)%40;i.style.width=`${l}%`,a.appendChild(i),s.appendChild(a)}return s}_buildBodyRow(e,t,s,n){const a=document.createElement("tr");a.setAttribute("part","row"),a.dataset.rowIndex=String(t);const i=this._getRowId(e,t);a.dataset.rowId=String(i),this._selection.has(i)&&a.classList.add("is-selected"),a.addEventListener("click",l=>{this._onRowClick(e,t,l)});for(let l=0;l<s.length;l++)a.appendChild(this._buildBodyCell(s[l],l,e,t,n));return a}_buildStackedRow(e,t,s){const n=document.createElement("tr");n.setAttribute("part","row"),n.classList.add("stack-row"),n.dataset.rowIndex=String(t);const a=this._getRowId(e,t);n.dataset.rowId=String(a),this._selection.has(a)&&n.classList.add("is-selected"),n.addEventListener("click",r=>{this._onRowClick(e,t,r)});const i=document.createElement("td");i.className="stack-body";let l=0,c=0;for(let r=0;r<s.length;r++){const d=s[r];if(d.key===$e||d.key===Ce){n.appendChild(this._buildBodyCell(d,r,e,t,0));continue}c++;const u=Nn(d,l++);if(u==="hidden")continue;const p=document.createElement("div");if(p.className=`stack-cell stack-${u}`,p.dataset.key=d.key,u==="meta"&&d.label){const m=document.createElement("span");m.className="stack-label",m.textContent=d.label,p.appendChild(m)}const h=document.createElement("span");h.className="stack-value";const g=e[d.key];d.render?this._mountCellContent(h,d.render(g,e,t)):g!=null&&(h.textContent=String(g)),p.appendChild(h),i.appendChild(p)}return i.colSpan=Math.max(1,c),n.appendChild(i),n}_buildBodyCell(e,t,s,n,a){const i=document.createElement("td");if(this._applyCellClasses(i,e,t,a),e.minWidth&&(i.style.minWidth=e.minWidth),e.key===$e){const c=this._getRowId(s,n),r=document.createElement("input");return r.type="checkbox",r.className="select-row-checkbox",r.setAttribute("data-noclick",""),r.setAttribute("aria-label","Select row"),r.checked=this._selection.has(c),r.addEventListener("change",()=>{r.checked?this.select(c):this.deselect(c)}),i.appendChild(r),this._stacked&&(i.setAttribute("data-noclick",""),i.addEventListener("click",d=>{d.target===i&&(r.checked=!r.checked,r.dispatchEvent(new Event("change")))})),i}if(e.key===Ce){if(!(this._subTable?!!this._subTable(s,n):!1))return i;const r=this._expanded.has(n),d=document.createElement("button");return d.type="button",d.className="expander",d.setAttribute("data-noclick",""),d.setAttribute("aria-expanded",r?"true":"false"),d.setAttribute("aria-label",r?"Collapse row":"Expand row"),d.textContent=r?"▾":"▸",d.addEventListener("click",u=>{this._toggleRow(n,s,u)}),i.appendChild(d),i}const l=s[e.key];if(e.render){const c=e.render(l,s,n);this._mountCellContent(i,c)}else l!=null&&(i.textContent=String(l));return i}_buildSubTableRow(e,t){const s=document.createElement("tr");s.classList.add("subtable"),s.setAttribute("part","subtable-row");const n=document.createElement("td");n.colSpan=t;const a=document.createElement("div");if(a.classList.add("subtable-inner"),e instanceof Node)a.appendChild(e);else if(zs(e))ke(e,a);else{const i=document.createElement("os-table");i.columns=e.columns,i.data=e.data,e.subTable&&(i.subTable=e.subTable),a.appendChild(i)}return n.appendChild(a),s.appendChild(n),s}_mountCellContent(e,t){if(typeof t=="string"){e.textContent=t;return}if(t instanceof Node){e.appendChild(t);return}zs(t)&&ke(t,e)}_onFilterChange(e,t){t===""?delete this._filters[e]:this._filters[e]=t,this.emit("os-table-filter-change",{filters:{...this._filters}});const n=this.shadowRoot?.querySelector("tbody");if(n){const a=this._effectiveColumns(),i=this._readStickyColumns();this._lastStickyIndex=this._computeLastStickyIndex(a,i),this._paintBody(n,a,i),this._applyStickyOffsets()}}_onRowClick(e,t,s){const n=s.composedPath?.()??[];for(const a of n){if(a instanceof Element&&a.hasAttribute("data-noclick"))return;if(a===this)break}this.emit("os-table-row-click",{row:e,index:t,originalEvent:s})}_toggleRow(e,t,s){s.stopPropagation();const n=this._expanded.has(e);n?this._expanded.delete(e):this._expanded.add(e),this.emit("os-table-expand-change",{row:t,index:e,expanded:!n}),this._schedulePaint()}_cycleSort(e){!this._sort||this._sort.key!==e?this._sort={key:e,direction:"asc"}:this._sort.direction==="asc"?this._sort={key:e,direction:"desc"}:this._sort=null,this.emit("os-table-sort-change",{sort:this._sort?{...this._sort}:null}),this._schedulePaint()}_emitSelectionChange(){this.emit("os-table-selection-change",{selection:Array.from(this._selection),rows:this.selectedRows})}_filteredRows(){const e=[],t=Object.keys(this._filters).filter(s=>this._filters[s]!=="");for(let s=0;s<this._data.length;s++){const n=this._data[s];let a=!0;for(const i of t){const l=this._columns.find(u=>u.key===i);if(l&&typeof l.filterRender=="function")continue;const c=this._filters[i]??"",r=n[i],d=r==null?"":String(r);if(l?.filter==="select"){if(d!==c){a=!1;break}}else if(!d.toLowerCase().includes(c.toLowerCase())){a=!1;break}}a&&e.push({row:n,index:s})}return e}_sortedRows(e){if(!this._sort)return e;const t=this._columns.find(a=>a.key===this._sort.key);if(!t)return e;const s=this._sort.direction==="desc"?-1:1,n=e.slice();return n.sort((a,i)=>{const l=t.sortValue?t.sortValue(a.row,a.row[t.key]):a.row[t.key],c=t.sortValue?t.sortValue(i.row,i.row[t.key]):i.row[t.key];return Dn(l,c)*s}),n}_uniqueValues(e){const t=new Set;for(const s of this._data){const n=s[e];n!=null&&t.add(String(n))}return Array.from(t).sort()}_visibleSelectionStats(){let e=0,t=0;for(const{row:s,index:n}of this._filteredRows())e++,this._selection.has(this._getRowId(s,n))&&t++;return{total:e,selected:t}}_readStickyColumns(){const e=parseInt(this.getAttribute("sticky-columns")||"0",10);return Number.isFinite(e)&&e>0?e:0}_readLoadingRows(){const e=parseInt(this.getAttribute("loading-rows")||"5",10);return Number.isFinite(e)&&e>0?Math.min(e,100):5}_readSelectable(){const e=this.getAttribute("selectable");return e==="single"?"single":e==="multi"||e===""?"multi":null}_isStickyIndex(e,t,s){return this._stacked||s.sticky===!1?!1:s.sticky===!0?!0:e<t}_computeLastStickyIndex(e,t){let s=-1;for(let n=0;n<e.length;n++)this._isStickyIndex(n,t,e[n])&&(s=n);return s}_applyCellClasses(e,t,s,n){t.key===Ce&&e.classList.add("col-expander"),t.key===$e&&e.classList.add("col-select"),t.align==="center"&&e.classList.add("align-center"),t.align==="end"&&e.classList.add("align-end"),this._isStickyIndex(s,n,t)&&(e.classList.add("is-sticky"),s===this._lastStickyIndex&&e.classList.add("is-sticky-edge"))}_effectiveColumns(){const e=[];return this._readSelectable()&&e.push({key:$e,label:"",width:"40px",align:"center"}),this._subTable&&e.push({key:Ce,label:"",width:"36px",align:"center"}),e.push(...this._columns),e}_applyStickyOffsets(){const e=this.shadowRoot;if(!e)return;const t=e.querySelector("thead tr");if(!t)return;const s=Array.from(t.children),n=[];let a=0;for(let l=0;l<s.length;l++)n[l]=a,s[l].classList.contains("is-sticky")&&(a+=s[l].offsetWidth);e.querySelectorAll("thead tr, tbody tr:not(.subtable):not(.empty):not(.skeleton)").forEach(l=>{const c=Array.from(l.children);for(let r=0;r<c.length;r++)c[r].classList.contains("is-sticky")&&(c[r].style.insetInlineStart=`${n[r]}px`)}),this._maybeWarnStickyOffsetRace(s,n)}_maybeWarnStickyOffsetRace(e,t){if(this._stickyRaceWarned)return;const s=this._readStickyColumns();if(s<2)return;const n=Math.min(s-1,e.length-1);if(n<=0||t[n]!==0||this.offsetWidth===0)return;this._stickyRaceWarned=!0;const a=e[0]?.offsetWidth??0;console.warn(`[os-table] sticky-columns: column ${n} resolved to inset-inline-start: 0px while the host is visible. ths[0].offsetWidth was ${a}px at measurement time. Likely a layout race — call recomputeLayout() after the panel finishes its mount/transition, or wrap the assignment of \`data\` in a requestAnimationFrame.`)}_measureHeaderHeight(){const e=this.shadowRoot;if(!e)return;const t=e.querySelector("thead tr");if(!t)return;const s=t.offsetHeight;s>0&&this.style.setProperty("--os-ui-table-header-height",`${s}px`)}_maybeWarnStickyHeader(){if(this._stickyHeaderWarned||!this.hasAttribute("sticky-header")||this.hasAttribute("loading")||this._data.length<8)return;const e=this.shadowRoot?.querySelector(".scroll");e&&e.offsetWidth!==0&&e.scrollHeight<=e.clientHeight+1&&(this._stickyHeaderWarned=!0,console.warn("[os-table] sticky-header is set but the table has no scroll container. Set --os-ui-table-max-height on the host (or wrap it in a scrolling parent) so the header has something to stick to."))}};He.props=["stickyColumns","stickyHeader","striped","hover","compact","bordered","empty","loading","loadingRows","selectable","stacked"],He.styles=[Rn],He.help=void 0;let At=He;function zs(o){return!!o&&o.__wpdHtml===!0}function Nn(o,e){return o.stack?o.stack:e===0?"title":o.label?"meta":"actions"}function Dn(o,e){if(o===e)return 0;if(o==null)return-1;if(e==null)return 1;if(typeof o=="number"&&typeof e=="number")return o-e;if(o instanceof Date&&e instanceof Date)return o.getTime()-e.getTime();const t=Number(o),s=Number(e);return Number.isFinite(t)&&Number.isFinite(s)?t-s:String(o).localeCompare(String(e))}Q("os-table",At);const On=H` ${_e}:host{display:flex;flex-direction:column;gap:8px;font-size:13px;color:var( --os-ui-fg,#1d2327 );min-width:0}:host( [ hidden ] ){display:none}.os-select__label{font-size:13px;line-height:1.5;color:var( --os-ui-fg-muted,#646970 )}.os-select__trigger{appearance:none;display:flex;align-items:center;gap:8px;width:100%;min-width:0;padding:8px 12px;background:var( --os-ui-hover,rgba( 0,0,0,0.05 ) );border:1px solid transparent;border-radius:8px;font:inherit;font-size:13px;line-height:1.5;color:var( --os-ui-fg,#1d2327 );text-align:start;cursor:pointer;transition:background-color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease}.os-select__trigger:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.08 ) );border-color:var( --os-ui-border-strong,#8c8f94 )}.os-select__trigger:focus,.os-select__trigger:focus-visible{outline:none;border-color:var( --os-ui-accent,#2271b1 );box-shadow:var( --_holo-focus-field )}.os-select__trigger:disabled{opacity:0.5;cursor:not-allowed}:host( [ plain ] ){display:inline-flex;flex-direction:row;align-items:center}:host( [ plain ] ) .os-select__label{display:none}:host( [ plain ] ) .os-select__trigger{width:auto;padding:4px 6px;background:transparent;border-color:transparent;border-radius:6px}:host( [ plain ] ) .os-select__trigger:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.05 ) );border-color:transparent}:host( [ plain ] ) .os-select__trigger:focus,:host( [ plain ] ) .os-select__trigger:focus-visible{border-color:var( --os-ui-accent,#2271b1 )}:host( [ plain ] ) .os-select__value{flex:0 1 auto}.os-select__value{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.os-select__value--placeholder{color:var( --os-ui-fg-muted,#646970 )}.os-select__chevron{flex:0 0 auto;pointer-events:none;color:var( --os-ui-fg-muted,#646970 )}.os-select__trigger:hover .os-select__chevron{color:var( --os-ui-fg,#1d2327 )}.os-select__trigger:focus .os-select__chevron,.os-select__trigger:focus-visible .os-select__chevron{color:var( --os-ui-accent,#2271b1 )}.os-select__popup{position:fixed;inset:auto;margin:0;max-height:min( 320px,60vh );overflow-y:auto;padding:4px;background:var( --os-ui-surface,#fff );border:1px solid var( --os-ui-border,#dcdcde );border-radius:8px;box-shadow:0 12px 32px rgba( 0,0,0,0.45 );color:var( --os-ui-fg,#1d2327 )}.os-select__popup:not( [ data-open ] ){display:none}.os-select__popup:popover-open{display:block}.os-select__popup[ data-open ]{position:absolute;top:calc( 100% + 4px );left:0;min-width:100%;z-index:1000}:host{position:relative}.os-select__option{display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:6px;font-size:13px;line-height:1.5;cursor:pointer;white-space:nowrap}.os-select__check{flex:0 0 auto;visibility:hidden}.os-select__option[ aria-selected='true' ] .os-select__check{visibility:visible}.os-select__option[ data-active ],.os-select__option:hover{background:var( --os-ui-accent,#2271b1 );color:var( --os-ui-accent-ink,var( --os-ui-fg-on-accent,#fff ) )}.os-select__option[ aria-disabled='true' ]{opacity:0.5;cursor:not-allowed}.os-select__option[ aria-disabled='true' ]:hover{background:transparent;color:inherit}@media ( prefers-reduced-motion:reduce ){.os-select__trigger{transition-duration:1ms}}`,qn=H`:host{display:none}`,Fe=class Fe extends X{render(){return k``}};Fe.props=["value","disabled"],Fe.styles=[qn],Fe.help=void 0;let Tt=Fe;Q("os-option",Tt);const Ue=class Ue extends X{constructor(){super(...arguments),this._optionObserver=null,this._open=!1,this._activeIndex=-1,this._typed="",this._dismissedAt=0,this._typedTimer=null,this._onWindowScroll=e=>{const t=e.target,s=this._popup();s&&t instanceof Node&&(t===s||s.contains(t))||this._hide()},this._onWindowResize=()=>this._hide()}set items(e){const t=this.querySelectorAll(":scope > os-option");for(const a of Array.from(t))a.remove();for(const a of e){const i=document.createElement("os-option");i.setAttribute("value",a.value),i.textContent=a.label,this.appendChild(i)}const s=this.value;!(s!==null&&e.some(a=>a.value===s))&&e.length>0&&(this.value=e[0].value),this.requestUpdate()}connectedCallback(){super.connectedCallback(),ht(this),this._optionObserver=new MutationObserver(()=>this.requestUpdate()),this._optionObserver.observe(this,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["value","disabled"],characterData:!0})}disconnectedCallback(){this._optionObserver?.disconnect(),this._optionObserver=null,this._teardownDismiss(),this._open=!1,this._typedTimer&&(clearTimeout(this._typedTimer),this._typedTimer=null)}render(){const e=this.label||"",t=this.value,s=this.placeholder||"",n=this.disabled!==null;e&&this.setAttribute("aria-label",e);const a=this.getAttribute("aria-label")||"",i=e||a||s,l=this._readOptions(),c=l.find(g=>g.value===t),r=c?c.label:s,d=this.id||"os-unnamed",u=`${d}__listbox`,p=`${d}__trigger`,h=this._open&&this._activeIndex>=0?`${d}__opt-${this._activeIndex}`:"";return k`
			...
			>
				<span
					...
					>${r}</span
				>
				${se("chevron-right",{size:16,rotate:90,className:"os-select__chevron"})}
			</button>
			<div
			...
			>
				${l.map((g,m)=>k`
						<div
							...
							?data-active=${this._open&&m===this._activeIndex}
							@click=${()=>this._onOptionClick(g)}
							@pointermove=${()=>this._setActive(m)}
						>
							${se("check",{size:16,className:"os-select__check"})}
		...
		`};let Mt=Ue;Q("os-select",Mt);const Hn=H`
	:host {
		display: flex;
...
`;function Fn(){const o="os-multiselect-popover-styles";if(document.getElementById(o))return;const e=document.createElement("style");e.id=o,e.textContent=`
...
`,document.head.appendChild(e)}typeof document<"u"&&Fn();const We=class We extends X{constructor(){super(...arguments),this._optionObserver=null,this._popover=null,this._teardownOpen=null,this._hasMore=!1,this._loadingMore=!1}set items(e){const t=this.querySelectorAll(":scope > os-option");for(const a of Array.from(t))a.remove();for(const a of e){const i=document.createElement("os-option");i.setAttribute("value",a.value),i.textContent=a.label,this.appendChild(i)}this._loadingMore=!1;const s=new Set(e.map(a=>a.value)),n=this._readValues().filter(a=>s.has(a));this._writeValueAttribute(n),this.requestUpdate(),this._refreshPopover()}get values(){return this._readValues()}set values(e){const t=Array.isArray(e)?e.map(s=>String(s)).filter(s=>s!==""):[];this._writeValueAttribute(t),this.requestUpdate(),this._refreshPopover()}get hasMore(){return this._hasMore}set hasMore(e){this._hasMore=!!e,this._refreshPopover()}get loadingMore(){return this._loadingMore}set loadingMore(e){this._loadingMore=!!e,this._refreshPopover()}appendItems(e){if(this._loadingMore=!1,!e||e.length===0){this._refreshPopover();return}const t=new Set(Array.from(this.querySelectorAll(":scope > os-option")).map(s=>s.getAttribute("value")));for(const s of e){if(t.has(s.value))continue;const n=document.createElement("os-option");n.setAttribute("value",s.value),n.textContent=s.label,this.appendChild(n)}this.requestUpdate(),this._refreshPopover()}connectedCallback(){super.connectedCallback(),ht(this),this._optionObserver=new MutationObserver(()=>{this.requestUpdate(),this._refreshPopover()}),this._optionObserver.observe(this,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["value","disabled"],characterData:!0})}disconnectedCallback(){this._optionObserver?.disconnect(),this._optionObserver=null,this._closePopover()}render(){const e=this.label||"",t=this.placeholder||"All",s=this.disabled!==null;e?this.setAttribute("aria-label",e):this.removeAttribute("aria-label");const n=e||t,a=this._summarize(t),i=this._readValues().length>0,c=`${this.id||"os-unnamed"}__trigger`;return k`
		...
		`}_onChange(e){const s=e.target.checked;s?this.setAttribute("checked",""):this.removeAttribute("checked"),this.emit("os-checkbox-change",{checked:s,value:this.value})}};Ve.props=["checked","value","label","disabled"],Ve.styles=[hi],Ve.help=void 0;let Nt=Ve;Q("os-checkbox",Nt);const fi=H`:host{--_border:var( --os-ui-stat-border,var( --os-ui-border,#dcdcde ) );--_bg:var( --os-ui-stat-bg,transparent );--_value-color:var( --os-ui-stat-value-color,var( --wp-admin-theme-color,#2271b1 ) );--_value-size:var( --os-ui-stat-value-size,20px );--_label-color:var( --os-ui-stat-label-color,var( --os-ui-fg-muted,#646970 ) );display:flex;flex-direction:column;gap:2px;padding:var( --os-ui-stat-padding,10px 12px );border:1px solid var( --_border );border-radius:var( --os-ui-stat-radius,8px );background:var( --_bg );min-inline-size:0}:host( [ hidden ] ){display:none}.value{font-size:var( --_value-size );font-weight:700;line-height:1.1;color:var( --_value-color );font-variant-numeric:tabular-nums}.label{display:inline-flex;align-items:center;gap:6px;font-size:11px;letter-spacing:0.05em;text-transform:uppercase;color:var( --_label-color )}.caption{font-size:11px;color:var( --_label-color )}.swatch{display:inline-block;inline-size:10px;block-size:10px;border-radius:3px;flex:none;background:var( --os-app-tone,var( --os-ui-fg-muted,#646970 ) )}`,Be=class Be extends X{render(){const e=this,t=e.caption||"";return k`
			<span class="value" part="value">${e.value||""}</span>
			<span class="label" part="label">
				${this.hasAttribute("swatch")?k`<i class="swatch" aria-hidden="true"></i>`:""}
				${e.label||""}
			</span>
			${t?k`<span class="caption" part="caption">${t}</span>`:""}
		`}};Be.props=["value","label","caption","swatch"],Be.styles=[fi],Be.help=void 0;let Dt=Be;Q("os-stat",Dt);const gi=()=>({view:"desk",filters:!1,peek:!1,focused:null,inspectorKey:"",fields:[]});function Fs(o){return pe((o.excerpt?.rendered??"").replace(/<[^>]*>/g," ")).replace(/\s+/g," ").trim()}function mi(o,e){if(!o.parent)return f("Top-level page");const t=e.find(s=>s.id===o.parent);return t?we(t):V(f("Parent page #%d"),o.parent)}function Ot(o,e){o.host.openUrl?.(_t(o.extra,e.id),we(e)||f("(no title)"),o.extra.mode==="pages"?"dashicons-admin-page":"dashicons-admin-post")}function Us(o){return k`<span class="os-posts-desk__status" data-status=${o.status}>${fs[o.status]??o.status}</span>`}function bi(o,e){return o.extra.frontPageId===e.id?f("Front page"):o.extra.postsPageId===e.id?f("Posts page"):""}function Ws(o,e){queueMicrotask(()=>o.querySelector(e)?.shadowRoot?.querySelector("button")?.focus())}function _i(o,e){const t=o.extra.mode==="pages",s=o.data?.list.items??[],n=ee(o)?.selection??new Set,a=s.length>0&&s.every(c=>n.has(c.id)),i=c=>{e.view=c.detail.value==="table"?"table":"desk",o.repaint()},l=c=>{const[r,d]=String(c.detail.value).split(":");o.dispatch("sort",{orderby:r,order:d})};return k`<div class="os-posts-desk__tools ${e.filters?"is-expanded":""}">
		<os-text-field id=${`${o.windowId}-content-search`} class="os-app-list__search" data-os-posts-search
			aria-label=${f(t?"Search pages":"Search posts")} os-bind="search" os-action="filter" os-debounce="250"
			value=${o.state.search} placeholder=${f(t?"Search pages…":"Search posts…")}></os-text-field>
		<os-button class="os-posts-desk__filter-toggle" variant="secondary" aria-expanded=${String(e.filters)} @click=${()=>{e.filters=!e.filters,o.repaint()}}>${f("Options")}</os-button>
		<os-checkbox label=${f("Select loaded")} ?checked=${a}
			@os-checkbox-change=${()=>{a?ee(o)?.clearSelection():ee(o)?.selectAll()}}></os-checkbox>
		<os-select aria-label=${f("Sort content")} value=${`${o.state.orderby}:${o.state.order}`} @os-pick=${l}>
			${t?k`<os-option value="menu_order:asc">${f("Page order")}</os-option>`:""}
			<os-option value="date:desc">${f("Newest first")}</os-option>
			<os-option value="date:asc">${f("Oldest first")}</os-option>
			<os-option value="modified:desc">${f("Recently edited")}</os-option>
			<os-option value="title:asc">${f("Title A–Z")}</os-option>
			<os-option value="title:desc">${f("Title Z–A")}</os-option>
			<os-option value="author:asc">${f("By author")}</os-option>
			<os-option value="author:desc">${f("By author, reversed")}</os-option>
			<os-option value="modified:asc">${f("Least recently edited")}</os-option>
			<os-option value="comment_count:asc">${f("Fewest comments")}</os-option>
			<os-option value="comment_count:desc">${f("Most comments")}</os-option>
		</os-select>
		<os-select aria-label=${f("Content view")} value=${e.view} @os-pick=${i} data-os-posts-view>
			<os-option value="desk">${f(t?"Page directory":"Writing desk")}</os-option>
			<os-option value="table">${f("Details table")}</os-option>
		</os-select>
	</div>`}function vi(o,e){const t=o.extra.mode==="pages",s=V(f(t?"Copy page ID %d":"Copy post ID %d"),e.id);return k`<os-button variant="ghost" class="os-posts-desk__id" title=${s} @click=${async n=>{n.stopPropagation();const i=await as(String(e.id))?V(f(t?"Copied page ID #%d.":"Copied post ID #%d."),e.id):f("Couldn’t copy the ID. Please try again.");o.host.toast?.({message:i})}}><span class="screen-reader-text">${s}</span><span aria-hidden="true">#${e.id}</span><span class="dashicons dashicons-admin-page" aria-hidden="true"></span></os-button>`}function yi(o,e){const t=e[o.key];let s="";return o.render?s=o.render(t,e,0):t!=null&&(s=String(t)),s==null||s===""||s instanceof Element&&!s.childNodes.length&&!s.textContent?"":k`<div class="os-posts-desk__plugin-stat" data-column=${o.key}><span class="os-posts-desk__plugin-stat-value">${s}</span><span class="os-posts-desk__plugin-stat-label">${o.label||o.key}</span></div>`}function xi(o,e,t,s,n=[]){const a=o.extra.mode==="pages",i=we(t)||f("(no title)"),l=xt(t),c=yt(t,"category")[0]?.name,r=bi(o,t),d=a?mi(t,o.data?.list.items??[]):pe(c||f("Story"));let u="";a?u=k`<div class="os-posts-desk__page-mark" aria-hidden="true"><span class="dashicons ${r?"dashicons-admin-home":"dashicons-admin-page"}"></span><i></i><i></i><i></i></div>`:l&&(u=k`<img class="os-posts-desk__thumbnail" src=${l.url} alt="" loading="lazy" @error=${h=>h.target.hidden=!0}>`);const p=()=>{e.focused=t.id,e.peek=!0,o.repaint(),Ws(o.root,"[data-os-posts-inspect-close]")};return k`<article class="os-posts-desk__story ${s===t.id?"is-current":""}" data-story-id=${t.id}>
		<div class="os-posts-desk__story-top">
			${Us(t)}
			${r?k`<span class="os-posts-desk__role">${r}</span>`:""}
			${vi(o,t)}
			<os-checkbox aria-label=${V(f("Select %s"),i)}
				class="os-posts-desk__select" ?checked=${ee(o)?.selection.has(t.id)??!1}
				@os-checkbox-change=${h=>{h.detail.checked?ee(o)?.select(t.id):ee(o)?.deselect(t.id)}}></os-checkbox>
		</div>
		<div class="os-posts-desk__story-content">
			${u}
			<div class="os-posts-desk__story-copy">
				<div class="os-posts-desk__kicker">${d}</div>
				<h3><a href=${_t(o.extra,t.id)} @click=${h=>{h.preventDefault(),Ot(o,t)}}>${i}</a></h3>
				<p class="os-posts-desk__excerpt">${a?`/${t.slug||""}`:Fs(t)||f("Every story starts somewhere. Open the editor to keep writing.")}</p>
			</div>
		</div>
		<div class="os-posts-desk__metrics" data-content-metrics=${`${t.id}:${t.modified_gmt}`} data-post-id=${t.id}>
			<os-stat data-metric="words" value="—" label=${f("Words")} title=${f("Words in the complete page or post body")}></os-stat>
			<os-stat data-metric="comments" value=${typeof t.openstation_comment_count=="number"?String(t.openstation_comment_count):"—"} label=${f("Comments")} title=${f("Approved comments")}></os-stat>
			${a?"":k`<os-stat value=${String(t.tags?.length??0)} label=${f("Tags")}></os-stat>`}
			${n.map(h=>yi(h,t))}
		</div>
		${t.openstation_lock?k`<p class="os-posts-desk__lock"><span class="dashicons dashicons-lock" aria-hidden="true"></span>${V(f("%s is editing"),t.openstation_lock.userName)}</p>`:""}
		<footer class="os-posts-desk__story-foot">
			<span class="os-posts-desk__byline">${vt(t).name}<span aria-hidden="true"> · </span><os-relative-time datetime=${t.modified_gmt||t.date_gmt}></os-relative-time></span>
			<os-button variant="ghost" @click=${()=>Ot(o,t)}>${f("Edit")}<span aria-hidden="true"> ↗</span></os-button>
			<os-button variant="secondary" data-inspect-id=${t.id} @click=${p}>${f("Details")}</os-button>
		</footer>
	</article>`}function ki(o,e,t,s,n,a){if(!a||!e.peek||e.focused===null)return"";const i=JSON.stringify([a,Array.from(n)]);if(i!==e.inspectorKey){e.inspectorKey=i,e.fields=Pt(t,new Map,s,!1,n).filter(r=>!["title","author","date"].includes(r.key)).map(r=>({key:r.key,label:r.label||r.key,node:r.render?r.render(a[r.key],a,0):String(a[r.key]??"—")}));for(const r of t.categories.pickers)r.isConnected||t.categories.pickers.delete(r)}const l=xt(a),c=()=>{const r=e.focused??a.id;e.focused=null,e.peek=!1,o.repaint(),Ws(o.root,`[data-inspect-id="${r}"]`)};return k`<aside class="os-posts-desk__inspector ${e.focused!==null?"is-open":""}" aria-label=${f("Content details")}
		@keydown=${r=>{r.key==="Escape"&&(r.stopPropagation(),c())}}>
		<header class="os-posts-desk__inspector-head"><span>${o.extra.mode==="pages"?f("PAGE DETAILS"):f("STORY DETAILS")}</span><os-button variant="ghost" data-os-posts-inspect-close @click=${c}>${f("Close details")}</os-button></header>
		<div class="os-posts-desk__inspector-scroll">
			${l?k`<img class="os-posts-desk__cover" src=${l.url} alt=${l.alt} @error=${r=>r.target.hidden=!0}>`:""}
			${Us(a)}
			<h2>${we(a)||f("(no title)")}</h2>
			<p class="os-posts-desk__byline">${vt(a).name}</p>
			${a.openstation_lock?k`<os-notice tone="warning">${V(f("%s is currently editing"),a.openstation_lock.userName)}</os-notice>`:""}
			<p class="os-posts-desk__preview">${Fs(a)||f("No excerpt yet.")}</p>
			<dl class="os-posts-desk__dates"><div><dt>${a.status==="future"?f("Scheduled for"):f("Date")}</dt><dd><os-relative-time datetime=${a.date_gmt||a.date}></os-relative-time></dd></div><div><dt>${f("Last edited")}</dt><dd><os-relative-time datetime=${a.modified_gmt||a.modified}></os-relative-time></dd></div></dl>
			<div class="os-posts-desk__fields">${e.fields.map(r=>k`<section data-detail-field=${r.key}><h3>${r.label}</h3>${r.node}</section>`)}</div>
		</div>
		<footer class="os-posts-desk__inspector-actions"><os-button variant="primary" @click=${()=>Ot(o,a)}>${f("Open editor")}<span aria-hidden="true"> ↗</span></os-button>
			${a.link&&a.status==="publish"?k`<a href=${a.link} target="_blank" rel="noopener noreferrer">${f("View on site")} ↗</a>`:""}
		</footer>
	</aside>`}function wi(o,e,t,s,n,a){const i=o.extra.mode==="pages"?f("No pages found."):f("No posts found."),l=o.data?.list.items??[],c=l.find(d=>d.id===e.focused)??l[0],r=l.length?Xn(t,s,n):[];return k`<div class="os-posts-desk__workspace ${e.focused!==null&&c?"has-detail":""}" ?hidden=${e.view!=="desk"}>
		<div class="os-posts-desk__feed" aria-label=${o.extra.mode==="pages"?f("Page directory"):f("Stories")}>
			${l.length?l.map(d=>xi(o,e,d,e.peek?c?.id:void 0,r)):k`<div class="os-posts-desk__empty"><span class="dashicons dashicons-welcome-write-blog" aria-hidden="true"></span><h3>${o.loading?f("Opening your workspace…"):i}</h3><p>${o.loading?f("Your content will appear here."):f("Try a different search or change the status filter.")}</p></div>`}
			${a??""}
		</div>
		${e.peek?ki(o,e,t,s,n,c):""}
	</div>`}function Ci(o){queueMicrotask(()=>{for(const t of o.querySelectorAll(".os-posts-desk__select, [data-os-posts-search]"))t.shadowRoot?.querySelector("input")?.setAttribute("aria-label",t.getAttribute("aria-label")||"");const e=o.querySelector(".os-posts-desk__filter-toggle");e?.shadowRoot?.querySelector("button")?.setAttribute("aria-expanded",e.getAttribute("aria-expanded")||"false")})}const $i="[openstation:desktop-mode-posts]",Si=50,qt=()=>({desk:gi(),feed:new ci,client:null,env:null,cellCache:new Map,table:Fo(),filterData:{authors:[],tags:[]},columnsKey:"",hidden:null,selected:0,bulkActions:null,extras:null,postsCtx:null,tab:"posts",canvases:{categories:null,tags:null},canvasPending:new Set,menu:null,disposed:!1,tagPage:0,tagTotalPages:1,tagFetching:!1}),Vs=o=>o.mode==="pages"?"pages":"posts";function Ei(o){return o.map(e=>`${e.id}:${e.status}:${e.modified_gmt}`).join("|")}function tt(o,e){return e.client||(e.client=ni(o.fetch)),e.client}function Se(o,e){return o.hidden||(o.hidden=Rs(e)),o.hidden}function Bs(o,e,t){let s="";t instanceof Error?s=t.message:t!=null&&(s=String(t)),o.host.toast?.({message:`${e} ${s}`.trim(),duration:6e3})}function Ht(o,e,t){return e.env||(e.env={extra:o.extra,client:tt(o,e),cells:t,openUrl:(s,n,a)=>o.host.openUrl?.(s,n,a),confirm:s=>o.host.confirm?.(s)??Promise.resolve(!1),toast:(s,n)=>Bs(o,s,n),announce:(s,n)=>o.host.announce?.("post",s,n),parentTitles:new Map,categories:{tree:null,pickers:new Set}}),e.env}function Ai(o,e){return{client:tt(o,e),extra:o.extra,openUrl:(t,s,n)=>o.host.openUrl?.(t,s,n),toast:(t,s)=>Bs(o,t,s),leaveFullscreen:()=>{const t=window.wp?.os?.windowManager?.getById?.(o.windowId);t?.isFullscreen?.()&&t.toggleFullscreen?.()}}}async function js(o,e){if(!(e.tagFetching||e.tagPage>=e.tagTotalPages)){e.tagFetching=!0;try{const t=e.tagPage+1,s=await tt(o,e).fetchTagOptions(t,Si);e.tagPage=t,e.tagTotalPages=Math.max(e.tagTotalPages,s.totalPages||t);const n=new Set(e.filterData.tags.map(a=>a.id));for(const a of s.items)n.has(a.id)||(e.filterData.tags.push(a),n.add(a.id));e.filterData.tagsHasMore=e.tagPage<e.tagTotalPages,o.repaint()}finally{e.tagFetching=!1}}}const Ti=o=>`${o.authors.map(e=>e.id).join(",")}|${o.tags.map(e=>e.id).join(",")}|${o.tagsHasMore?1:0}`,Ks=o=>o.split(",").map(e=>parseInt(e.trim(),10)).filter(e=>Number.isFinite(e)&&e>0),Gs=(o,e)=>o.length===e.length&&o.every((t,s)=>t===e[s]);function Mi(o,e,t){const s=o.extra;t.getRowId=a=>a.id,t.subTable=a=>pn(a);const n=oi(o.state.orderby);t.sort=t.columns.some(a=>a.key===n)?{key:n,direction:o.state.order}:null,t.addEventListener("os-table-selection-change",()=>{e.selected=t.selection?.size??0,o.repaint()}),t.addEventListener("os-table-sort-change",a=>{const i=a.detail?.sort,l=s.defaultOrderby??"date";o.dispatch("sort",i?{orderby:si(i.key,l),order:i.direction}:{orderby:l,order:s.defaultOrder??"desc"})}),t.addEventListener("os-table-filter-change",a=>{const i=a.detail?.filters??{},l=Ks(i.author??""),c=Ks(i.tags??"");Gs(l,o.state.author)&&Gs(c,o.state.tag)||(o.local("set-column-filters",{author:l,tag:c}),o.dispatch("filter"))})}function Ii(o,e={}){const t=e.terms,s=e.cells??{},n="nativePostsHiddenColumns",a=(c,r,d)=>{if(!t||r.canvases[d]||r.canvasPending.has(d))return;const u=c.root.querySelector(d==="categories"?"[data-os-posts-cats-host]":"[data-os-posts-tags-host]");u&&(r.canvasPending.add(d),t[d](u,Ai(c,r)).then(p=>{if(r.disposed){p();return}r.canvases[d]=p}).catch(p=>{console.error(`${$i} ${d} canvas failed`,p)}).finally(()=>r.canvasPending.delete(d)))},i=(c,r,d,u)=>(r.bulkActions||(r.bulkActions=ei(Qn(d,p=>c.dispatch("trash",{ids:p})))),k`
			<div class="os-app-list__toolbar-right ${"os-app-list__bulk--footer"}" data-os-posts-bulk ?hidden=${r.selected===0}>
				<span class="os-app-list__count" data-os-posts-count>${V(f("%d selected"),r.selected)}</span>
				<os-button variant="ghost" @click=${()=>ee(c)?.clearSelection()}>${f("Clear selection")}</os-button>
				<span class="os-app-list__bulk-actions" data-os-posts-bulk-actions>${r.bulkActions.map(p=>k`<os-button
						variant=${p.variant??"secondary"}
						data-os-posts-bulk-action=${p.id}
						@click=${()=>void ri(c,p,Rt(c,r))}
					>${p.icon?k`<span class="dashicons ${p.icon}" aria-hidden="true"></span>`:""} ${p.label}</os-button>`)}</span>
			</div>
		`),l=(c,r,d,u)=>{const p=r.feed.reconcile(c),{state:h,data:g}=p,m=g?.list,b=d==="pages",w=c.extra,$=Ht(c,r,s);Ls($,m?.items??[]);const T=()=>c.host.openUrl?.(w.newPostUrl??"",f(b?"Add New Page":"Add New Post"),b?"dashicons-admin-page":"dashicons-admin-post");return r.extras||(r.extras=ti(Rt(c,r))),k`
			<header class="os-posts-desk__hero">
				<div><span class="os-posts-desk__eyebrow">${f(b?"THE SHAPE OF YOUR SITE":"YOUR EDITORIAL SPACE")}</span><h2>${f(b?"A place for every page.":"Stories in motion.")}</h2><p>${f(b?"Find a page. See where it belongs. Make it yours.":"From first thought to published story. Keep the words moving.")}</p></div>
				<os-button variant="holo" data-os-posts-new @click=${T}><span class="dashicons dashicons-plus" aria-hidden="true"></span>${f(b?"New page":"Write a post")}</os-button>
			</header>
			<header class="os-app-list__toolbar" data-os-posts-toolbar>
				<div class="os-app-list__toolbar-left">
					${Oo({segments:Jn(),value:h.status,bind:"status",action:"filter",label:f("Filter by status"),phone:u})}
				</div>

				<div class="os-app-list__toolbar-trailing">
					<span data-os-posts-toolbar-extras>${r.extras}</span>
					<os-button variant="ghost" os-action="refresh" data-os-posts-refresh title=${f("Refresh")}>
						<span class="dashicons dashicons-update" aria-hidden="true"></span>${f("Refresh")}
					</os-button>
				</div>
			</header>
			${m?.error?k`<os-notice tone="danger">${m.error}</os-notice>`:""}
			${_i(p,r.desk)}
			${wi(p,r.desk,$,r.filterData,Se(r,n),r.feed.tail())}
			<div class="os-app-list__body" data-os-posts-body ?hidden=${r.desk.view!=="table"}>
				<os-table
					data-os-posts-table
					os-preserve
					selectable="multi"
				...
				>
					<div slot="empty" class="os-app-list__empty">
						<span class="dashicons ${b?"dashicons-admin-page":"dashicons-admin-post"}" aria-hidden="true"></span>
						<p>${f(b?"No pages found.":"No posts found.")}</p>
						<p class="os-app-list__empty-hint">
							${f("Try a different search or change the status filter.")}
						</p>
					</div>
				</os-table>
			</div>
			${r.desk.view==="table"?r.feed.tail():""}
			${i(c,r,d)}
		`};return Uo(o,{local:{"set-column-filters":(c,r)=>{c.author=Array.isArray(r.author)?r.author:[],c.tag=Array.isArray(r.tag)?r.tag:[]}},placeholder:c=>({list:{items:[],total:0,pages:0,page:c.page,perPage:c.perPage,error:"",code:""}}),view:c=>{const r=c.ui(qt),d=Vs(c.extra),u=dt(),p=l(c,r,d,u),h=`os-app-list desktop-mode-posts${d==="pages"?" desktop-mode-pages":""}`;if(!t&&!e.atlas)return k`<div class=${h} data-os-posts-root data-desk-options=${String(r.desk.filters)}><style>${Hs.cssText}${qs.cssText}</style>
					<div class="os-app-list__panel">${p}</div>
				</div>`;const g=m=>{const b=m.detail?.value??"posts";if(r.tab=b,b==="atlas"&&!r.atlas&&e.atlas){const w=c.root.querySelector("[data-os-pages-atlas]");w&&(r.atlas=e.atlas(w,c))}(b==="categories"||b==="tags")&&a(c,r,b)};return k`<div class=${h} data-os-posts-root data-desk-options=${String(r.desk.filters)}><style>${Hs.cssText}${qs.cssText}</style>
				<os-tabs value=${r.tab} class="os-app-list__tabs" @os-tab-change=${g}>
					<os-tab value="posts">${f(d==="pages"?"All pages":"All posts")}</os-tab>
					${e.atlas?k`<os-tab value="atlas">${f("Page atlas")}</os-tab>`:""}
					${t?k`<os-tab value="categories">${f("Categories")}</os-tab><os-tab value="tags">${f("Tags")}</os-tab>`:""}
				</os-tabs>
				<os-tabpanel for="posts" class="os-app-list__panel">${p}</os-tabpanel>
				${e.atlas?k`<os-tabpanel for="atlas" class="os-app-list__panel"><div data-os-pages-atlas class="os-pages-atlas-host" os-preserve></div></os-tabpanel>`:""}
				${t?k`<os-tabpanel for="categories" class="os-app-list__panel">
			...
			</div>`},mounted:c=>{const r=c.ui(qt),d=[];r.stats=pi(c.root,c.fetch,Vs(c.extra)),r.stats.sync(),d.push(()=>r.stats?.dispose());const u=Ht(c,r,s),p=new MutationObserver(()=>{ee(c)?.toggleAttribute("loading",c.root.getAttribute("aria-busy")==="true")});p.observe(c.root,{attributes:!0,attributeFilter:["aria-busy"]}),d.push(()=>p.disconnect()),tt(c,r).fetchAuthorOptions().then(w=>{r.filterData.authors=w,c.repaint()}),r.filterData.loadMoreTags=()=>void js(c,r),js(c,r),r.menu=qo(c.root,{section:f("Show columns"),prefix:o,items:Yn(u),isChecked:w=>!Se(r,n).has(w),onToggle:w=>{const $=Se(r,n);$.has(w)?$.delete(w):$.add(w);const T=window.wp?.os;T&&typeof T.updateOsSettings=="function"&&T.updateOsSettings({[n]:Array.from($).sort()},{windowId:c.windowId}),c.repaint()}}),d.push(()=>r.menu?.dispose());const h=window.wp?.os;if(h&&typeof h.subscribeOsSettings=="function"){let w=Array.from(Se(r,n)).sort().join(",");d.push(h.subscribeOsSettings(()=>{const $=Rs(n),T=Array.from($).sort().join(",");T!==w&&(w=T,r.hidden=$,c.repaint(),r.menu?.refresh())}))}h&&typeof h.subscribe=="function"&&d.push(h.subscribe("os.term.changed",w=>{w?.taxonomy==="category"&&(Et(u),Tn(u))}));const g=()=>c.repaint();document.addEventListener("os-mode-changed",g),d.push(()=>document.removeEventListener("os-mode-changed",g));const m=Rt(c,r),b=window.wp?.hooks;return b&&typeof b.doAction=="function"&&b.doAction(jn,m),document.dispatchEvent(new CustomEvent("os-posts-window-opened",{detail:m})),()=>{r.disposed=!0;for(const w of d)w();r.feed.dispose(),r.atlas?.(),r.canvases.categories?.(),r.canvases.tags?.(),r.canvases={categories:null,tags:null},Et(u)}},updated:c=>{Ci(c.root);const r=ee(c);if(!r)return;c.loading&&r.setAttribute("loading","");const d=c.ui(qt);d.stats?.sync(),d.feed.observe(c.root);const{state:u,data:p}=c,h=Ht(c,d,s),g=`${Array.from(Se(d,n)).sort().join(",")}|${Ti(d.filterData)}`;g!==d.columnsKey&&(d.columnsKey=g,d.cellCache.clear(),d.table.invalidateColumns());const m=d.feed.items;d.table.sync({table:r,rows:m,listKey:[u.perPage,u.search,u.status,u.orderby,u.order,u.author.join(","),u.tag.join(",")].join("|"),fingerprint:Ei(m),columns:w=>(d.cellCache.clear(),Pt(h,d.cellCache,d.filterData,w,Se(d,n))),wire:()=>Mi(c,d,r),onSelection:w=>{d.selected=w.length}}).dataChanged&&(m.some(w=>w.id===d.desk.focused)||(d.desk.focused=null),queueMicrotask(()=>{d.disposed||c.repaint()}),d.cellCache.clear(),Ls(h,m),p&&li(p))}})}function Xs(o){return Math.hypot(o.b.x-o.a.x,o.b.y-o.a.y)}function Ys(o){return{x:(o.a.x+o.b.x)/2,y:(o.a.y+o.b.y)/2}}function Li(o,e,t,s){const n=Xs(e),a=Xs(t),i=n>0&&a>0?a/n:1,l=Math.max(s.min,Math.min(s.max,o.scale*i)),c=Ys(e),r=Ys(t),d=(c.x-o.x)/o.scale,u=(c.y-o.y)/o.scale;return{scale:l,x:r.x-d*l,y:r.y-u*l}}function zi(o,e){const t=o.ownerDocument.defaultView,s=new AbortController,n=new Map,a=o.style.touchAction;o.style.touchAction="none";let i=!1,l=0;const c=u=>{const p=o.getBoundingClientRect();return{x:u.clientX-p.left,y:u.clientY-p.top}},r={capture:!0,passive:!1,signal:s.signal};t.addEventListener("pointerdown",u=>{u.pointerType!=="touch"||!u.composedPath().includes(o)||(n.set(u.pointerId,c(u)),n.size>=2&&(u.preventDefault(),i||(i=!0,e.start())))},r),t.addEventListener("pointermove",u=>{if(!n.has(u.pointerId))return;if(!i){n.set(u.pointerId,c(u));return}const[p,h]=n.values();if(n.set(u.pointerId,c(u)),u.preventDefault(),h){const[g,m]=n.values();e.write(Li(e.read(),{a:p,b:h},{a:g,b:m},e.bounds))}},r);const d=u=>{n.delete(u.pointerId)&&i&&(u.preventDefault(),n.size||(i=!1,l=performance.now()+300,e.end()))};return t.addEventListener("pointerup",d,r),t.addEventListener("pointercancel",d,r),t.addEventListener("lostpointercapture",d,r),t.addEventListener("click",u=>{u.detail>0&&(i||performance.now()<l)&&u.composedPath().includes(o)&&(u.preventDefault(),u.stopImmediatePropagation())},r),{active:()=>i,dispose:()=>{s.abort(),n.clear(),i&&e.end(),i=!1,o.style.touchAction=a}}}const Zs="__openStationSharedStores";function Pi(){const o=window;let e=o[Zs];return e||(e=new Map,o[Zs]=e),e}function Ri(o,e){const t=Pi();let s=t.get(o);s||(s={state:e(),listeners:new Set,rebuild:e},t.set(o,s));const n={get state(){return s.state},set state(a){s.state=a},getState(){return s.state},notify(){for(const a of Array.from(s.listeners))try{a(s.state)}catch(i){console.error(`[desktop-mode/shared-store:${o}] subscriber threw:`,i)}},subscribe(a){return s.listeners.add(a),()=>{s.listeners.delete(a)}},setState(a){const i=s.state;if(typeof i!="object"||i===null){console.warn(`[desktop-mode/shared-store:${o}] setState called on a primitive store; use the state setter instead.`);return}Object.assign(i,a),n.notify()},reset(){const a=s.rebuild(),i=s.state;if(typeof i=="object"&&i!==null&&typeof a=="object"&&a!==null){const l=i;for(const c of Object.keys(l))delete l[c];Object.assign(l,a)}else s.state=a;s.listeners.clear()}};return n}const Js=Object.freeze({top:0,right:0,bottom:0,left:0});function Ni(o,e,t,s){return{left:o,top:e,width:t,height:s,right:o+t,bottom:e+s}}function Di(o,e){const t=n=>Math.min(Math.max(0,e.width),Math.max(0,n)),s=n=>Math.min(Math.max(0,e.height),Math.max(0,n));return{top:s(o.top-e.top),bottom:s(e.bottom-o.bottom),left:t(o.left-e.left),right:t(e.right-o.right)}}const Oi="os/work-area";function qi(){return{insets:{...Js},rect:{x:0,y:0,width:0,height:0},viewport:{x:0,y:0,width:0,height:0},area:{width:0,height:0}}}const Hi=Ri(Oi,()=>({snapshot:qi(),installed:!1,areaEl:null}));function Fi(o){const{installed:e,snapshot:t}=Hi.state;if(!e)return{...Js};const s=t.viewport;return Di(Ni(s.x,s.y,s.width,s.height),o.getBoundingClientRect())}const st=170,Ui=()=>({pixiInteractionAt:0,pinchUntil:0,lastFocusChange:0,panMovedDist:0,panActive:!1,panStart:null}),ne=o=>performance.now()<o.pinchUntil;function ve(o,e){e.stopPropagation?.(),o.pixiInteractionAt=performance.now()}function Qs(o,e){return!o||!e?.global?1/0:Math.hypot(e.global.x-o.x,e.global.y-o.y)}function Wi(o,e,t){const s=performance.now();return ne(o)||s-o.lastFocusChange<250||s-o.pixiInteractionAt<250||o.panMovedDist>4?!1:e.target===t}function Vi(o,e,t){const s={targetScale:o.scale.x,targetWorldX:o.x,targetWorldY:o.y,ease(){const l=s.targetScale-o.scale.x,c=s.targetWorldX-o.x,r=s.targetWorldY-o.y;(Math.abs(l)>5e-4||Math.abs(c)>.5||Math.abs(r)>.5)&&(o.scale.set(o.scale.x+l*.22),o.x+=c*.22,o.y+=r*.22)},stageToWorld(i){return{x:(i.x-o.x)/o.scale.x,y:(i.y-o.y)/o.scale.y}},pan(i,l){o.x+=i,o.y+=l,s.targetWorldX+=i,s.targetWorldY+=l},fitToView(i,l={}){const c=l.padding??90,r=l.animate??!1,d=e.getBoundingClientRect(),u=Fi(e),p=u.left,h=u.top,g=Math.max(0,d.width-u.left-u.right),m=Math.max(0,d.height-u.top-u.bottom);if(!i||g===0||m===0){const z=p+g/2,N=h+m/2;s.targetScale=1,s.targetWorldX=z,s.targetWorldY=N,r||(o.x=z,o.y=N,o.scale.set(1));return}const b=Math.max(1,i.maxX-i.minX),w=Math.max(1,i.maxY-i.minY),$=(g-c*2)/b,T=(m-c*2)/w,R=Math.max(.2,Math.min(1.5,Math.min($,T))),O=(i.minX+i.maxX)/2,I=(i.minY+i.maxY)/2,x=p+g/2-O*R,E=h+m/2-I*R;s.targetScale=R,s.targetWorldX=x,s.targetWorldY=E,r||(o.scale.set(R),o.x=x,o.y=E)},frameOn(i,l){const c=e.getBoundingClientRect();if(c.width<=0||c.height<=0)return!1;const r=st+70,d=c.width*.85/(2*r),u=c.height*.85/(2*r),p=Math.max(.5,Math.min(1.6,Math.min(d,u)));return s.targetScale=p,s.targetWorldX=c.width/2-i*p,s.targetWorldY=c.height/2-l*p,!0},dispose(){e.removeEventListener("wheel",n),a.dispose()}};function n(i){i.preventDefault();const c=Math.exp(-i.deltaY*8e-4),r=s.targetScale,d=Math.max(.3,Math.min(2.5,r*c));if(Math.abs(d-r)<5e-4)return;const u=e.getBoundingClientRect(),p=i.clientX-u.left,h=i.clientY-u.top,g=(p-s.targetWorldX)/r,m=(h-s.targetWorldY)/r;s.targetScale=d,s.targetWorldX=p-g*d,s.targetWorldY=h-m*d}const a=zi(e,{read:()=>({x:o.x,y:o.y,scale:o.scale.x}),write:i=>{o.scale.set(i.scale),o.x=i.x,o.y=i.y,s.targetScale=i.scale,s.targetWorldX=i.x,s.targetWorldY=i.y},bounds:{min:.2,max:2.5},start:()=>t?.start(),end:()=>t?.end()});return e.addEventListener("wheel",n,{passive:!1}),s}function Bi(o,e,t,s){let n=!1,a=0,i=0;const l=24,c=80;let r=null;const d=()=>{const p=t.getBoundingClientRect();e.renderer.resize(p.width,p.height),e.stage.hitArea=new o.Rectangle(0,0,p.width,p.height),!n&&p.width>0&&p.height>0&&(n=!0,a=p.width,i=p.height,s.onFirstFit(),t.classList.remove("is-loading")),r!==null&&window.clearTimeout(r),r=window.setTimeout(()=>{r=null;const h=t.getBoundingClientRect();(Math.abs(h.width-a)>=l||Math.abs(h.height-i)>=l)&&(a=h.width,i=h.height,s.onSettle())},c),e.render(),s.onResize?.()},u=new ResizeObserver(d);return u.observe(t),()=>{u.disconnect(),r!==null&&window.clearTimeout(r)}}const Ee='-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',Ae=4;function Te(o,e){return o.length>e?o.slice(0,e-1)+"…":o}async function ji(o,e){const t=window.wp?.os;if(!t||typeof t.loadModules!="function")return o.textContent=`${e} ${f("Shell modules API missing.")}`,null;try{await t.loadModules(["pixijs"])}catch{return o.textContent=e,null}const s=window.PIXI;return s||(o.textContent=e),s??null}async function Ki(o,e,t){const s=new o.Application;await s.init({resizeTo:e,backgroundAlpha:0,antialias:!0,autoDensity:!0,resolution:Math.min(window.devicePixelRatio||1,2)}),e.appendChild(s.canvas),s.canvas.classList.add(t);const n=new o.Container;return n.x=e.clientWidth/2,n.y=e.clientHeight/2,s.stage.addChild(n),s.stage.eventMode="static",s.stage.hitArea=new o.Rectangle(0,0,e.clientWidth,e.clientHeight),{app:s,world:n}}function Gi(o,e,t){try{o.ticker?.stop()}catch{}try{o.destroy({removeView:!0},{children:!0})}catch{}e.replaceChildren(),e.classList.remove(...t)}function ot(o=document.body){try{const e=getComputedStyle(o),t=e.getPropertyValue("--os-ui-accent").trim()||e.getPropertyValue("--wp-admin-theme-color").trim();if(!t)return 210;const s=document.createElement("span");s.style.color=t,o.appendChild(s);const n=getComputedStyle(s).color;s.remove();const a=n.match(/\d+/g);return!a||a.length<3?210:Xi(parseInt(a[0],10),parseInt(a[1],10),parseInt(a[2],10))}catch{return 210}}function Xi(o,e,t){const s=o/255,n=e/255,a=t/255,i=Math.max(s,n,a),l=Math.min(s,n,a),c=i-l;if(c===0)return 210;let r;switch(i){case s:r=(n-a)/c+(n<a?6:0);break;case n:r=(a-s)/c+2;break;default:r=(s-n)/c+4;break}return Math.round(r*60)}function eo(o,e,t){const s=e/100,n=t/100,a=(1-Math.abs(2*n-1))*s,i=o/60,l=a*(1-Math.abs(i%2-1));let c=0,r=0,d=0;i<1?(c=a,r=l):i<2?(c=l,r=a):i<3?(r=a,d=l):i<4?(r=l,d=a):i<5?(c=l,d=a):(c=a,d=l);const u=n-a/2;return Math.round((c+u)*255)*65536+Math.round((r+u)*255)*256+Math.round((d+u)*255)}function Yi(o,e){const t=Math.floor(o/65536)%256,s=Math.floor(o/256)%256,n=o%256,a=i=>Math.round(i*(1+e));return a(t)*65536+a(s)*256+a(n)}function to(o){return`#${o.toString(16).padStart(6,"0")}`}function Zi(o){const e=document.createElement("div");return e.innerHTML=o,e.textContent||e.innerText||""}const Ji=H` .os-term-canvas__intro{padding:22px 24px 18px;background:var( --os-ui-surface,#fff );flex-shrink:0}.os-term-canvas__intro h2{font-size:28px;font-weight:500;letter-spacing:-0.035em;margin:0 0 6px;color:var( --os-ui-fg,#1d2327 )}.os-term-canvas__intro p{font-size:12px;margin:0;color:var( --os-ui-fg-muted,#646970 )}.os-term-directory{position:absolute;inset-block:12px;inset-inline-start:12px;inline-size:min( 300px,calc( 100% - 24px ) );z-index:5;box-sizing:border-box;display:flex;flex-direction:column;gap:10px;padding:16px;border:1px solid var( --os-ui-border,#dcdcde );border-radius:12px;background:var( --os-ui-surface,#fff );color:var( --os-ui-fg,#1d2327 );box-shadow:var( --os-ui-shadow-sheet,0 6px 24px rgba( 0,0,0,0.12 ) )}.os-term-directory header{display:flex;align-items:center;justify-content:space-between}.os-term-directory h3{font-size:17px;margin:0;color:inherit}.os-term-directory__count{font-size:11px;margin:0;color:var( --os-ui-fg-muted,#646970 )}.os-term-directory__items{overflow:auto;min-block-size:0;flex:1;display:flex;flex-direction:column;gap:4px}.os-term-directory__items os-button{flex-shrink:0}.os-term-directory__items os-button::part( button ){inline-size:100%;justify-content:space-between;text-align:start;white-space:normal}.os-term-directory__items small{color:var( --os-ui-fg-muted,#646970 );white-space:nowrap;font-size:10px}@container content-desk (max-width:540px){.os-term-canvas__intro{padding:14px}.os-term-canvas__intro h2{font-size:23px}.os-term-canvas__intro p{display:none}}.os-term-canvas__toolbar{flex-wrap:wrap;padding:12px 20px}.os-term-canvas__toolbar>button,.os-term-directory__toggle{flex-shrink:0;white-space:nowrap}.os-term-canvas__toolbar .os-term-canvas__hint{flex-basis:100%;font-size:11px}.os-term-canvas__search-input{border-radius:8px;border:1px solid var( --os-ui-border,#dcdcde );padding:8px 10px;background:var( --os-ui-surface,#fff );color:var( --os-ui-fg,#1d2327 )}`,B="os-term-canvas";function Ft(o,e){const t=document.createElement("button");return t.type="button",t.className=`${B}__btn${o?` ${B}__btn--${o}`:""}`,t.textContent=e,t}function Qi(o,e,t){o.replaceChildren(),o.classList.add(B,e);const s=document.createElement("style");s.textContent=Ji.cssText,o.appendChild(s);const n=document.createElement("header");n.className="os-term-canvas__intro";const a=document.createElement("h2");a.textContent=f(e==="os-mindmap"?"Give your ideas a home.":"Find the threads.");const i=document.createElement("p");i.textContent=f(e==="os-mindmap"?"Build branches for your stories. Choose a category to explore its posts.":"See what connects your stories. Choose a tag to follow the conversation."),n.append(a,i),o.appendChild(n);const l=document.createElement("div");l.className=`${B}__toolbar`;const c=t.buttons.map(b=>{const w=Ft(b.variant??"",""),$=document.createElement("span");return $.className=`dashicons ${b.icon}`,$.setAttribute("aria-hidden","true"),w.replaceChildren($,document.createTextNode(b.label)),b.title&&(w.title=b.title),l.appendChild(w),w}),r=document.createElement("div");r.className=`${B}__search`;const d=document.createElement("input");d.type="search",d.className=`${B}__search-input`,d.placeholder=t.searchPlaceholder,d.setAttribute("aria-label",t.searchAria),r.appendChild(d);const u=document.createElement("ul");u.className=`${B}__search-results`,u.hidden=!0,r.appendChild(u);const p=document.createElement("span");p.className=`${B}__hint`,p.textContent=t.hint,l.appendChild(r),l.appendChild(p),o.appendChild(l);const h=document.createElement("div");h.className=`${B}__layout`,o.appendChild(h);const g=document.createElement("div");g.className=`${B}__stage is-loading`,h.appendChild(g);const m=document.createElement("aside");return m.className=`${B}__sidebar`,h.appendChild(m),{toolbar:l,stage:g,sidebar:m,buttons:c,searchWrap:r,searchInput:d,searchResults:u}}function ea(o,e){const{searchInput:t,searchResults:s,searchWrap:n}=o;let a=[],i=0;const l=()=>{s.querySelectorAll(`.${B}__search-result`).forEach((g,m)=>{const b=m===i;g.classList.toggle("is-active",b),b&&g.scrollIntoView({block:"nearest"})})},c=()=>{t.value="",s.hidden=!0,s.replaceChildren(),a=[],i=0},r=g=>{c(),e.select(g)},d=()=>{const g=t.value.trim().toLowerCase();if(g.length===0){s.hidden=!0,s.replaceChildren(),a=[],i=0;return}a=e.matches(g).sort((m,b)=>b.count-m.count).slice(0,10),i=0,s.replaceChildren(),a.forEach((m,b)=>{const w=document.createElement("li"),$=document.createElement("button");$.type="button",$.className=`${B}__search-result`,b===0&&$.classList.add("is-active");const T=document.createElement("span");T.className=`${B}__search-title`,T.textContent=m.name||`#${m.id}`;const R=document.createElement("span");R.className=`${B}__search-meta`,R.textContent=V(de("%d post","%d posts",m.count),m.count),$.appendChild(T),$.appendChild(R),$.addEventListener("mousedown",O=>{O.preventDefault(),r(m)}),$.addEventListener("mouseenter",()=>{i=b,l()}),w.appendChild($),s.appendChild(w)}),s.hidden=a.length===0},u=g=>{if(g.key==="ArrowDown"||g.key==="ArrowUp"||g.key==="Enter"){if(a.length===0)return;g.preventDefault(),g.key==="ArrowDown"?(i=Math.min(i+1,a.length-1),l()):g.key==="ArrowUp"?(i=Math.max(i-1,0),l()):r(a[i])}else g.key==="Escape"&&c()},p=()=>{setTimeout(()=>{s.hidden=!0},120)},h=g=>{n.contains(g.target)||(s.hidden=!0)};return t.addEventListener("input",d),t.addEventListener("focus",d),t.addEventListener("keydown",u),t.addEventListener("blur",p),document.addEventListener("click",h),()=>{t.removeEventListener("input",d),t.removeEventListener("focus",d),t.removeEventListener("keydown",u),t.removeEventListener("blur",p),document.removeEventListener("click",h)}}const ta=H` ${_e} ${vs} ${ys} ${xs} ${ks} ${Cs}:host{display:inline-flex}:host( [ fill-cell ] ){display:flex;width:100%}button{appearance:none;display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:var( --os-ui-button-padding,6px 12px );border-radius:var( --os-ui-button-border-radius,6px );font:inherit;font-weight:500;cursor:pointer;transition:background-color var( --_holo-t ) ease,color var( --_holo-t ) ease,border-color var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease,transform 80ms ease;background:var( --os-ui-button-bg,transparent );background-image:var( --os-ui-button-bg-image,none );background-repeat:var( --os-ui-button-bg-image-repeat,repeat );background-size:var( --os-ui-button-bg-image-size,auto );background-position:var( --os-ui-button-bg-image-position,center );color:var( --os-ui-button-fg,var( --os-ui-fg,#1d2327 ) );border:var( --os-ui-button-border,1px solid var( --os-ui-border,#c3c4c7 ) )}:host( [ fill-cell ] ) button{width:100%;min-height:var( --os-ui-button-min-height,44px )}button:disabled{opacity:0.5;cursor:not-allowed}button:active:not(:disabled ){transform:translateY( 1px )}button:hover:not(:disabled ){background-color:var( --os-ui-button-bg-hover,var( --os-ui-hover,rgba( 0,0,0,0.04 ) ) )}button:focus-visible{outline:none;box-shadow:var( --_holo-focus )}button:disabled::before,button:disabled::after,button:disabled .os-holo-glint{opacity:0 !important}:host( [ variant='primary' ] ) button{background-color:var( --os-ui-button-bg,var( --wp-admin-theme-color,#2271b1 ) );color:var( --os-ui-button-fg,var( --os-ui-accent-ink,var( --os-ui-fg-on-accent,#fff ) ) );border:var( --os-ui-button-border,1px solid transparent )}:host( [ variant='primary' ] ) button:hover:not(:disabled ){filter:brightness( 1.06 );background-color:var( --os-ui-button-bg,var( --wp-admin-theme-color,#2271b1 ) );box-shadow:var( --_holo-glow )}:host( [ variant='holo' ] ) button{background-color:transparent;background-image:var( --_holo-fill );background-size:220% 220%;background-position:22% 28%;background-repeat:no-repeat;color:var( --os-ui-button-fg,var( --_holo-ink ) );border:var( --os-ui-button-border,1px solid transparent );box-shadow:var( --_holo-glow );font-weight:600;transition:background-position var( --_holo-t ) ease,box-shadow var( --_holo-t ) ease,filter var( --_holo-t ) ease,transform 80ms ease}:host( [ variant='holo' ] ) button:hover:not(:disabled ){background-color:transparent;background-position:74% 66%;box-shadow:var( --_holo-glow-strong )}:host( [ variant='holo' ] ) button:active:not(:disabled ){background-position:88% 82%;filter:brightness( 0.94 )}:host( [ variant='holo' ] ) button::before{display:none}@media ( prefers-reduced-motion:reduce ){:host( [ variant='holo' ] ) button:hover:not(:disabled ),:host( [ variant='holo' ] ) button:active:not(:disabled ){background-position:22% 28%}}:host( [ variant='secondary' ] ) button{background-color:var( --os-ui-button-bg,var( --os-ui-hover,rgba( 0,0,0,0.06 ) ) );color:var( --os-ui-button-fg,var( --os-ui-fg,#1d2327 ) );border:var( --os-ui-button-border,1px solid transparent )}:host( [ variant='secondary' ] ) button:hover:not(:disabled ){background-color:var( --os-ui-button-bg-hover,var( --os-ui-hover,rgba( 0,0,0,0.1 ) ) )}:host( [ variant='danger' ] ) button{background-color:var( --os-ui-button-bg,transparent );color:var( --os-ui-button-fg,var( --os-ui-danger,#d63638 ) );border:var( --os-ui-button-border,1px solid currentColor )}:host( [ variant='danger' ] ) button:hover:not(:disabled ){background-color:var( --os-ui-danger,#d63638 );color:var( --os-ui-fg-on-accent,#fff )}:host( [ variant='danger' ] ) button::after{display:none}:host( [ variant='link' ] ) button{background-color:transparent;background-image:none;color:var( --os-ui-button-fg,var( --wp-admin-theme-color,#2271b1 ) );border:0;padding:0;text-decoration:underline}:host( [ variant='link' ] ) button::before,:host( [ variant='link' ] ) button::after,:host( [ variant='link' ] ) .os-holo-glint,:host( [ variant='link' ] ) .os-holo-ring{display:none}:host( [ variant='link' ] ) button:active:not(:disabled ){transform:none}:host( [ busy ] ) button{pointer-events:none;opacity:0.75}:host( [ variant='holo' ][ busy ] ) button{animation:os-holo-drift 12s ease-in-out infinite}@media ( prefers-reduced-motion:reduce ){:host( [ variant='holo' ][ busy ] ) button{animation:none}}.os-button__spinner{box-sizing:border-box;display:inline-block;width:12px;height:12px;border:2px solid currentColor;border-right-color:transparent;border-radius:50%;animation:os-button-spin 0.6s linear infinite;flex-shrink:0}@keyframes os-button-spin{to{transform:rotate( 360deg )}}`,je=class je extends X{render(){const e=this.disabled!==null,t=this.busy!==null,s=this.type||"button";return k`
			<button
				part="button"
			...
			>
				${t?k`<span class="os-button__spinner" aria-hidden="true"></span>`:""}
				<slot></slot>
				<!--
		...
		`}};je.props=["variant","disabled","type","busy","fill-cell"],je.styles=[ta],je.help=void 0;let Ut=je;Q("os-button",Ut);const sa=H` ${_e} ${ws}:host{--_field-size:var( --os-ui-field-font-size,13px );--_field-radius:var( --os-ui-field-radius,6px );display:flex;flex-direction:column;gap:4px;font-size:var( --_field-size );color:var( --os-ui-fg,#1d2327 );min-width:0}:host( [ hidden ] ){display:none}.os-text-field__label{font-size:12px;color:var( --os-ui-fg-muted,#646970 )}.os-text-field__label--hidden{position:absolute;width:1px;height:1px;margin:-1px;padding:0;border:0;overflow:hidden;white-space:nowrap;clip-path:inset( 50% )}.os-text-field__row{position:relative;display:flex;align-items:center;width:100%}input{appearance:none;-webkit-appearance:none;display:block;width:100%;min-width:0;box-sizing:border-box;padding:7px 10px;background:var( --os-window-bg,#fff );border:1px solid var( --os-ui-border,#dcdcde );border-radius:var( --_field-radius );font:inherit;font-size:var( --_field-size );color:var( --os-ui-fg,#1d2327 )}.os-text-field__suffix{position:absolute;inset-inline-end:10px;top:50%;transform:translateY( -50% );pointer-events:none;font-size:12px;color:var( --os-ui-fg-muted,#646970 )}.os-text-field__row--has-reveal input{padding-inline-end:36px}:host( [ clearable ] ) input::-webkit-search-cancel-button{-webkit-appearance:none;display:none}.os-text-field__row--has-clear input{padding-inline-end:36px}.os-text-field__row--has-reveal.os-text-field__row--has-clear input{padding-inline-end:68px}.os-text-field__clear{position:absolute;inset-inline-end:0;top:0;bottom:0;width:34px;display:flex;align-items:center;justify-content:center;padding:0;border:none;background:transparent;color:var( --os-ui-fg-muted,#646970 );cursor:pointer;border-radius:0 6px 6px 0}.os-text-field__row--has-reveal .os-text-field__clear{inset-inline-end:34px;border-radius:0}.os-text-field__clear:hover{color:var( --os-ui-accent,#2271b1 )}.os-text-field__clear:focus-visible{outline:none;color:var( --os-ui-accent,#2271b1 );box-shadow:inset 0 0 0 2px var( --os-ui-accent,#2271b1 )}.os-text-field__clear:disabled{opacity:0.45;cursor:default}.os-text-field__reveal{position:absolute;inset-inline-end:0;top:0;bottom:0;width:34px;display:flex;align-items:center;justify-content:center;padding:0;border:none;background:transparent;color:var( --os-ui-fg-muted,#646970 );cursor:pointer;border-radius:0 6px 6px 0;transition:color 0.12s ease}.os-text-field__reveal:hover{color:var( --os-ui-accent,#2271b1 )}.os-text-field__reveal:focus-visible{outline:none;color:var( --os-ui-accent,#2271b1 );box-shadow:inset 0 0 0 2px var( --os-ui-accent,#2271b1 );border-radius:0 6px 6px 0}.os-text-field__reveal:disabled{opacity:0.45;cursor:not-allowed}.os-text-field__input--masked{-webkit-text-security:disc;text-security:disc}@supports not ( ( -webkit-text-security:disc ) or ( text-security:disc ) ){.os-text-field__input--masked{font-family:text-security-disc,"password",monospace;letter-spacing:0.2em}}input:disabled{opacity:0.55;cursor:not-allowed;background:var( --os-ui-hover,rgba( 0,0,0,0.03 ) )}input[ aria-invalid='true' ],input[ aria-invalid='true' ]:hover:not(:disabled ){border-color:var( --os-ui-danger,#d63638 )}input[ aria-invalid='true' ]:focus,input[ aria-invalid='true' ]:focus-visible{border-color:var( --os-ui-danger,#d63638 );box-shadow:0 0 0 1px var( --os-ui-danger,#d63638 ),0 0 0 4px rgba( 214,54,56,0.18 )}input[ type='number' ]::-webkit-inner-spin-button,input[ type='number' ]::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}input[ type='number' ]{-moz-appearance:textfield}`,Ke=class Ke extends X{constructor(){super(...arguments),this._revealed=!1}connectedCallback(){super.connectedCallback(),ht(this)}render(){const e=this.label||"",t=this.hideLabel!==null,s=this.value??"",n=this.placeholder||"",a=this.disabled!==null,i=this.readonly!==null,l=this.autocomplete,c=this.type||"text",r=c==="password";let d=l||"off";r&&(!l||d==="off")&&(d="new-password");const u=this.maxlength,p=this.minlength,h=this.pattern||"",g=this.name||"",m=this.suffix||"",b=this.invalid!==null,w=this.reveal!==null,$=this.clearable!==null,T=c==="password",R=T&&!(w&&this._revealed);let O;T||w&&this._revealed?O="text":O=c;const I=["os-text-field__row",w?"os-text-field__row--has-reveal":"",$?"os-text-field__row--has-clear":""].filter(Boolean).join(" "),x=R?"os-text-field__input os-text-field__input--masked":"os-text-field__input",z=`${this.id||"os-unnamed"}__input`;return k`
			...
			<span class=${I}>
				...
				/>
				${m?k`<span class="os-text-field__suffix">${m}</span>`:k``}
				${$&&s!==""?this._renderClearButton(a):k``}
				${w?this._renderRevealButton(a):k``}
			</span>
		`}_renderClearButton(e){return k`
			<button
				type="button"
		...
		`}_onClear(){this.value="",this.emit("os-input-change",{value:""}),this.emit("os-input-commit",{value:""}),this.shadowRoot?.querySelector("input")?.focus()}_renderRevealButton(e){const t=this._revealed?"Hide":"Show";return k`
			<button
				type="button"
	...
	`}function aa(o,e,t){const s=e.trim().toLocaleLowerCase();return o.filter(n=>(!t||n.count===0)&&pe(n.name).toLocaleLowerCase().includes(s)).sort((n,a)=>pe(n.name).localeCompare(pe(a.name)))}function ra(o,e,t){const s=document.createElement("os-button");s.setAttribute("variant","secondary"),s.textContent=f("Browse topics"),s.className="os-term-directory__toggle",o.toolbar.insertBefore(s,o.searchWrap);const n=document.createElement("section");n.className="os-term-directory",n.setAttribute("aria-label",f("Topic directory")),n.hidden=!0,o.stage.appendChild(n);let a="",i=!1,l=50;const c=()=>s.shadowRoot?.querySelector("button")?.focus(),r=()=>{n.hidden=!0,s.setAttribute("aria-expanded","false"),s.shadowRoot?.querySelector("button")?.setAttribute("aria-expanded","false"),c()},d=()=>{const h=e(),g=aa(h,a,i);ke(k`<header><h3>${f("Your topics")}</h3><os-button variant="ghost" @click=${r}>${f("Close")}</os-button></header>
			<os-text-field label=${f("Find a topic")} value=${a} placeholder=${f("Find a topic…")}
				@os-input-change=${m=>{a=m.detail.value,l=50,d()}}></os-text-field>
			<os-select label=${f("Show topics")} value=${i?"unused":"all"}
				@os-pick=${m=>{i=m.detail.value==="unused",l=50,d()}}>
				<os-option value="all">${f("All topics")}</os-option><os-option value="unused">${f("No posts yet")}</os-option>
			</os-select>
			<p class="os-term-directory__count" role="status">${V(de("%d topic","%d topics",g.length),g.length)}</p>
			<div class="os-term-directory__items">${g.slice(0,l).map(m=>k`<os-button variant="ghost" data-topic-id=${m.id} @click=${()=>{r(),t(m.id)}}><span>${pe(m.name)}</span><small>${V(de("%d post","%d posts",m.count),m.count)}</small></os-button>`)}
				${g.length===0?k`<p>${f("No matching topics. Try another search or filter.")}</p>`:""}
				${g.length>l?k`<os-button variant="secondary" @click=${()=>{l+=50,d()}}>${f("Show more")}</os-button>`:""}
			</div>`,n)},u=()=>{if(!n.hidden){r();return}d(),n.hidden=!1,s.setAttribute("aria-expanded","true"),s.shadowRoot?.querySelector("button")?.setAttribute("aria-expanded","true"),queueMicrotask(()=>n.querySelector("os-text-field")?.shadowRoot?.querySelector("input")?.focus())},p=h=>{h.key==="Escape"&&(h.stopPropagation(),r())};return s.addEventListener("click",u),n.addEventListener("keydown",p),()=>{s.removeEventListener("click",u),n.removeEventListener("keydown",p),s.remove(),n.remove()}}function nt(o){const e=ye(o,"--os-ui-surface","#ffffff",16777215);return{surface:e,raised:ye(o,"--os-ui-surface-elevated","#ffffff",e),fg:ye(o,"--os-ui-fg","#1d2327",e),muted:ye(o,"--os-ui-fg-muted","#50575e",e),faint:ye(o,"--os-ui-fg-faint","#b0b3b8",e),border:ye(o,"--os-ui-border","#dcdcde",e),accent:ye(o,"--os-ui-accent","#2271b1",e)}}function ye(o,e,t,s=16777215){const n=document.createElement("span");n.hidden=!0,n.style.color=`var(${e}, ${t})`,o.append(n);const a=getComputedStyle(n).color;n.remove();const i=a.match(/[\d.]+/g)?.map(Number);if(!i||i.length<3)return parseInt(t.slice(1),16);const l=i[3]??1,c=a.startsWith("color(srgb");return[16,8,0].reduce((r,d,u)=>{const p=i[u]*(c?255:1),h=Math.floor(s/2**d)%256;return r+Math.round(p*l+h*(1-l))*2**d},0)}function la(o,e,t){let s=JSON.stringify([nt(o),t?.()]);const n=()=>{const i=JSON.stringify([nt(o),t?.()]);i!==s&&(s=i,e())},a=new MutationObserver(n);for(let i=o;i;i=i.parentElement)a.observe(i,{attributes:!0,attributeFilter:["style","class"]});return document.addEventListener("os-desktop-theme-changed",n),()=>{a.disconnect(),document.removeEventListener("os-desktop-theme-changed",n)}}function so(o,e){const t=a=>[16,8,0].reduce((i,l,c)=>{const r=Math.floor(a/2**l)%256/255;return i+(r<=.04045?r/12.92:((r+.055)/1.055)**2.4)*[.2126,.7152,.0722][c]},0),s=t(o),n=a=>{const i=t(a);return(Math.max(i,s)+.05)/(Math.min(i,s)+.05)};return n(e.fg)>=n(e.surface)?e.fg:e.surface}const ca=10,da=22,ua=6e4;function pa(o){const{pixi:e,postLayer:t,postChipLayer:s,postEdgeGfx:n,interaction:a,env:i,palette:l}=o,c=new Map,r=new Map,d=new Map;let u=0;const p=new e.Container;p.eventMode="passive",p.visible=!1,t.addChild(p);const h=new e.Graphics,g=new e.Graphics,m=new e.Text({text:"1 / 1",style:{fill:l.muted,fontSize:o.pagerLabelSize,fontFamily:Ee,fontWeight:"600"},...o.pagerTextRes?{resolution:o.pagerTextRes}:{}});m.anchor.set(.5);for(const x of[h,g])x.eventMode="static",x.cursor="pointer",x.hitArea=new e.Circle(0,0,16),x.on("pointerdown",E=>ve(a,E));p.addChild(h),p.addChild(m),p.addChild(g),h.on("pointertap",x=>{ne(a)||(ve(a,x),a.lastFocusChange=performance.now(),I.focusPage>1&&(I.focusPage--,I.load()))}),g.on("pointertap",x=>{ne(a)||(ve(a,x),a.lastFocusChange=performance.now(),I.focusPage<I.focusTotalPages&&(I.focusPage++,I.load()))});function b(x,E,z){x.clear(),x.circle(0,0,14),x.fill({color:z?l.surface:l.raised,alpha:z?.7:1}),x.stroke({color:l.border,width:1,alpha:1});const N=x.children?.[0]??null;if(N)N.text=E,N.style.fill=z?l.faint:l.muted;else{const F=new e.Text({text:E,style:{fill:z?l.faint:l.muted,fontSize:o.pagerGlyphSize,fontFamily:Ee,fontWeight:"600"},...o.pagerTextRes?{resolution:o.pagerTextRes}:{}});F.anchor.set(.5),x.addChild(F)}}function w(){const x=I.focusId===null?null:o.getCenter(I.focusId);if(!x||I.focusTotalPages<=1){p.visible=!1;return}p.visible=!0;const E=I.focusPage<=1,z=I.focusPage>=I.focusTotalPages;b(h,"◀",E),b(g,"▶",z),h.cursor=E?"default":"pointer",g.cursor=z?"default":"pointer",m.text=`${I.focusPage} / ${I.focusTotalPages}`,h.x=-38,g.x=38,h.y=0,g.y=0,m.x=0,m.y=0,p.x=x.x,p.y=x.y+st+60}function $(x,E){x.titleText.style.fill=l.fg;const z=Te(E.title,da);x.titleText.text!==z&&(x.titleText.text=z);const N=9,F=3,U=4,j=6,G=x.titleText.width,q=x.titleText.height,P=N+U*2+j+G+N,D=Math.max(q,U*2)+F*2,v=-P/2,_=-D/2;x.bg.clear(),x.bg.roundRect(v,_,P,D,D/2),x.cachedHover?(x.bg.fill({color:l.raised,alpha:1}),x.bg.stroke({color:E.tone,width:1.5,alpha:1})):(x.bg.fill({color:l.surface,alpha:1}),x.bg.stroke({color:l.border,width:1,alpha:1})),x.dot.clear(),x.dot.circle(v+N+U,0,U),x.dot.fill({color:E.tone,alpha:.85}),x.dot.stroke({color:l.border,width:1}),x.titleText.x=v+N+U*2+j,x.titleText.y=-q/2}function T(x){if(r.has(x.id))return;const E=new e.Container;E.eventMode="static",E.cursor="pointer",E.alpha=0;const z=new e.Graphics,N=new e.Graphics,F=new e.Text({text:x.title,style:{fill:l.fg,fontSize:o.chipFontSize,fontFamily:Ee,fontWeight:"500"},resolution:o.chipTextRes});E.addChild(z),E.addChild(N),E.addChild(F);const U={container:E,bg:z,dot:N,titleText:F,cachedHover:!1};r.set(x.id,U),s.addChild(E),E.on("pointerdown",j=>ve(a,j)),E.on("pointertap",()=>{ne(a)||(i.leaveFullscreen(),i.openUrl(x.editUrl,x.title,"dashicons-admin-post"),o.onOpenPost())}),E.on("pointerover",()=>{U.cachedHover=!0,$(U,x)}),E.on("pointerout",()=>{U.cachedHover=!1,$(U,x)}),$(U,x)}function R(x){I.clear();const E=I.focusId===null?null:o.getCenter(I.focusId);if(!E)return;const z=x.length,N=st+Math.max(0,z-8)*6;x.forEach((F,U)=>{const j=2*Math.PI/Math.max(1,z)*U-Math.PI/2,G=new e.Graphics;t.addChild(G);const q={id:F.id,title:F.title,editUrl:F.editUrl,angle:j,r:N,x:E.x,y:E.y,tx:E.x+Math.cos(j)*N,ty:E.y+Math.sin(j)*N,gfx:G,tone:E.tone};c.set(F.id,q),T(q)}),w()}function O(x,E){I.focusTotalPages=x.totalPages,Number.isFinite(x.realTotal)&&x.realTotal>=0&&o.onCountReconciled(E,x.realTotal),R(x.items)}const I={repaintTheme(){const x=I.focusId===null?null:o.getCenter(I.focusId)?.tone;m.style.fill=l.muted,w();for(const E of c.values()){x!=null&&(E.tone=x);const z=r.get(E.id);z&&$(z,E)}},focusId:null,focusPage:1,focusTotalPages:1,async load(){if(I.focusId===null)return;const x=++u,E=I.focusId,z=`${E}:${I.focusPage}`,N=d.get(z);if(N&&performance.now()-N.fetchedAt<ua){O(N,E);return}try{const F=await i.client.fetchTermPosts(o.param,E,I.focusPage,ca);if(x!==u||I.focusId!==E)return;const U=i.extra.editPostUrlBase??"",j=U.includes("?")?"&":"?",G={items:F.items.map(q=>({id:q.id,title:Zi(q.title),editUrl:`${U}${j}post=${q.id}&action=edit`})),totalPages:F.totalPages,realTotal:F.total,fetchedAt:performance.now()};d.set(z,G),O(G,E)}catch(F){i.toast(f("Couldn’t load posts:"),F)}},clear(){for(const x of c.values())t.removeChild(x.gfx),x.gfx.destroy();c.clear();for(const x of r.values())s.removeChild(x.container),x.container.destroy({children:!0});r.clear(),n.clear(),p.visible=!1},invalidate(){u++},ease(){for(const x of c.values())x.x+=(x.tx-x.x)*.18,x.y+=(x.ty-x.y)*.18,x.gfx.x=x.x,x.gfx.y=x.y},drawEdges(){n.clear();const x=I.focusId===null?null:o.getCenter(I.focusId);if(x)for(const E of c.values())n.moveTo(x.x,x.y),n.lineTo(E.x,E.y),n.stroke({color:x.tone,width:1,alpha:.35})},syncChips(x){for(const E of c.values()){const z=r.get(E.id);z&&(z.container.x=E.x,z.container.y=E.y,z.container.scale.set(x),z.container.alpha<1&&(z.container.alpha=Math.min(1,z.container.alpha+.18)))}}};return I}const Vt=st+130;async function oo(o,e,t){const s=await ji(o,t.unavailable);if(!s)return null;const n=s,a=Qi(o,t.modifier,t.chrome),{stage:i,sidebar:l}=a,{app:c,world:r}=await Ki(n,i,`${B}__canvas`),d={};for(const P of t.layers){const D=new n.Container;d[P]=D,r.addChild(D)}const u=new n.Graphics;d.postEdge.addChild(u);const p=nt(i),h=Ui();let g=null;const m=Vi(r,i,{start:()=>{h.pinchUntil=1/0,h.panActive=!1,h.panStart=null,g?.cancelGesture()},end:()=>{h.pinchUntil=performance.now()+300}});let b=null,w=null,$=performance.now(),T=null,R=null,O=null,I=!1;const x=pa({palette:p,pixi:n,postLayer:d.post,postChipLayer:d.postChip,postEdgeGfx:u,env:e,param:t.taxonomy,interaction:h,...t.fan,getCenter:P=>g?.center(P)??null,onCountReconciled:(P,D)=>g?.countReconciled(P,D),onOpenPost:()=>q.closeFocus()}),z=la(i,()=>{Object.assign(p,nt(i)),g?.themeChanged(),x.repaintTheme(),c.render()}),N=()=>document.hidden||i.clientWidth===0||i.clientHeight===0,F=()=>{if(w=null,I||!g||N())return;const P=performance.now(),D=Math.min(50,P-$);$=P,m.ease(),g.frame(D),w=requestAnimationFrame(F)},U=()=>{I||w!==null||!g||N()||($=performance.now(),w=requestAnimationFrame(F))},j=()=>U();c.stage.on("pointerdown",P=>{if(ne(h))return;const D=P;h.panActive=!0,h.panStart={x:D.global.x,y:D.global.y},h.panMovedDist=0}),c.stage.on("pointermove",P=>{if(ne(h))return;const D=P;if(!g?.pointerMove(D,m.stageToWorld(D.global))&&h.panActive&&h.panStart){const v=D.global.x-h.panStart.x,_=D.global.y-h.panStart.y;m.pan(v,_),h.panMovedDist+=Math.sqrt(v*v+_*_),h.panStart={x:D.global.x,y:D.global.y}}});const G=async P=>{ne(h)||await g?.pointerUp(P),h.panActive=!1,h.panStart=null};c.stage.on("pointerup",P=>void G(P)),c.stage.on("pointerupoutside",P=>void G(P)),c.canvas.addEventListener("click",P=>{Wi(h,P,c.canvas)&&!g?.dragging()&&x.focusId!==null&&q.closeFocus()});const q={palette:p,pixi:n,app:c,world:r,layers:d,postEdgeGfx:u,chrome:a,stage:i,sidebar:l,interaction:h,camera:m,fan:x,terms:[],nudge:null,async focusOn(P){if(x.focusId===P){q.closeFocus();return}const D=x.focusId!==null;x.focusId=P,x.focusPage=1,h.lastFocusChange=performance.now();const v=g?.center(P)??null;v&&(D||(b={scale:m.targetScale,x:m.targetWorldX,y:m.targetWorldY}),m.frameOn(v.x,v.y),q.nudge={x:v.x,y:v.y,radius:Vt},g?.focusOpened?.(v)),g?.focusChanged(),await x.load()},closeFocus(){x.focusId=null,h.lastFocusChange=performance.now(),x.invalidate(),q.nudge=null,g?.focusClosed?.(),b&&(m.targetScale=b.scale,m.targetWorldX=b.x,m.targetWorldY=b.y,b=null),x.clear(),g?.focusChanged()},recenter(){const P=x.focusId===null?null:g?.center(x.focusId)??null;P&&m.frameOn(P.x,P.y)||m.fitToView(g?.bounds()??null,{animate:!0})},syncEmptyHint(P){const D=i.querySelector(`.${B}__empty`);if(P&&!D){const v=document.createElement("div");v.className=`${B}__empty`,v.textContent=t.emptyHint,i.appendChild(v)}else!P&&D&&D.remove()},async refreshCounts(){if(q.terms.length!==0)try{const P=await e.client.fetchTermCounts(t.restTaxonomy,q.terms.map(v=>v.id));let D=!1;q.terms=q.terms.map(v=>{const _=P[String(v.id)];return typeof _=="number"&&_!==v.count?(D=!0,{...v,count:_}):v}),D&&!I&&g?.countsChanged()}catch{}},start(P){g=P,T=Bi(n,c,i,{onFirstFit:()=>m.fitToView(P.bounds()),onSettle:()=>q.recenter(),onResize:U}),document.addEventListener("visibilitychange",j),R=ra(a,()=>q.terms,D=>void q.focusOn(D)),O=ea(a,{matches:P.search,select:D=>void q.focusOn(D.id)}),U(),q.refreshCounts()},teardown(){I=!0,w!==null&&(cancelAnimationFrame(w),w=null),document.removeEventListener("visibilitychange",j),T?.(),O?.(),R?.(),z(),m.dispose(),Gi(c,o,[B,t.modifier])}};try{q.terms=await e.client.fetchAllTerms(t.taxonomy)}catch(P){e.toast(t.loadFailed,P)}return q}const Bt=18;function ha(o,e,t,s){const n=new Map,a=s.palette;function i(r,d){const u=s.isFocused(d.id),p=Te(d.name,Bt),h=String(d.count);r.nameText.text!==p&&(r.nameText.text=p),r.countText.text!==h&&(r.countText.text=h),r.cachedName=p,r.cachedCount=d.count,r.cachedFocused=u,r.cachedColor=d.color;const g=9,m=3,b=5,w=5,$=2,T=r.nameText.width,R=r.nameText.height,O=r.countText.width,I=r.countText.height,x=Math.max(18,O+w*2),E=I+$*2,z=g+T+b+x+g,N=Math.max(R,E)+m*2,F=-z/2;r.bg.clear(),r.bg.roundRect(F,0,z,N,N/2),u?(r.bg.fill(a.raised),r.bg.stroke({color:d.color,width:2})):r.cachedHover?(r.bg.fill({color:a.raised,alpha:1}),r.bg.stroke({color:d.color,width:1.5,alpha:1})):(r.bg.fill({color:a.surface,alpha:1}),r.bg.stroke({color:a.border,width:1,alpha:1})),r.nameText.x=F+g,r.nameText.y=(N-R)/2,r.nameText.style.fill=a.fg;const U=F+g+T+b,j=(N-E)/2;r.countBg.clear(),r.countBg.roundRect(U,j,x,E,E/2),r.countBg.fill(d.color),r.countText.style.fill=so(d.color,a),r.countText.x=U+(x-O)/2,r.countText.y=j+(E-I)/2}function l(r){const d=n.get(r.id);if(d)return d;const u=new o.Container;u.eventMode="static",u.cursor="pointer";const p=new o.Graphics,h=new o.Text({text:Te(r.name,Bt),style:{fill:a.fg,fontSize:14,fontFamily:Ee,fontWeight:"600"},resolution:Ae}),g=new o.Graphics,m=new o.Text({text:String(r.count),style:{fill:a.fg,fontSize:12,fontFamily:Ee,fontWeight:"700"},resolution:Ae});u.addChild(p),u.addChild(h),u.addChild(g),u.addChild(m);const b={container:u,bg:p,nameText:h,countBg:g,countText:m,cachedName:"",cachedCount:-1,cachedFocused:!1,cachedHover:!1,cachedColor:-1};return n.set(r.id,b),e.addChild(u),u.on("pointerdown",w=>ve(t,w)),u.on("pointertap",()=>{ne(t)||s.onTap(r.id)}),u.on("pointerover",()=>{b.cachedHover=!0,i(b,r)}),u.on("pointerout",()=>{b.cachedHover=!1,i(b,r)}),b}const c={relayout(r){i(l(r),r)},destroy(r){const d=n.get(r);d&&(e.removeChild(d.container),d.container.destroy({children:!0}),n.delete(r))},sync(r,d,u){for(const h of[...n.keys()])r.has(h)||c.destroy(h);const p=u!==null;for(const h of r.values()){const g=l(h);g.container.x=h.x,g.container.y=h.y+h.radius+6,g.container.scale.set(d);const m=u===h.id,b=!p||m?1:.4;for(const w of[g.container,h.gfx])Math.abs(w.alpha-b)>.005?w.alpha+=(b-w.alpha)*.18:w.alpha=b;(g.cachedName!==Te(h.name,Bt)||g.cachedCount!==h.count||g.cachedFocused!==m||g.cachedColor!==h.color)&&i(g,h)}}};return c}function no(o,e,t,s,n,a,i,l,c){const r=1-o;return{x:r*r*r*e+3*r*r*o*s+3*r*o*o*a+o*o*o*l,y:r*r*r*t+3*r*r*o*n+3*r*o*o*i+o*o*o*c}}function jt(o,e,t,s,n,a,i={}){const l=s-e,c=e+l*.5,r=s-l*.5,d=i.alpha??.5,u=i.width??1.5;if(!i.dashed){o.moveTo(e,t),o.bezierCurveTo(c,t,r,n,s,n),o.stroke({color:a,width:u,alpha:d});return}const p=32,h=i.dashPhase??0,g=Math.max(1,i.dashStride??1);let m=e,b=t;for(let w=1;w<=p;w++){const $=no(w/p,e,t,c,t,r,n,s,n);Math.floor((w-1+h)/g)%2===0&&(o.moveTo(m,b),o.lineTo($.x,$.y),o.stroke({color:a,width:u,alpha:d})),m=$.x,b=$.y}}function le(o,e,t,s){const n=e.gfx;n.clear();const a=e.radius;t||(n.circle(0,5,a),n.fill({color:0,alpha:.18})),t&&(n.circle(0,0,a+10),n.fill({color:e.color,alpha:.22})),n.circle(0,0,a),n.fill(Yi(e.color,-.18)),n.circle(0,-a*.06,a*.94),n.fill(e.color),n.circle(-a*.32,-a*.42,a*.3),n.fill({color:16777215,alpha:.32}),n.circle(0,0,a),n.stroke({color:s.border,width:t?3:2,alignment:0}),n.x=e.x,n.y=e.y,n.zIndex=10,n.hitArea=new o.Circle(0,0,a+4)}function io(o,e,t,s){le(o,e,!1,s);const n=e.gfx,a=Math.sin(performance.now()/280)*.5+.5;n.circle(0,0,e.radius+6+a*5),n.stroke({color:t,width:3,alpha:.6+a*.35}),n.circle(0,0,e.radius*.42),n.fill({color:t,alpha:.85}),n.hitArea=new o.Circle(0,0,e.radius+12)}const fa=5500,ao=.05,ga=130;function ro(o,e,t){const s=Array.from(o.values());for(const n of s){if(n.pinned){n.x+=(n.tx-n.x)*.12,n.y+=(n.ty-n.y)*.12,n.gfx.x=n.x,n.gfx.y=n.y;continue}let a=0,i=0;for(const c of s){if(n===c)continue;const r=n.x-c.x,d=n.y-c.y,u=r*r+d*d+1,p=fa/u,h=Math.sqrt(u);a+=r/h*p,i+=d/h*p}const l=o.get(n.parent);if(l){const c=l.x-n.x,r=l.y-n.y,d=Math.sqrt(c*c+r*r)||1,u=d-ga;a+=c/d*u*ao,i+=r/d*u*ao}else a+=-n.x*8e-4,i+=-n.y*8e-4;if(t.nudge&&n.id!==t.focusId){const c=n.x-t.nudge.x,r=n.y-t.nudge.y,d=Math.sqrt(c*c+r*r)||1,u=t.nudge.radius+n.radius;d<u&&(a+=c/d*18*(u-d),i+=r/d*18*(u-d))}n!==t.dragNode&&(n.x+=a*e*.001+(n.tx-n.x)*.02,n.y+=i*e*.001+(n.ty-n.y)*.02),n.gfx.x=n.x,n.gfx.y=n.y}}function ma(o,e){const t={dragNode:null,focusId:null,nudge:null};for(let s=0;s<e;s++)ro(o,16,t);for(const s of o.values())s.tx=s.x,s.ty=s.y}const te=B;function it(o,e,t){const s=document.createElement("div");s.className=`${te}__sidebar-header`;const n=document.createElement("span");n.className=`${te}__sidebar-dot`,n.style.background=e;const a=document.createElement("code");a.className=`${te}__sidebar-slug`,a.textContent=t,s.appendChild(n),s.appendChild(a),o.appendChild(s)}function lo(o,e){const t=document.createElement("label");t.className=`${te}__sidebar-label`,t.textContent=e,o.appendChild(t)}function ze(o,e,t,s){lo(o,e);const n=document.createElement("input");return n.type="text",n.className=`${te}__editor-name`,n.value=t,n.placeholder=s,o.appendChild(n),n}function Kt(o,e){const t=ze(o,f("Slug"),e,f("auto-from-name"));return t.spellcheck=!1,t.autocapitalize="off",t.addEventListener("input",()=>{const s=t.value,n=s.toLowerCase().replace(/[^a-z0-9-]+/g,"-");if(s!==n){const a=t.selectionStart??n.length;t.value=n,t.setSelectionRange(a,a)}}),t}function at(o,e){lo(o,f("Description"));const t=document.createElement("textarea");return t.className=`${te}__editor-desc`,t.value=e,t.placeholder=f("Description (optional)"),t.rows=4,o.appendChild(t),t}function co(o,e){const t=document.createElement("p");t.className=`${te}__sidebar-meta`,t.textContent=e,o.appendChild(t)}const fe=Ft;function rt(o,e){const t=document.createElement("div");t.className=`${te}__editor-actions`;for(const s of e)t.appendChild(s);o.appendChild(t)}function uo(o,e,t,s){const n=document.createElement("div");n.className=`${te}__sidebar-empty`;const a=document.createElement("span");a.className=`dashicons ${e}`,a.setAttribute("aria-hidden","true"),n.appendChild(a);const i=document.createElement("h3");i.className=`${te}__sidebar-empty-title`,i.textContent=t,n.appendChild(i);const l=document.createElement("p");l.className=`${te}__sidebar-empty-hint`,l.textContent=s,n.appendChild(l),o.appendChild(n)}function po(o){const e=Ft("danger",f("Delete"));let t=null;return e.addEventListener("click",async()=>{if(!e.classList.contains("is-armed")){e.textContent=f("Click again to delete"),e.classList.add("is-armed"),t!==null&&window.clearTimeout(t),t=window.setTimeout(()=>{e.textContent=f("Delete"),e.classList.remove("is-armed"),t=null},2500);return}t!==null&&(window.clearTimeout(t),t=null),await o()}),e}function ho(o,e,t){o.addEventListener("keydown",s=>{s.key==="Enter"?(s.preventDefault(),e()):s.key==="Escape"&&t()})}function ba(o,e){const{sidebar:t,client:s}=o,n=e.parent!==0?o.node(e.parent):void 0,a=n?n.color:o.clusterColor(o.terms().length);it(t,to(a),n?V(f("New child of %s"),n.name):f("New root category"));const i=ze(t,f("Name"),"",f("e.g. Recipes"));requestAnimationFrame(()=>i.focus());const l=Kt(t,""),c=at(t,""),r=fe("primary",f("Create")),d=fe("danger",f("Cancel")),u=()=>{o.setDraft(null),ie(o)},p=async()=>{const h=i.value.trim();if(!h){i.focus();return}r.disabled=!0;try{const g=await s.createCategory(h,e.parent,{slug:l.value.trim()||void 0,description:c.value||void 0}),m={id:g.id,name:g.name,slug:g.slug||"",parent:g.parent,count:0,description:g.description||"",isDefault:!1};o.terms().some(b=>b.id===m.id)||o.setTerms(o.terms().concat(m)),o.setDraft(null),o.buildTree(),o.setFocus(g.id),ie(o),await o.loadPosts()}catch(g){r.disabled=!1,o.toast(f("Couldn’t create:"),g)}};r.addEventListener("click",()=>void p()),d.addEventListener("click",u),ho(i,()=>void p(),u),rt(t,[r,d])}function _a(o,e){const{sidebar:t,client:s}=o,n=e.id;it(t,to(e.color),`#${n}`);const a=o.terms().find(h=>h.id===n),i=ze(t,f("Name"),e.name,f("Name")),l=Kt(t,a?.slug||""),c=at(t,e.description||"");co(t,V(de("%d post in this category.","%d posts in this category.",e.count),e.count));const r=fe("secondary",f("+ Child"));r.addEventListener("click",()=>{o.setDraft({parent:n}),ie(o)});const d=e.parent&&e.parent!==0?fe("secondary",f("Make root")):null;d&&(d.title=f("Promote this category to a top-level root (no parent)."),d.addEventListener("click",async()=>{try{await s.updateTerm("categories",n,{parent:0});const h=o.node(n);h&&(h.parent=0),o.setTerms(o.terms().map(g=>g.id===n?{...g,parent:0}:g)),o.buildTree(),ie(o)}catch(h){o.toast(f("Couldn’t reparent:"),h)}}));const u=fe("primary",f("Save"));u.addEventListener("click",async()=>{const h=i.value.trim();if(!h)return;const g=c.value,m=l.value.trim(),b=a?.slug??"";if(h===e.name&&g===(e.description||"")&&m===b)return;const w={name:h,description:g};m!==b&&(w.slug=m);try{const $=await s.updateTerm("categories",n,w),T=o.node(n);T&&(T.name=$.name,T.description=$.description),o.setTerms(o.terms().map(R=>R.id===n?{...R,name:$.name,description:$.description,slug:$.slug||R.slug}:R)),o.relayoutNode(n),ie(o)}catch($){o.toast(f("Couldn’t save:"),$)}});const p=po(async()=>{try{await s.deleteTerm("categories",n),o.setTerms(o.terms().filter(h=>h.id!==n)),o.setFocus(null),o.clearPosts(),o.buildTree(),ie(o)}catch(h){o.toast(f("Couldn’t delete:"),h)}});rt(t,d?[r,d,u,p]:[r,u,p])}function ie(o){o.sidebar.replaceChildren();const e=o.draft();if(e!==null){ba(o,e);return}const t=o.focusId();if(t===null){uo(o.sidebar,"dashicons-admin-tools",f("No category selected"),f("Click a node on the mindmap to edit its name, description, and posts."));return}const s=o.node(t);if(!s){o.setFocus(null),ie(o);return}_a(o,s)}const fo=22,va=48;function go(o,e){const t=Math.max(1,...e.map(s=>s.count));return fo+(va-fo)*Math.sqrt(o/t)}function Gt(o){return o.isDefault||o.id===1||o.slug==="uncategorized"||o.name.toLowerCase()==="uncategorized"}async function ya(o,e){const t=await oo(o,e,{taxonomy:"categories",restTaxonomy:"category",modifier:"os-mindmap",unavailable:f("Mindmap unavailable."),loadFailed:f("Couldn’t load categories:"),emptyHint:f('No custom categories yet. Click "Add root category" to start branching.'),chrome:{buttons:[{variant:"primary",icon:"dashicons-plus",label:f("Add root category")},{icon:"dashicons-image-rotate",label:f("Recenter")}],searchPlaceholder:f("Search categories…"),searchAria:f("Search categories in the mindmap"),hint:f("Click a node to focus + edit · drag onto another to reparent · scroll or pinch to zoom")},layers:["edge","postEdge","post","node","chip","postChip"],fan:{chipFontSize:14,chipTextRes:Ae,pagerLabelSize:14,pagerGlyphSize:16,pagerTextRes:Ae}});if(!t)return()=>{};const s=t,{pixi:n,layers:a,fan:i,camera:l,interaction:c,world:r,palette:d}=s,{client:u}=e,p=new n.Graphics;a.edge.addChild(p);const h=new Map,g=ha(n,a.chip,c,{palette:d,isFocused:v=>i.focusId===v,onTap:v=>void s.focusOn(v)});let m=null,b=null,w=null,$={x:0,y:0};const T=new Map;let R=null,O=ot(o);const I=v=>eo((O+v*47)%360,55,52);function x(v,_){let S=h.get(v.id);if(S)S.parent=v.parent,S.name=v.name,S.description=v.description,S.count=v.count,S.depth=_.depth,S.color=_.color,S.radius=_.radius,S.tx=_.tx,S.ty=_.ty,S.pinned=_.pinned;else{const y=new n.Graphics;y.eventMode="static",y.cursor="pointer",S={id:v.id,parent:v.parent,name:v.name,description:v.description,count:v.count,x:_.tx,y:_.ty,tx:_.tx,ty:_.ty,radius:_.radius,depth:_.depth,color:_.color,gfx:y,pinned:_.pinned},a.node.addChild(y);const C=S;y.on("pointerdown",A=>N(A,C)),h.set(v.id,S)}return le(n,S,!1,d),S}function E(){const v=s.terms,_=new Map;for(const W of v){const M=_.get(W.parent)??[];M.push(W),_.set(W.parent,M)}const S=_.get(0)??[],y=S.filter(W=>!Gt(W)),C=S.find(Gt),A=(W,M,Y,ae,me)=>{const J=y.length>1?110+y.length*28:0,be=C?Math.max(J,140):J,xe=M===0?be:be+160+(M-1)*150,Da=M===0?I(Y):h.get(W.parent)?.color??I(Y);x(W,{tx:xe*Math.cos(ae),ty:xe*Math.sin(ae),radius:go(W.count,v),depth:M,color:Da,pinned:M===0});const es=_.get(W.id)??[];if(es.length>0){const Co=me/es.length;es.forEach((Oa,qa)=>A(Oa,M+1,Y,ae-me/2+Co*(qa+.5),Co*.85))}},L=new Set(v.map(W=>W.id));for(const[W,M]of h)L.has(W)||(a.node.removeChild(M.gfx),M.gfx.destroy(),h.delete(W),g.destroy(W));const K=Math.max(1,y.length);y.forEach((W,M)=>A(W,0,M,2*Math.PI/K*M,2*Math.PI/K)),C&&x(C,{tx:0,ty:0,radius:go(C.count,v),depth:0,color:9211796,pinned:!0}),s.syncEmptyHint(v.length<=1)}function z(){p.clear();for(const v of h.values()){const _=v.parent?h.get(v.parent):void 0;if(!_)continue;const S=m!==null&&v===m,y=i.focusId!==null&&(v.id===i.focusId||v.parent===i.focusId),C=i.focusId!==null&&!y?.35:1;jt(p,_.x,_.y,v.x,v.y,_.color,S?{dashed:!0,alpha:.28*C}:{alpha:.5*C})}if(m&&b){const{x:v,y:_}=m,{x:S,y,color:C}=b;jt(p,v,_,S,y,C,{alpha:.22,width:9}),jt(p,v,_,S,y,C,{alpha:.95,width:2.5,dashed:!0,dashStride:2,dashPhase:Math.floor(performance.now()/70)});const A=performance.now()%1300/1300,L=S-v,K=no(A,v,_,v+L*.5,_,S-L*.5,y,S,y);p.circle(K.x,K.y,5),p.fill({color:16777215,alpha:.95}),p.stroke({color:C,width:2,alpha:1})}i.drawEdges()}function N(v,_){if(ne(c))return;ve(c,v),m=_,_.pinned=!0,_.tx=_.x,_.ty=_.y,w={x:v.global.x,y:v.global.y};const S=l.stageToWorld(v.global);$={x:_.x-S.x,y:_.y-S.y}}function F(v,_){let S=h.get(_),y=32;for(;S&&y-- >0;){if(S.id===v)return!0;if(!S.parent)return!1;S=h.get(S.parent)}return!1}function U(v){if(!m)return;const _=v.x+$.x,S=v.y+$.y;m.x=_,m.y=S,m.tx=_,m.ty=S,m.gfx.x=_,m.gfx.y=S;let y=null;for(const C of h.values()){if(C===m)continue;const A=C.x-v.x,L=C.y-v.y;if(A*A+L*L<C.radius*C.radius){y=C;break}}y!==b&&(b&&le(n,b,i.focusId===b.id,d),b=y,y&&io(n,y,m.color,d))}async function j(v){if(!m)return;const _=m,S=b,y=Qs(w,v);if(m=null,b=null,w=null,_.pinned=_.depth===0,!S&&y<2){s.focusOn(_.id);return}if(S&&S.id!==_.parent&&!F(_.id,S.id))try{await u.updateTerm("categories",_.id,{parent:S.id}),_.parent=S.id,s.terms=s.terms.map(C=>C.id===_.id?{...C,parent:S.id}:C),E()}catch(C){e.toast(f("Reparent failed:"),C)}else le(n,_,i.focusId===_.id,d),S&&le(n,S,i.focusId===S.id,d)}const G={sidebar:s.sidebar,client:u,toast:e.toast,terms:()=>s.terms,setTerms:v=>{s.terms=v},node:v=>h.get(v),focusId:()=>i.focusId,setFocus:v=>{i.focusId=v},draft:()=>R,setDraft:v=>{R=v&&v.parent!==0&&!h.get(v.parent)?R:v},clusterColor:I,buildTree:E,relayoutNode:v=>{const _=h.get(v);_&&g.relayout(_)},clearPosts:()=>i.clear(),loadPosts:()=>i.load()};function q(){if(h.size===0)return null;const v=30,_={minX:1/0,minY:1/0,maxX:-1/0,maxY:-1/0};for(const S of h.values())_.minX=Math.min(_.minX,S.tx-S.radius),_.minY=Math.min(_.minY,S.ty-S.radius),_.maxX=Math.max(_.maxX,S.tx+S.radius),_.maxY=Math.max(_.maxY,S.ty+S.radius+v);return _}const[P,D]=s.chrome.buttons;return D.addEventListener("click",()=>s.recenter()),P.addEventListener("click",()=>{R={parent:0},ie(G)}),E(),ie(G),ma(h,80),s.start({themeChanged:()=>{const v=ot(o);if(v!==O){O=v;const _=s.terms.filter(S=>!S.parent&&!Gt(S));for(const S of h.values()){let y=S;const C=new Set;for(;y.parent&&h.has(y.parent)&&!C.has(y.id);)C.add(y.id),y=h.get(y.parent);const A=_.findIndex(L=>L.id===y.id);A>=0&&(S.color=I(A))}}for(const _ of h.values())le(n,_,i.focusId===_.id,d),g.relayout(_)},center:v=>{const _=h.get(v);return _?{x:_.x,y:_.y,tone:_.color}:null},countReconciled:(v,_)=>{const S=h.get(v);S&&S.count!==_&&(S.count=_,s.terms=s.terms.map(y=>y.id===v?{...y,count:_}:y),g.relayout(S))},focusChanged:()=>{for(const v of h.values())le(n,v,i.focusId===v.id,d);ie(G)},focusOpened:v=>{T.clear();for(const _ of h.values()){if(_.id===i.focusId||!_.pinned)continue;const S=_.x-v.x,y=_.y-v.y,C=Math.sqrt(S*S+y*y)||1;if(C>=Vt+_.radius)continue;T.set(_.id,{tx:_.tx,ty:_.ty});const A=Vt+_.radius+20;_.tx=v.x+S/C*A,_.ty=v.y+y/C*A}},focusClosed:()=>{for(const[v,_]of T){const S=h.get(v);S&&(S.tx=_.tx,S.ty=_.ty)}T.clear()},bounds:q,frame:v=>{ro(h,v,{dragNode:m,focusId:i.focusId,nudge:s.nudge}),i.ease(),z(),m&&b&&io(n,b,m.color,d);const _=1/Math.max(.01,r.scale.x);g.sync(h,_,i.focusId),i.syncChips(_)},countsChanged:()=>{for(const v of s.terms){const _=h.get(v.id);_&&_.count!==v.count&&(_.count=v.count,g.relayout(_))}E(),l.fitToView(q(),{animate:!0})},dragging:()=>m!==null,pointerMove:(v,_)=>m?(U(_),!0):!1,pointerUp:j,cancelGesture:()=>{m&&(m.pinned=m.depth===0,le(n,m,i.focusId===m.id,d)),b&&le(n,b,i.focusId===b.id,d),m=null,b=null,w=null},search:v=>Array.from(h.values()).filter(_=>_.name.toLowerCase().includes(v))}),()=>s.teardown()}const xa=H`:host{display:inline-flex;max-width:100%}.os-tag-input{display:inline-flex;flex-wrap:wrap;align-items:center;gap:var( --os-ui-tag-input-gap,4px );padding:var( --os-ui-tag-input-padding,2px );min-height:24px;max-width:100%}.os-tag-input__chips{display:inline-flex;flex-wrap:wrap;align-items:center;gap:var( --os-ui-tag-input-gap,4px );min-width:0}.os-tag-input__add{appearance:none;display:inline-flex;align-items:center;gap:3px;padding:1px 8px;min-height:22px;font:inherit;font-size:11px;font-weight:500;line-height:1;color:var( --os-ui-tag-input-add-fg,var( --os-ui-fg-muted,#50575e ) );background:transparent;border:1px dashed var( --os-ui-tag-input-add-border,var( --os-ui-border,#c3c4c7 ) );border-radius:999px;cursor:pointer;transition:background-color 0.12s ease,color 0.12s ease,border-color 0.12s ease}.os-tag-input__add:hover:not(:disabled ){background:var( --os-ui-hover,rgba( 0,0,0,0.04 ) );color:var( --os-ui-tag-input-add-fg-hover,var( --os-ui-fg,#1d2327 ) );border-color:var( --os-ui-tag-input-add-border-hover,var( --os-ui-border-strong,#8c8f94 ) )}.os-tag-input__add:focus-visible{outline:none;border-style:solid;box-shadow:var( --os-ui-focus-ring,0 0 0 2px rgba( 12,11,15,0.9 ),0 0 0 4px #2271b1 )}.os-tag-input__add:disabled{opacity:0.5;cursor:not-allowed}.os-tag-input__add svg{display:block}.os-tag-input__editor{position:relative;display:inline-flex;align-items:center;flex:0 1 auto;min-width:120px}.os-tag-input__input{appearance:none;font:inherit;font-size:var( --os-ui-field-font-size-compact,12px );line-height:1.4;padding:2px 8px;border:1px solid var( --os-ui-tag-input-input-border,var( --os-ui-border,#2271b1 ) );border-radius:999px;background:var( --os-ui-tag-input-input-bg,var( --os-ui-surface,#fff ) );color:var( --os-ui-tag-input-input-fg,var( --os-ui-fg,#1d2327 ) );min-width:80px;max-width:240px}.os-tag-input__input:focus{outline:none;border-color:var( --os-ui-accent,#2271b1 );box-shadow:var( --os-ui-focus-ring-field,0 0 0 1px #2271b1,0 0 0 4px rgba( 34,113,177,0.18 ) )}.os-tag-input__suggestions{position:absolute;top:calc( 100% + 4px );left:0;min-width:220px;max-width:320px;max-height:240px;overflow-y:auto;padding:4px 0;background:var( --os-ui-tag-input-pop-bg,var( --os-ui-surface,#fff ) );color:var( --os-ui-tag-input-pop-fg,var( --os-ui-fg,#1d2327 ) );border:1px solid var( --os-ui-tag-input-pop-border,var( --os-ui-border,#c3c4c7 ) );border-radius:8px;box-shadow:0 6px 16px rgba( 0,0,0,0.08 ),0 1px 2px rgba( 0,0,0,0.06 );z-index:50}.os-tag-input__suggestion-item{display:flex;align-items:center;gap:6px;padding:6px 12px;font-size:13px;cursor:pointer;user-select:none}.os-tag-input__suggestion-item[ aria-selected='true' ]{background:color-mix( in srgb,var( --wp-admin-theme-color,#2271b1 ) 10%,transparent );color:var( --wp-admin-theme-color,#2271b1 )}.os-tag-input__suggestion-create{font-style:italic;color:var( --os-ui-tag-input-create-fg,var( --os-ui-fg-muted,#50575e ) );border-top:1px solid var( --os-ui-tag-input-pop-divider,var( --os-ui-border,#f0f0f1 ) )}.os-tag-input__suggestion-create[ aria-selected='true' ]{color:var( --wp-admin-theme-color,#2271b1 )}.os-tag-input__suggestion-empty,.os-tag-input__suggestion-loading{display:flex;align-items:center;gap:8px;padding:8px 12px;font-size:12px;color:var( --os-ui-tag-input-pop-muted,var( --os-ui-fg-muted,#646970 ) )}.os-tag-input__suggestion-spinner{display:inline-block;width:10px;height:10px;border-radius:50%;border:2px solid currentColor;border-top-color:transparent;animation:os-tag-input-spin 0.8s linear infinite}@keyframes os-tag-input-spin{to{transform:rotate( 360deg )}}:host( [ disabled ] ) .os-tag-input{opacity:0.6;pointer-events:none}`,Ge=class Ge extends X{constructor(){super(...arguments),this._value=[],this._suggestions=[],this._suggestionsLoading=!1,this._query="",this._highlight=-1,this._focusedChip=-1,this._onDocumentPointerDown=e=>{!this.isOpen||e.composedPath().includes(this)||this.closeInput()}}get _input(){const e=this.shadowRoot;return e?e.querySelector(".os-tag-input__input"):null}get value(){return this._value}set value(e){this._value=Array.isArray(e)?e.slice():[],this._focusedChip>=this._value.length&&(this._focusedChip=-1),this.requestUpdate()}get suggestions(){return this._suggestions}set suggestions(e){this._suggestions=Array.isArray(e)?e.slice():[],this._highlight=this._suggestions.length>0?0:-1,this._suggestionsLoading=!1,this.requestUpdate()}get suggestionsLoading(){return this._suggestionsLoading}set suggestionsLoading(e){this._suggestionsLoading=!!e,this.requestUpdate()}get query(){return this._query}get isOpen(){return this.open!==null}openInput(){this.isOpen||(this.open="",this._query="",this._highlight=-1,this.emit("os-tag-open",{}),queueMicrotask(()=>{this._input?.focus(),this._emitSuggest("")}))}closeInput(){this.isOpen&&(this.open=null,this._query="",this._suggestions=[],this._highlight=-1,this._suggestionsLoading=!1,this.emit("os-tag-close",{}),this.requestUpdate())}connectedCallback(){super.connectedCallback(),document.addEventListener("pointerdown",this._onDocumentPointerDown,!0)}disconnectedCallback(){document.removeEventListener("pointerdown",this._onDocumentPointerDown,!0)}render(){const e=this.isOpen,t=this.disabled!==null,s=this.readonly!==null,n=this.removable!==null||this.removable===null&&!s,a=this.creatable!==null,i=this["add-label"]||"Add",l=this.placeholder||"Add a tag…";return k`
			<span
				class="os-tag-input"
				role="group"
				aria-label=${this.label??""}
			>
				${this._renderChips(n,t)}
				${this._renderTrailing({isOpen:e,readonly:s,disabled:t,placeholder:l,creatable:a,addLabel:i})}
			</span>
		`}_renderTrailing(e){return e.isOpen?this._renderEditor(e.placeholder,e.creatable):e.readonly||e.disabled?k``:this._renderTrigger(e.addLabel)}_renderChips(e,t){const s=this._value;return s.length===0?k``:k`
			<span class="os-tag-input__chips" role="list">
				${s.map((n,a)=>{const i=n.tone??"neutral";return k`
						<os-chip
							role="listitem"
							size="compact"
							tone=${i}
							...
							?disabled=${t}
							?pending=${!!n.pending}
							tabindex=${a===this._focusedChip?"0":"-1"}
							data-idx=${String(a)}
							@os-chip-dismiss=${l=>this._onChipDismiss(l,n)}
							@focus=${()=>this._focusedChip=a}
						></os-chip>
					`})}
			</span>
		`}_renderTrigger(e){const t=this.disabled!==null;return k`
			<button
				type="button"
		...
		`}_renderEditor(e,t){const s=this._suggestions.length>0||this._suggestionsLoading||t&&this._query.trim().length>0;return k`
			<span class="os-tag-input__editor">
				...
				/>
				${s?this._renderSuggestions(t):k``}
			</span>
		`}_renderSuggestions(e){const t=this._query.trim(),s=this._suggestions,n=e&&t.length>0&&!s.some(a=>a.label.toLowerCase()===t.toLowerCase())&&!this._value.some(a=>a.label.toLowerCase()===t.toLowerCase());return k`
			<div
				class="os-tag-input__suggestions"
				role="listbox"
			>
				${this._suggestionsLoading?k`
							<div class="os-tag-input__suggestion-loading">
								<span class="os-tag-input__suggestion-spinner" aria-hidden="true"></span>
								<span>Searching…</span>
							</div>
					  `:k``}
				${s.length===0&&!this._suggestionsLoading&&!n?k`
							<div class="os-tag-input__suggestion-empty">
								${t.length>0?"No matches.":"Type to search."}
							</div>
					  `:k``}
				${s.map((a,i)=>{const l=i===this._highlight;return k`
						<div
							id=${`os-tag-suggestion-${i}`}
							role="option"
							aria-selected=${l?"true":"false"}
							class="os-tag-input__suggestion-item"
							@mousedown=${c=>{c.preventDefault(),this._addSuggestion(a,!1)}}
				...
				${n?k`
							<div
								...
								@mousedown=${a=>{a.preventDefault(),this._addSuggestion({label:t},!0)}}
		...
		`}_onChipDismiss(e,t){e.stopPropagation(),this.emit("os-tag-remove",{tag:t})}_onInput(e){const t=e.target.value;this._query=t,this._emitSuggest(t)}_emitSuggest(e){const t=parseInt(this["min-query"]||"0",10)||0;if(e.length<t){this._suggestions=[],this._suggestionsLoading=!1,this.requestUpdate();return}this._suggestionsLoading=!0,this.requestUpdate(),this.emit("os-tag-suggest",{query:e})}_onInputKeyDown(e){const t=this.creatable!==null,s=this._suggestions,n=this._query.trim(),a=t&&n.length>0&&!s.some(l=>l.label.toLowerCase()===n.toLowerCase())&&!this._value.some(l=>l.label.toLowerCase()===n.toLowerCase()),i=s.length+(a?1:0);switch(e.key){case"ArrowDown":{if(i===0)return;e.preventDefault(),this._highlight=this._highlight+1>=i?0:this._highlight+1,this.requestUpdate();return}case"ArrowUp":{if(i===0)return;e.preventDefault(),this._highlight=this._highlight<=0?i-1:this._highlight-1,this.requestUpdate();return}case"Enter":{if(e.preventDefault(),this._highlight>=0&&this._highlight<s.length){this._addSuggestion(s[this._highlight],!1);return}if(this._highlight===s.length&&a){this._addSuggestion({label:n},!0);return}if(a&&n.length>0){this._addSuggestion({label:n},!0);return}return}case"Escape":{e.preventDefault(),this.closeInput();return}case"Backspace":{if(this._query===""&&this._value.length>0){e.preventDefault();const l=this._value.length-1;this._focusedChip===l?(this.emit("os-tag-remove",{tag:this._value[l]}),this._focusedChip=-1):(this._focusedChip=l,this.requestUpdate())}return}default:this._focusedChip!==-1&&(this._focusedChip=-1)}}_onInputBlur(e){queueMicrotask(()=>{this.shadowRoot?.activeElement||this.closeInput()})}_addSuggestion(e,t){if(this._value.some(n=>n.label.toLowerCase()===e.label.toLowerCase())){this._query="",this._highlight=-1,this._suggestions=[],this.requestUpdate(),this._input?.focus();return}this.emit("os-tag-add",{tag:e,isNew:t}),this._query="",this._highlight=-1,this._suggestions=[],this._suggestionsLoading=!1,this.requestUpdate(),queueMicrotask(()=>{this._input?.focus()})}};Ge.props=["label","placeholder","add-label","creatable","removable","disabled","readonly","size","min-query","open"],Ge.styles=[xa],Ge.help=void 0;let Xt=Ge;Q("os-tag-input",Xt);function ka(){return se("plus",{size:14})}function wa(o,e){const{client:t}=e,s=document.createElement("span");s.style.cssText="display:inline-flex;align-items:center;width:100%;min-width:0;";const n=document.createElement("os-tag-input");n.setAttribute("creatable",""),n.setAttribute("removable",""),n.setAttribute("min-query","0"),n.setAttribute("placeholder",f("Add tag…")),n.setAttribute("add-label",f("Tag")),n.setAttribute("data-noclick","");const a=yt(o,"post_tag").map(c=>({id:c.id,label:c.name}));n.value=a;const i={tags:a.slice(),suggestAbort:null,suggestDebounce:null,lastQuery:""},l=c=>{i.tags=c.slice(),n.value=c};return n.addEventListener("os-tag-suggest",c=>{const r=c.detail?.query??"";i.lastQuery=r,i.suggestDebounce!==null&&window.clearTimeout(i.suggestDebounce),i.suggestDebounce=window.setTimeout(async()=>{i.suggestDebounce=null,i.suggestAbort?.abort();const d=new AbortController;i.suggestAbort=d;try{const u=await t.searchTags(r,d.signal);if(i.lastQuery!==r)return;const p=new Set(i.tags.map(h=>h.id));n.suggestions=u.filter(h=>!p.has(h.id)).map(h=>({id:h.id,label:h.name}))}catch(u){if(u?.name==="AbortError")return;n.suggestions=[],console.warn("[openstation:desktop-mode-posts] tag search failed",u)}finally{n.suggestionsLoading=!1}},200)}),n.addEventListener("os-tag-add",async c=>{const r=c.detail;if(r?.tag){l([...i.tags,{id:r.tag.id,label:r.tag.label,pending:!0}]);try{const d=r.isNew||typeof r.tag.id!="number"?await t.createTag(r.tag.label):{id:Number(r.tag.id),name:r.tag.label,slug:""},u=[...i.tags.filter(p=>!p.pending).map(p=>Number(p.id)),d.id];await t.updatePostTags(o.id,u),l(i.tags.map(p=>p.label.toLowerCase()===r.tag.label.toLowerCase()?{id:d.id,label:d.name}:p)),e.announce("tagged",[o.id])}catch(d){l(i.tags.filter(u=>u.label.toLowerCase()!==r.tag.label.toLowerCase())),e.toast(V(f('Couldn’t add tag "%s".'),r.tag.label),d)}}}),n.addEventListener("os-tag-remove",async c=>{const r=c.detail;if(!r?.tag)return;const d=r.tag,u=i.tags.slice();l(i.tags.map(p=>p.label===d.label?{...p,pending:!0}:p));try{const p=u.filter(h=>h.label!==d.label).map(h=>Number(h.id)).filter(h=>Number.isFinite(h));await t.updatePostTags(o.id,p),l(u.filter(h=>h.label!==d.label)),e.announce("untagged",[o.id])}catch(p){l(u),e.toast(V(f('Couldn’t remove tag "%s".'),d.label),p)}}),s.appendChild(n),s}const Pe=11,Ca=6,Yt=4,mo=8,bo=22,_o=o=>eo(o,70,48);function $a(o,e,t,s,n){const a=new o.Container;a.eventMode="static",a.cursor="pointer";const i=new o.Graphics,l=new o.Graphics,c=(p,h,g,m)=>new o.Text({text:p,style:{fill:h,fontSize:g,fontFamily:Ee,fontWeight:m},resolution:Ae}),r=c("#",n.accent,s,"700"),d=c(Te(t.name,bo),n.fg,s,"600"),u=c(String(t.count),n.fg,Math.max(10,Math.round(s*.55)),"700");for(const p of[i,l,r,d,u])a.addChild(p);return e.addChild(a),{container:a,shadow:i,bg:l,hashText:r,nameText:d,countText:u,cachedHover:!1}}function Sa(o,e,t){const s=o.chip,n=Te(o.name,bo),a=String(o.count);s.nameText.text!==n&&(s.nameText.text=n),s.countText.text!==a&&(s.countText.text=a),s.nameText.style.fontSize=o.fontSize,s.hashText.style.fontSize=o.fontSize,s.countText.style.fontSize=Math.max(10,Math.round(o.fontSize*.55));const i=s.nameText.height,l=Math.max(18,s.countText.width+10),c=Math.max(14,s.countText.height+4);o.width=Pe+s.hashText.width+Yt+s.nameText.width+mo+l+Pe,o.height=Math.max(i,c)+Ca*2,vo(o,e,t)}function vo(o,e,t){const s=o.chip,n=o.width,a=o.height,i=-n/2,l=-a/2,c=a/2,r=e||s.cachedHover?t.raised:t.surface,d=e?t.accent:t.border,u=_o(o.hue);s.shadow.clear(),s.shadow.roundRect(i-1,l+3,n+2,a+2,c+1);let p=.1;e?p=.18:s.cachedHover&&(p=.16),s.shadow.fill({color:0,alpha:p}),s.bg.clear(),s.bg.roundRect(i,l,n,a,c),s.bg.fill(r),s.bg.stroke({color:d,width:e?2:1.25,alpha:e?1:.85});const h=s.hashText.width,g=s.nameText.width,m=s.nameText.height,b=s.countText.width,w=s.countText.height,$=Math.max(18,b+10),T=Math.max(14,w+4);s.hashText.x=i+Pe,s.hashText.y=(a-m)/2+l,s.hashText.style.fill=t.accent,s.nameText.x=i+Pe+h+Yt,s.nameText.y=(a-m)/2+l,s.nameText.style.fill=t.fg;const R=i+Pe+h+Yt+g+mo,O=(a-T)/2+l;s.bg.roundRect(R,O,$,T,T/2),s.bg.fill(u),s.countText.x=R+($-b)/2,s.countText.y=O+(T-w)/2,s.countText.style.fill=so(u,t)}const yo=11,Ea=28,Aa=14;function xo(o,e){const t=Math.sqrt(o/Math.max(1,e));return Math.round(yo+(Ea-yo)*t)}function Ta(o,e){return o.x<e.x+e.w&&o.x+o.w>e.x&&o.y<e.y+e.h&&o.y+o.h>e.y}function ko(o){let e=0;for(let t=0;t<o.length;t++)e=(e*31+o.charCodeAt(t))%2147483647;return e}function Zt(o,e){return((e+ko(o)%256*1.4)%360+360)%360}function wo(o){const e=ko(o);return(e%2===0?-1:1)*(Math.floor(e/2)%4)*.011}function Ma(o,e,t,s=0,n=0){if(t.length===0)return{x:s,y:n};const a=Aa,i=(c,r)=>{const d={x:c-o/2-a,y:r-e/2-a,w:o+a*2,h:e+a*2};return!t.some(u=>Ta(d,u))};if(i(s,n))return{x:s,y:n};let l=0;for(let c=0;c<1e4;c++){l+=.18;const r=l*5,d=s+r*Math.cos(l),u=n+r*Math.sin(l)*.7;if(i(d,u))return{x:d,y:u}}return{x:s,y:n+(t.length+1)*(e+a)}}function Jt(o,e,t,s){let n=0;const a=()=>{const i=n++;if(i===0)return{x:0,y:0};const l=i*2.4,c=120+i*70;return{x:c*Math.cos(l),y:c*Math.sin(l)*.8}};for(const i of o){let l=0,c=0,r=!1;const d=s.get(i.id);if(d&&d.length>0){let p=0,h=0,g=0;for(const m of d){const b=t.get(m.id);b&&(p+=b.x*m.shared,h+=b.y*m.shared,g+=m.shared)}g>0&&(l=p/g,c=h/g,r=!0)}if(!r){const p=a();l=p.x,c=p.y}const u=Ma(i.width,i.height,e,l,c);i.tx=u.x,i.ty=u.y,t.set(i.id,{x:u.x,y:u.y}),e.push({x:u.x-i.width/2,y:u.y-i.height/2,w:i.width,h:i.height})}}function Ia(){try{const o=window.location.host||"unknown",e=window.location.pathname.replace(/\/?wp-admin\/?.*$/,"");return`os-tagcloud-positions:${o}${e}`}catch{return"os-tagcloud-positions:fallback"}}function La(o){const e=new Map;try{const t=window.localStorage.getItem(o),s=t?JSON.parse(t):null;if(!s||typeof s!="object")return e;for(const[n,a]of Object.entries(s)){const i=parseInt(n,10),l=a;Number.isFinite(i)&&typeof l?.x=="number"&&typeof l?.y=="number"&&e.set(i,{x:l.x,y:l.y})}}catch{}return e}function Qt(o,e){try{const t={};for(const[s,n]of e)t[String(s)]=n;window.localStorage.setItem(o,JSON.stringify(t))}catch{}}function za(o){const{sidebar:e,client:t}=o;it(e,`hsl( ${o.themeHue}deg 60% 55% )`,f("New tag"));const s=ze(e,f("Name"),"",f("e.g. featured"));requestAnimationFrame(()=>s.focus());const n=at(e,""),a=fe("primary",f("Create")),i=fe("danger",f("Cancel")),l=()=>{o.setDraft(!1),ce(o)},c=async()=>{const r=s.value.trim();if(!r){s.focus();return}a.disabled=!0;try{const d=await t.createTag(r),u={id:d.id,name:d.name,slug:d.slug||"",parent:0,count:0,description:d.description||"",isDefault:!1};o.terms().some(h=>h.id===u.id)||o.setTerms(o.terms().concat(u));const p=n.value.trim();if(p)try{const h=await t.updateTerm("tags",d.id,{description:p});o.setTerms(o.terms().map(g=>g.id===h.id?{...g,description:h.description||p}:g))}catch(h){o.toast(f("Tag created but description failed:"),h)}o.setDraft(!1),o.buildCloud(),o.setFocus(d.id),ce(o),await o.loadPosts()}catch(d){a.disabled=!1,o.toast(f("Couldn’t create:"),d)}};a.addEventListener("click",()=>void c()),i.addEventListener("click",l),ho(s,()=>void c(),l),rt(e,[a,i])}function Pa(o,e){const{sidebar:t,client:s}=o,n=e.id;it(t,`hsl( ${e.hue}deg 60% 55% )`,`#${n}`);const a=o.terms().find(u=>u.id===n),i=ze(t,f("Name"),e.name,f("Name")),l=Kt(t,a?.slug||""),c=at(t,e.description||"");co(t,V(de("%d post tagged with this.","%d posts tagged with this.",e.count),e.count));const r=fe("primary",f("Save"));r.addEventListener("click",async()=>{const u=i.value.trim();if(!u)return;const p=c.value,h=l.value.trim(),g=a?.slug??"";if(u===e.name&&p===(e.description||"")&&h===g)return;const m={name:u,description:p};h!==g&&(m.slug=h);try{const b=await s.updateTerm("tags",n,m);o.applyTagUpdate(n,{name:b.name,description:b.description,slug:b.slug||e.slug}),o.setTerms(o.terms().map(w=>w.id===n?{...w,name:b.name,description:b.description,slug:b.slug||w.slug}:w)),ce(o)}catch(b){o.toast(f("Couldn’t save:"),b)}});const d=po(async()=>{try{await s.deleteTerm("tags",n),o.setTerms(o.terms().filter(u=>u.id!==n)),o.forgetTag(n),o.setFocus(null),o.clearPosts(),o.buildCloud(),ce(o)}catch(u){o.toast(f("Couldn’t delete:"),u)}});rt(t,[r,d])}function ce(o){if(o.sidebar.replaceChildren(),o.draft()){za(o);return}const e=o.focusId();if(e===null){uo(o.sidebar,"dashicons-tag",f("No tag selected"),f("Click a tag on the cloud to edit it, or click + Add tag to create a new one."));return}const t=o.tag(e);if(!t){o.setFocus(null),ce(o);return}Pa(o,t)}async function Ra(o,e){const t=await oo(o,e,{taxonomy:"tags",restTaxonomy:"post_tag",modifier:"os-tagcloud",unavailable:f("Tag cloud unavailable."),loadFailed:f("Couldn’t load tags:"),emptyHint:f('No tags yet. Click "Add tag" to start building the cloud.'),chrome:{buttons:[{variant:"primary",icon:"dashicons-plus",label:f("Add tag")},{icon:"dashicons-image-rotate",label:f("Recenter")},{icon:"dashicons-grid-view",label:f("Reflow"),title:f("Recompute the chip layout from scratch — discards manual repositioning.")}],searchPlaceholder:f("Search tags…"),searchAria:f("Search tags in the cloud"),hint:f("Click a tag to focus + edit · drag to reposition · scroll or pinch to zoom")},layers:["postEdge","chip","post","postChip"],fan:{chipFontSize:12,chipTextRes:Ae,pagerLabelSize:12,pagerGlyphSize:14}});if(!t)return()=>{};const s=t,{pixi:n,layers:a,fan:i,camera:l,interaction:c,world:r,palette:d}=s,{client:u}=e,p=new Map;let h=null,g={x:0,y:0},m=null,b=null,w=!1;const $=Ia(),T=La($);let R=new Map,O=ot(o);const I=y=>i.focusId===y,x=y=>Sa(y,I(y.id),d),E=y=>vo(y,I(y.id),d);function z(){const y=s.terms,C=new Set(y.map(M=>M.id));for(const[M,Y]of p)C.has(M)||(a.chip.removeChild(Y.chip.container),Y.chip.container.destroy({children:!0}),p.delete(M));const A=Math.max(1,...y.map(M=>M.count)),L=[];for(const M of y){const Y=xo(M.count,A),ae=Zt(M.slug||M.name,O),me=wo(M.slug||M.name),J=p.get(M.id);if(J){J.name=M.name,J.slug=M.slug,J.description=M.description,J.count=M.count,J.fontSize=Y,J.hue=ae,J.rotation=me,x(J);continue}const be=T.get(M.id),xe={id:M.id,name:M.name,slug:M.slug,description:M.description,count:M.count,fontSize:Y,hue:ae,rotation:me,x:be?.x??0,y:be?.y??0,tx:be?.x??0,ty:be?.y??0,width:0,height:0,chip:$a(n,a.chip,M,Y,d)};p.set(M.id,xe),x(xe),N(xe),be||L.push(xe)}const K=[],W=new Map;for(const M of p.values())L.includes(M)||(K.push({x:M.tx-M.width/2,y:M.ty-M.height/2,w:M.width,h:M.height}),W.set(M.id,{x:M.tx,y:M.ty}));L.sort((M,Y)=>Y.count-M.count),Jt(L,K,W,R);for(const M of L)M.x=M.tx,M.y=M.ty;s.syncEmptyHint(y.length===0)}function N(y){const C=y.chip.container;C.on("pointerdown",A=>{if(ne(c))return;const L=A;ve(c,A),h=y,b={x:y.tx,y:y.ty},m={x:L.global.x,y:L.global.y};const K=l.stageToWorld(L.global);g={x:y.x-K.x,y:y.y-K.y}}),C.on("pointerover",()=>{y.chip.cachedHover=!0,E(y)}),C.on("pointerout",()=>{y.chip.cachedHover=!1,E(y)})}function F(y){const C=s.nudge;for(const A of p.values()){if(A===h)continue;let L=A.tx,K=A.ty;if(C&&A.id!==i.focusId){const M=A.tx-C.x,Y=A.ty-C.y,ae=Math.sqrt(M*M+Y*Y)||1,me=C.radius+Math.max(A.width,A.height)/2;if(ae<me){const J=me+12;L=C.x+M/ae*J,K=C.y+Y/ae*J}}const W=1-Math.exp(-y*.012);A.x+=(L-A.x)*W,A.y+=(K-A.y)*W}}function U(){const y=1/Math.max(.01,r.scale.x),C=i.focusId!==null;for(const A of p.values()){const L=A.chip.container;L.x=A.x,L.y=A.y,L.scale.set(1),L.rotation=A.rotation;const K=!C||i.focusId===A.id?1:.32;Math.abs(L.alpha-K)>.005?L.alpha+=(K-L.alpha)*.18:L.alpha=K}i.syncChips(y)}function j(y){if(!h)return;const C=h,A=Qs(m,y);h=null,m=null,A<3?s.focusOn(C.id):(T.set(C.id,{x:C.tx,y:C.ty}),Qt($,T))}const G={sidebar:s.sidebar,client:u,toast:e.toast,themeHue:O,terms:()=>s.terms,setTerms:y=>{s.terms=y},tag:y=>p.get(y),focusId:()=>i.focusId,setFocus:y=>{i.focusId=y},draft:()=>w,setDraft:y=>{w=y},applyTagUpdate:(y,C)=>{const A=p.get(y);A&&(A.name=C.name,A.description=C.description,A.slug=C.slug,A.hue=Zt(A.slug||A.name,O),A.rotation=wo(A.slug||A.name),x(A))},forgetTag:y=>{T.delete(y),Qt($,T)},buildCloud:z,clearPosts:()=>i.clear(),loadPosts:()=>i.load()};function q(){if(p.size===0)return null;const y={minX:1/0,minY:1/0,maxX:-1/0,maxY:-1/0};for(const C of p.values())y.minX=Math.min(y.minX,C.tx-C.width/2),y.minY=Math.min(y.minY,C.ty-C.height/2),y.maxX=Math.max(y.maxX,C.tx+C.width/2),y.maxY=Math.max(y.maxY,C.ty+C.height/2);return y}function P(){const y=[],C=new Map,A=[];for(const L of p.values())T.has(L.id)?(y.push({x:L.tx-L.width/2,y:L.ty-L.height/2,w:L.width,h:L.height}),C.set(L.id,{x:L.tx,y:L.ty})):A.push(L);A.sort((L,K)=>K.count-L.count),Jt(A,y,C,R)}async function D(){try{R=await u.fetchTagCooccurrence("tags",8),R.size>0&&P()}catch{}}const[v,_,S]=s.chrome.buttons;return _.addEventListener("click",()=>s.recenter()),v.addEventListener("click",()=>{w=!0,ce(G)}),S.addEventListener("click",()=>{T.clear(),Qt($,T);for(const C of p.values())C.tx=0,C.ty=0;const y=Array.from(p.values()).sort((C,A)=>A.count-C.count);Jt(y,[],new Map,R),l.fitToView(q(),{animate:!0}),D()}),z(),ce(G),s.start({themeChanged:()=>{O=ot(o);for(const y of p.values())y.hue=Zt(y.slug||y.name,O),E(y)},center:y=>{const C=p.get(y);return C?{x:C.x,y:C.y,tone:_o(C.hue)}:null},countReconciled:(y,C)=>{const A=p.get(y);A&&A.count!==C&&(A.count=C,s.terms=s.terms.map(L=>L.id===y?{...L,count:C}:L),x(A))},focusChanged:()=>{for(const y of p.values())E(y);ce(G)},bounds:q,frame:y=>{F(y),i.ease(),i.drawEdges(),U()},countsChanged:()=>{const y=Math.max(1,...s.terms.map(C=>C.count));for(const C of s.terms){const A=p.get(C.id);A&&(A.count=C.count,A.fontSize=xo(C.count,y),x(A))}i.focusId!==null&&ce(G)},dragging:()=>h!==null,pointerMove:(y,C)=>h?(h.x=C.x+g.x,h.y=C.y+g.y,h.tx=h.x,h.ty=h.y,!0):!1,pointerUp:j,cancelGesture:()=>{h&&b&&(h.x=h.tx=b.x,h.y=h.ty=b.y),h=null,m=null,b=null},search:y=>Array.from(p.values()).filter(C=>C.name.toLowerCase().includes(y)||C.slug.toLowerCase().includes(y))}),D(),()=>s.teardown()}const Na=Ii("desktop-mode-posts",{cells:{tags:wa,categories:Mn},terms:{categories:ya,tags:Ra}});return lt.default=Na,Object.defineProperties(lt,{__esModule:{value:!0},[Symbol.toStringTag]:{value:"Module"}}),lt}({});
